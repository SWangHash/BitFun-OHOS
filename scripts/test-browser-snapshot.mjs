...
try {
  ...
  if (process.argv.includes('--native-ocr')) {
    ...
    const nativeCode = await new Promise((resolveExit, reject) => {
      ...
      const child = spawn(process.env.BITFUN_TEST_BINARY || 'cargo', process.env.BITFUN_TEST_BINARY ? nativeArgs : ['test', '-p', 'bitfun-desktop', '--lib', 'native_vision_reads_rendered_fixture', '--', ...nativeArgs], {
        cwd: root, stdio: 'inherit', windowsHide: true,
