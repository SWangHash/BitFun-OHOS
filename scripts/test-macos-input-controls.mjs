...
const directory = await mkdtemp(join(tmpdir(), 'bitfun-directed-input-'));
...
try {
  ...
  let testBinary = process.env.BITFUN_TEST_BINARY;
  if (!testBinary) {
    testBinary = await new Promise((resolve, reject) => {
      const build = spawn('cargo', ['test', '-p', 'bitfun-desktop', ...(orchestration ? ['--features', 'devtools', '--test', 'computer_use_native_roundtrip'] : ['--lib']), '--no-run', '--message-format=json'], {
      ...
      build.stdout.on('data', chunk => {
        ...
        for (const line of lines) {
          ...
          if (message.reason === 'compiler-artifact' && message.target.name === (orchestration ? 'computer_use_native_roundtrip' : 'bitfun_desktop_lib') && message.executable) executable = message.executable;
  ...
  const targetBinary = await bundleExecutable('BitFun Input Target', 'dev.bitfun.input-fixture.target');
  const observerBinary = await bundleExecutable('BitFun Input Observer', 'dev.bitfun.input-fixture.observer');
  ...
  const testEnvironment = {
    ...process.env, BITFUN_INPUT_FIXTURE_PID: pid,
    BITFUN_INPUT_FIXTURE_RESULT: result, BITFUN_INPUT_OBSERVER_RESULT: `${result}.observer`,
    BITFUN_INPUT_HOST_READY: hostReady,
  };
  ...
  observer = spawn(observerBinary, ['--observer', `${result}.observer`, String(initialWindow)], { cwd: root, stdio: ['ignore', 'pipe', 'inherit'], windowsHide: true, env: { ...process.env, BITFUN_INPUT_COVER_TARGET: semantic ? '1' : '0' } });
  ...
  if (orchestration) {
    ...
    await run(testBinary, testArgs, { ...testEnvironment, BITFUN_INPUT_OBSERVER_PID: String(observer.pid) });
  }
