...
try {
  ...
  const command = process.env.BITFUN_TEST_BINARY || 'cargo';
  const args = process.env.BITFUN_TEST_BINARY ? testArgs : ['test', '-p', 'bitfun-desktop', '--lib', 'native_directed_input_fixture_counts_and_stop_release', '--', ...testArgs.slice(1)];
  await run(command, args, {
    ...process.env, BITFUN_INPUT_FIXTURE_PID: pid, BITFUN_INPUT_FIXTURE_X: x,
    BITFUN_INPUT_FIXTURE_Y: y, BITFUN_INPUT_FIXTURE_RESULT: result,
  });
