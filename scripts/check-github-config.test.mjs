...
test('Relay image rebuild resolves the release tag instead of the newer workflow commit', (t) => {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'bitfun-relay-rebuild-ref-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  ...
  for (const checkoutRef of ['', 'newer-branch']) {
    ...
    `], {
      ...
      env: {
        ...
        GITHUB_REPOSITORY: 'GCWing/BitFun', GITHUB_OUTPUT: output,
...
test('Relay image tag selection keeps Beta and old stable backfills away from latest', {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'bitfun-image-tags-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  for (const scenario of [
    ...
    const result = spawnSync('bash', ['-c', `gh() { printf 'v1.0.0\\n'; }\n${step.run}`], {
      env: { ...process.env, GITHUB_OUTPUT: output, GITHUB_REPOSITORY: 'test-owner/BitFun',
        IMAGE: 'ghcr.io/test-owner/bitfun-relay-server', RELEASE_TAG: `v${scenario.version}`,
    ...
    assert.deepEqual(tags, [
      'value<<EOF', `ghcr.io/test-owner/bitfun-relay-server:v${scenario.version}`,
      `ghcr.io/test-owner/bitfun-relay-server:${scenario.version}`,
      ...(scenario.latest ? ['ghcr.io/test-owner/bitfun-relay-server:latest'] : []), 'EOF',
    ]);
  }
});
...
test('beta channel readback retries stale content and fails if it never converges', {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'bitfun-beta-readback-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
...
test('nightly and beta use the shared build-version projection', () => {
  ...
  assert.equal(
    ...
    'https://bitfun.com/release/latest-v1.json',
  );
...
test('public beta launch uses Latest while legacy updater bytes stay on 0.2.19', () => {
  ...
  for (const version of ['0.2.19', '1.0.0-beta']) {
    const cwd = mkdtempSync(path.join(tmpdir(), 'bitfun-legacy-feed-'));
    try {
...
test('release publication omits target overrides and uploads before publishing', (t) => {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'bitfun-publish-api-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
