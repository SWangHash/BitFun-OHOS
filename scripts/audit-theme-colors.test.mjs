...
test('theme CSS var contract registry is explicit and non-overlapping', () => {
  ...
  for (const contract of PACKAGE_CSS_VAR_DEFINITION_CONTRACTS) {
    ...
    assert.equal(
      contract.variables.every(name => /^--bitfun-[a-z0-9-]+$/.test(name)),
      true,
      `${contract.packageName} must expose canonical --bitfun-* variables only`,
    );
  }
  assert.ok(packageVariableNames.includes('--bitfun-space-1'));
  assert.ok(packageVariableNames.includes('--bitfun-color-surface-canvas'));
  assert.ok(CANONICAL_THEME_COLOR_TOKENS.length > 0);
  assert.equal(
    CANONICAL_THEME_COLOR_TOKENS.every(token => (
      /^--bitfun-[a-z0-9-]+$/.test(token.cssVariable)
...
test('theme color audit emits scoped machine-readable reports', (t) => {
  const { dir, sourceRoot } = createFixture({
    'design-system/packages/theme-bitfun/src/test.tokens.css': [
      ':root {',
      '  --bitfun-appearance-token-color-text-primary: #111111;',
      '  --static-only: #222222;',
      '}',
      '',
    ].join('\n'),
...
test('theme color audit reports static contract token external consumption', (t) => {
  const { dir, sourceRoot } = createFixture({
    ...
    'app/App.scss': [
      '.app {',
      '  color: var(--bitfun-appearance-token-color-text-primary);',
...
test('theme color audit does not require a widget payload on surfaces without that contract', (t) => {
  const { dir, sourceRoot } = createFixture({
    'main.tsx': "import '@bitfun/theme-bitfun/default.css';\n",
    'app/App.scss': '.app { color: var(--bitfun-color-content-primary); }\n',
  });
  t.after(() => fs.rmSync(dir, { recursive: true, force: true }));
...
test('theme color audit counts only the generated widget variable contract', (t) => {
  const { dir, sourceRoot } = createFixture({
    'design-system/packages/theme-bitfun/src/test.tokens.css': [
      ':root {',
      '  --bitfun-appearance-token-color-bg-primary: #121214;',
      '  --bitfun-appearance-token-color-bg-secondary: #1c1c1f;',
      '  --bitfun-font-family-mono: monospace;',
      '}',
      '',
    ].join('\n'),
    'infrastructure/appearance/adapters/widgetAppearanceVariables.ts': [
      'const FALLBACK_VAR = {',
      "  bgPrimary: '--bitfun-appearance-token-color-bg-primary',",
      "  bgSecondary: '--bitfun-appearance-token-color-bg-secondary',",
      '} as const;',
      '',
      'const WIDGET_APPEARANCE_STATIC_SHELL_VARS = {',
      "  '--bitfun-appearance-token-btn-primary-bg': 'var(--bitfun-appearance-token-color-bg-primary)',",
      '} as const;',
      '',
      'export const WIDGET_APPEARANCE_VARIABLE_NAMES = [',
      "  '--bitfun-appearance-token-color-bg-primary',",
      "  '--bitfun-appearance-token-color-bg-secondary',",
...
test('theme color audit reports fallback tokens that lack a boundary contract', (t) => {
  const { dir, sourceRoot } = createFixture({
    'app/App.scss': [
      '.app {',
      '  color: var(--runtime-accent, var(--bitfun-appearance-token-color-accent-500));',
      '}',
      '',
    ].join('\n'),
  });
...
test('theme color audit reports specialized color domains separately from app UI', (t) => {
  const { dir, sourceRoot } = createFixture({
    'design-system/packages/theme-bitfun/src/test.tokens.css': ':root { --bitfun-appearance-token-color-text-primary: #111111; }\n',
    ...
    'tools/generative-widget/appearancePayload.ts': "export const fallback = { '--bitfun-appearance-token-color-text-primary': '#666666' };\n",
...
test('theme color audit can resolve peer-owned public package variables explicitly', (t) => {
  const { dir, sourceRoot } = createFixture({
    'components/Button.css': '.button { color: var(--bitfun-color-content-primary); padding: var(--bitfun-space-2); }\n',
  });
  t.after(() => fs.rmSync(dir, { recursive: true, force: true }));
...
test('theme color audit reports app literals that duplicate token values', (t) => {
  const { dir, sourceRoot } = createFixture({
    'design-system/packages/theme-bitfun/src/test.tokens.css': [
      ':root {',
      '  --bitfun-color-accent-default: #3b82f6;',
      '  --bitfun-color-status-warning-content: #f59e0b;',
      '}',
      '',
    ].join('\n'),
    'app/App.scss': [
  ...
  assert.deepEqual(
    ...
    [['--bitfun-color-accent-default'], ['--bitfun-color-status-warning-content']],
  );
...
test('theme color audit excludes test files from production color budgets', (t) => {
  const { dir, sourceRoot } = createFixture({
    'design-system/packages/theme-bitfun/src/test.tokens.css': ':root { --bitfun-color-status-danger-content: #ef4444; }\n',
...
test('theme color audit accepts registered dynamic CSS var families', (t) => {
  const { dir, sourceRoot } = createFixture({
    'infrastructure/appearance/adapters/ThemeTokenAppearanceAdapter.ts': [
      ...
      "  document.documentElement.style.setProperty(`--bitfun-font-size-${key}`, value);",
      '}',
      '',
    ].join('\n'),
  });
...
test('theme color audit requires exact exports for dynamic-family CSS var usages', (t) => {
  const { dir, sourceRoot } = createFixture({
    'design-system/packages/theme-bitfun/src/test.tokens.css': [
      ':root {',
      '  --bitfun-appearance-token-color-purple-200: rgba(139, 92, 246, 0.15);',
      '}',
      '',
    ].join('\n'),
    'infrastructure/appearance/adapters/ThemeTokenAppearanceAdapter.ts': [
      "for (const [key, value] of Object.entries(theme.colors.purple)) {",
      "  document.documentElement.style.setProperty(`--bitfun-appearance-token-color-purple-${key}`, value);",
      '}',
      '',
    ].join('\n'),
    'app/components/NavPanel/Tabs.scss': [
      '.tabs {',
      '  border-color: var(--bitfun-appearance-token-color-purple-300);',
      '}',
      '',
    ].join('\n'),
  });
  ...
  assert.deepEqual(
    ...
    [['--bitfun-appearance-token-color-purple-300', '--bitfun-appearance-token-color-purple-']],
  );
  ...
  assert.match(`${blocked.stdout}\n${blocked.stderr}`, /--bitfun-appearance-token-color-purple-300/);
});
