...
export BITFUN_ATSPI_FIXTURE_READY="$fixture_ready"
python3 - <<'PY' &
import gi
import os
gi.require_version('Gtk', '3.0')
from gi.repository import Gtk, GLib
window = Gtk.Window(title='BitFun AT-SPI Fixture')
...
window.show_all()
def ready():
    with open(os.environ['BITFUN_ATSPI_FIXTURE_READY'], 'w') as stream:
        stream.write('ready')
    return False
...
PY
fixture_pid=$!
export BITFUN_ATSPI_FIXTURE_PID="$fixture_pid"
