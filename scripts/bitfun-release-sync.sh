#!/usr/bin/env bash
#
# sync-release.sh — Mirror BitFun release assets from GitHub to bitfun.com.
#
# Flow:
...
# to https://bitfun.com/release/latest-v1.json and downloads from this mirror.
...
#   */10 * * * * /root/repos/BitFun/scripts/bitfun-release-sync.sh \
#       >> /var/log/bitfun-release-sync.log 2>&1
# Host restore: deploy/bitfun-host/README.md
#
# Optional immediate trigger. The release workflow POSTs to
# ${BITFUN_SYNC_WEBHOOK_URL} once assets are published (see the "Request
# bitfun mirror sync" step in .github/workflows/desktop-package.yml) so a
...
# listener unless BITFUN_SYNC_WEBHOOK_URL is actually configured.
#
...
case "$RELEASE_CHANNEL" in
  ...
  beta)
    CHANNEL_PATH="/beta"
    GITHUB_RELEASE_ROOT="https://github.com/GCWing/BitFun/releases/download/channel-v1-beta"
    ;;
  *)
...
BITFUN_BASE_URL="https://bitfun.com/release${CHANNEL_PATH}"
...
# release; mirroring those first meant the window in which bitfun advertised
