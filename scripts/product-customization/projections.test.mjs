...
test('build environment isolates custom Cargo output without overriding the default', () => {
  ...
  assert.equal(customEnvironment.BITFUN_PRODUCT_ID, 'acme');
  assert.equal(customEnvironment.BITFUN_DATA_NAMESPACE, 'acme');
  assert.equal(customEnvironment.BITFUN_HIDDEN_DATA_DIRECTORY, '.acme');
