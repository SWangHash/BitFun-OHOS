...
const initial: InstallOptions = {
  installPath: 'D:\\BitFun', desktopShortcut: false, startMenu: true,
...
it('updates an install option once per label or native input click and keeps the path editable', async () => {
  ...
  const label = container.querySelector<HTMLLabelElement>('[data-bitfun-component="checkbox"]')!;
  ...
  const path = container.querySelector<HTMLInputElement>('[data-bitfun-component="input"] input')!;
  await act(async () => {
    Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')!.set!.call(path, 'D:\\Apps\\BitFun');
    path.dispatchEvent(new Event('input', { bubbles: true }));
  });
  expect(path.value).toBe('D:\\Apps\\BitFun');
  expect(clearError).toHaveBeenCalledTimes(1);
  expect(refreshDiskSpace).toHaveBeenLastCalledWith('D:\\Apps\\BitFun');
});
...
it('keeps path and install/navigation actions disabled during installation', async () => {
  ...
  for (const button of buttons) {
    ...
    expect(button.dataset.bitfunComponent).toBe('button');
    act(() => button.click());
  }
...
it('connects launch-after-install on the theme page and preserves window control handlers', async () => {
  ...
  const buttons = [...container.querySelectorAll<HTMLButtonElement>('[data-bitfun-component="icon-button"]')];
  act(() => buttons[0].click());
  act(() => buttons[1].click());
