...
`--bitfun-*` variables projected by [installerThemeRuntime.ts](src/theme/installerThemeRuntime.ts).

Buttons, inputs, fields, selectors, checkboxes, radio controls, page headers, and
native scrollbar styling come from the public `@bitfun/ui` package. The
