...
/// Detect an existing BitFun install (Tauri NSIS or this installer) via Add/Remove Programs registry.
#[tauri::command]
...
pub(crate) fn launch_legacy_data_migrator(
    ...
    let _ = request;
    Err("Data Migrator is distributed separately. Download and run BitFun Data Migrator to import legacy data.".to_string())
}
...
/// Actual install root is always under an `BitFun` directory: `{user choice}/BitFun`.
    ...
    fn installer_preferences_do_not_create_app_config() {
        tauri::async_runtime::block_on(async {
            ...
            apply_first_launch_language_at(&handoff_path, "en")
                .await
                .unwrap();
            apply_theme_preference_at(&handoff_path, "bitfun-light")
                .await
                .unwrap();
