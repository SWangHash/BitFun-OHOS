...
async function ensureLightAppearance(): Promise<void> {
  const isLight = await browser.execute(() => (
    document.documentElement.getAttribute('data-bitfun-appearance') === 'bitfun-light'
  ));
  if (isLight) return;
...
async function openLocalOverview(): Promise<WebdriverIO.Element> {
  ...
  await browser.waitUntil(
    async () => (await trigger.getAttribute('data-bitfun-state')) === 'local',
    {
      timeout: 15_000,
...
describe('L0 Device Overview', () => {
  it('presents local work as a complete, compact state', async () => {
    ...
    expect(await popover.getAttribute('data-bitfun-state')).toBe('local');
  ...
  it('opens the full connection configuration dialog without SSH', async () => {
    ...
    const dialog = await $('[data-bitfun-component="remote-connect-dialog"][data-bitfun-part="root"]');
    await dialog.waitForDisplayed({
      timeout: 10_000,
      timeoutMsg: 'The full Remote Connect dialog did not open',
    });
    await expectPopupCloseContract(
      '[data-bitfun-component="remote-connect-dialog"][data-bitfun-part="root"]',
    );
    expect(await $('[data-testid="nav-device-status-popover"]').isExisting()).toBe(false);

    const groupTabs = await $$('[data-bitfun-component="remote-connect-dialog"][data-bitfun-part="groupTab"]');
    ...
    const accountPanel = await $('[data-bitfun-component="remote-account-panel"]');
    await accountPanel.waitForDisplayed({ timeout: 10_000 });
    const accountView = await accountPanel.getAttribute('data-bitfun-view');
    if (accountView === 'login') {
      ...
      expect(await accountPanel.$('[data-bitfun-part="server"]').isDisplayed()).toBe(true);
    }
