...
describe('L1 Task Board', () => {
  ...
  it('keeps the reference hierarchy while running a real scheduled-task flow', async () => {
    ...
    expect(await $('.bitfun-todos__guide').isExisting()).toBe(false);
    ...
    expect(await row.$('.bitfun-todos__row-icon').isDisplayed()).toBe(true);
    ...
    await $('.bitfun-todos__today-button').click();
    await browser.waitUntil(async () => (
      await $('[data-testid="todos-calendar-month"]').getText()
    ) === currentMonth, {
      timeout: 5_000,
