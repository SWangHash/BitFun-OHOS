...
async function selectAppearance(appearanceId: string): Promise<void> {
  ...
  await browser.waitUntil(async () => {
    return browser.execute((expectedId: string) => {
      return document.documentElement.getAttribute('data-bitfun-appearance') === expectedId;
    }, appearanceId);
  }, {
    timeout: 10000,
    interval: 100,
...
describe('L0 Appearance', () => {
    ...
    await browser.waitUntil(async () => {
      return browser.execute(() => {
        ...
        return document.readyState === 'complete'
          && root.getAttribute('data-bitfun-appearance-root') === 'true'
          && root.getAttribute('data-bitfun-appearance') !== null
          && root.getAttribute('data-bitfun-appearance-mode') !== null;
      });
    }, {
      timeout: 20000,
      interval: 200,
  ...
  it('should expose the root Appearance contract', async () => {
    const appearance = await browser.execute(() => {
      const root = document.documentElement;
      return {
        id: root.getAttribute('data-bitfun-appearance'),
        mode: root.getAttribute('data-bitfun-appearance-mode'),
        revision: root.getAttribute('data-bitfun-appearance-revision'),
        isRoot: root.getAttribute('data-bitfun-appearance-root'),
      };
    });
  ...
  it('should expose compiled Appearance tokens', async () => {
    const appearanceStyles = await browser.execute(() => {
      ...
      const appearanceVariables = Array.from(styles)
        .filter(property => property.startsWith('--bitfun-color-'));

      return {
        variableCount: appearanceVariables.length,
        background: styles.getPropertyValue('--bitfun-color-surface-canvas').trim(),
        text: styles.getPropertyValue('--bitfun-color-content-primary').trim(),
        accent: styles.getPropertyValue('--bitfun-color-accent-default').trim(),
      };
    });
  ...
  it('should project the neutral and navy light palette into the native app', async () => {
    ...
    const lightNavigation = await browser.execute(() => {
      ...
      return {
        primary: styles.getPropertyValue('--bitfun-color-surface-canvas').trim(),
        scene: styles.getPropertyValue('--bitfun-color-surface-scene').trim(),
        softSurface: styles.getPropertyValue('--bitfun-color-action-secondary-background').trim(),
        text: styles.getPropertyValue('--bitfun-color-content-primary').trim(),
        mutedText: styles.getPropertyValue('--bitfun-color-content-muted').trim(),
        accent: styles.getPropertyValue('--bitfun-color-accent-default').trim(),
        primaryButton: styles.getPropertyValue('--bitfun-color-action-primary-background').trim(),
        successBackground: styles.getPropertyValue('--bitfun-color-status-success-surface').trim(),
        errorBackground: styles.getPropertyValue('--bitfun-color-status-danger-surface').trim(),
        border: styles.getPropertyValue('--bitfun-color-border-default').trim(),
  ...
  it('should compile inverse new-session hover colors for the light appearance', async () => {
    ...
    const hoverContract = await browser.execute(() => {
      ...
      return {
        appearanceMode: document.documentElement.getAttribute('data-bitfun-appearance-mode'),
        textPrimary: rootStyle.getPropertyValue('--bitfun-color-content-primary').trim(),
        scene: rootStyle.getPropertyValue('--bitfun-color-surface-scene').trim(),
    ...
    expect(hoverContract).toMatchObject({
      ...
      textPrimary: '#1c1c1f',
      scene: '#ffffff',
      buttonBackground: 'var(--bitfun-color-content-primary)',
      buttonBorder: 'var(--bitfun-color-content-primary)',
      iconColor: 'var(--bitfun-color-surface-scene)',
  ...
  it('should keep Mini Apps and the selected navigation surface visually light', async () => {
    ...
    const navBack = await waitForDisplayed(
      '[data-bitfun-component="nav-bar"][data-bitfun-part="back"]:not(.is-inactive)',
    );
    await navBack.click();

    const miniAppsEntry = await waitForDisplayed('[data-testid="nav-miniapps-entry"]');
    await miniAppsEntry.click();
    await waitForDisplayed('[data-bitfun-scene="miniapp-gallery"]');
    await waitForDisplayed('.miniapp-gallery-scene__tabs .bitfun-tabs__tab--active');
    await waitForDisplayed('[data-bitfun-component="mini-app-card"]', 20000);
    ...
    const miniAppPresentation = await browser.execute(() => {
      ...
      const scene = document.querySelector<HTMLElement>('[data-bitfun-scene="miniapp-gallery"]');
      ...
      const card = document.querySelector<HTMLElement>('[data-bitfun-component="mini-app-card"]');
          ...
          ? (() => {
              ...
              return {
                ...
                neutralActionBackground: window.getComputedStyle(card)
                  .getPropertyValue('--bitfun-color-action-neutral-surface')
                  .trim(),
    ...
    await saveElementScreenshot(
      '[data-bitfun-component="mini-app-card"]',
      'l0-appearance-light-miniapp-card',
    );
    await saveElementScreenshot(
      '[data-bitfun-scene="miniapp-gallery"]',
    ...
    const miniAppHoverContract = await browser.execute(() => {
      ...
      const lightInverseRule = styleRules.find(rule => (
        rule.selectorText.includes('data-bitfun-appearance')
    ...
    expect(miniAppHoverContract).toEqual({
      cardBackground: 'var(--bitfun-color-surface-subtle)',
      cardBorder: 'var(--bitfun-color-border-default)',
      cardShadow: 'var(--bitfun-shadow-sm)',
      ...
      lightInverseBackground: 'var(--bitfun-color-content-on-light)',
      lightInverseColor: 'var(--bitfun-color-content-on-dark)',
    });
  ...
  it('should render monochrome structural chrome against a white workspace', async () => {
    ...
    const contrastPresentation = await browser.execute(() => {
      ...
      return {
        appearanceId: document.documentElement.getAttribute('data-bitfun-appearance'),
        appearanceMode: document.documentElement.getAttribute('data-bitfun-appearance-mode'),
        contentBackgroundToken: rootStyles.getPropertyValue('--bitfun-color-surface-canvas').trim(),
        contentTextToken: rootStyles.getPropertyValue('--bitfun-color-content-primary').trim(),
        contentSecondaryTextToken: rootStyles.getPropertyValue('--bitfun-color-content-secondary').trim(),
        contentBorderToken: rootStyles.getPropertyValue('--bitfun-color-border-subtle').trim(),
        contentSurfaceToken: rootStyles.getPropertyValue('--bitfun-color-surface-subtle').trim(),
        configSectionBackgroundToken: rootStyles.getPropertyValue('--bitfun-component-config-page-section-background').trim(),
        configSectionBorderToken: rootStyles.getPropertyValue('--bitfun-component-config-page-section-border').trim(),
        configSectionBorderWidthToken: rootStyles.getPropertyValue('--bitfun-component-config-page-section-border-width').trim(),
        configDividerToken: rootStyles.getPropertyValue('--bitfun-component-config-page-divider').trim(),
        chromeBackgroundToken: navStyles.getPropertyValue('--bitfun-color-surface-canvas').trim(),
        chromeTextToken: navStyles.getPropertyValue('--bitfun-color-content-primary').trim(),
        bodyBackground: bodyStyles.backgroundColor,
        workbenchBackground: workbenchStyles.backgroundColor,
        navBackground: navStyles.backgroundColor,
        navTextToken: navStyles.getPropertyValue('--bitfun-color-content-primary').trim(),
        ...
        sceneBarTextToken: sceneBarStyles.getPropertyValue('--bitfun-color-content-primary').trim(),
        viewportBackground: viewportStyles.backgroundColor,
        viewportTextToken: viewportStyles.getPropertyValue('--bitfun-color-content-primary').trim(),
  ...
  it('should preserve design-system inline spacing on Appearance action buttons', async () => {
    await openAppearanceSettings();
    await waitForDisplayed('[data-bitfun-part="packageActions"] [data-bitfun-component="button"]');

    const buttonSpacing = await browser.execute(() => {
      return Array.from(document.querySelectorAll<HTMLElement>(
        '[data-bitfun-part="packageActions"] [data-bitfun-component="button"]',
      )).map((button) => {
  ...
  it('should switch to another built-in Appearance', async () => {
    ...
    const before = await browser.execute(() => ({
      id: document.documentElement.getAttribute('data-bitfun-appearance'),
      revision: document.documentElement.getAttribute('data-bitfun-appearance-revision'),
    }));
