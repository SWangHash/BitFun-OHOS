...
describe('L1 Chat Input Contract Controls', () => {
  ...
  it('orders add, Harness, and the selected Agent mode from left to right', async function () {
    ...
    const modeOptions = await $$([
      '[data-bitfun-boost-item-kind="mode"]',
      ':not([aria-disabled="true"])',
    ].join(''));
    let selectedAlternativeMode = false;
    for (const option of modeOptions) {
      const modeId = await option.getAttribute('data-bitfun-mode-id');
    ...
    const removeMode = await modeChip.$('[data-bitfun-part="modeChipRemove"]');
  ...
  it('activates Creative Harness with the standard catalog icon', async function () {
    ...
    expect(await creativeProfile.getAttribute('data-bitfun-state')).toBe('available');

    const creativeIcon = await creativeProfile.$('[data-bitfun-name="creative"]');
