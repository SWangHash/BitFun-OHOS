...
export class MiniAppsPage extends BasePage {
  ...
  private installedCard(appId: string): string {
    return `[data-bitfun-component="mini-app-card"]${this.appAttribute(appId)}`;
  }
  ...
  private runner(appId: string): string {
    return `[data-bitfun-scene="miniapp"]${this.appAttribute(appId)}`;
  }
  ...
  async openGallery(): Promise<void> {
    ...
    await this.waitForElement('[data-bitfun-scene="miniapp-gallery"]', 20000);
  ...
  async returnToGallery(): Promise<void> {
    ...
    await this.waitForElement('[data-bitfun-scene="miniapp-gallery"]', 10000);
  }
  ...
  async getRunningCardIds(): Promise<string[]> {
    const cards = await browser.$$(
      '[data-testid="miniapp-running-zone"] [data-bitfun-component="mini-app-card"][data-miniapp-id]',
    );
    const ids: string[] = [];
