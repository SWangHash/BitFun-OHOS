...
export class TaskBoardPage extends BasePage {
  ...
  async selectEmptyDay(): Promise<TaskBoardDayDetailMetrics> {
    ...
    for (const cell of cells) {
      ...
      if (!className.includes('bitfun-todos__calendar-cell--has-items')) {
        emptyCell = cell;
        break;
      }
    }
  ...
  private async deleteTaskRow(row: WebdriverIO.Element): Promise<void> {
    const deleteButton = await row.$('.bitfun-todos__row-action-buttons button:last-child');
    await deleteButton.click();

    const confirm = await $('[data-bitfun-component="confirm-dialog"][data-bitfun-part="actions"] button:last-child');
  ...
  async getLayoutMetrics(): Promise<TaskBoardLayoutMetrics | null> {
    return browser.execute(() => {
      ...
      const header = document.querySelector<HTMLElement>('.bitfun-todos__head');
      const panes = document.querySelector<HTMLElement>('[data-bitfun-part="panes"]');
      ...
      const todayButton = document.querySelector<HTMLElement>('.bitfun-todos__today-button');
      const navigationButton = document.querySelector<HTMLElement>('[data-testid="todos-calendar-prev"]');
      const weekdays = Array.from(document.querySelectorAll<HTMLElement>('.bitfun-todos__calendar-weekday'));
