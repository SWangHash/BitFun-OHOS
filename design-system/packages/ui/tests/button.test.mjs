...
test("Button exposes the neutral outline contract by default", () => {
  ...
  assert.match(markup, /data-bitfun-component="button"/);
  assert.match(markup, /data-bitfun-tone="neutral"/);
...
test("static labels retain native button semantics without adding overflow machinery", () => {
  ...
  assert.match(markup, /data-bitfun-part="label">Show previous attempts<\/span>/);
  assert.match(markup, /data-bitfun-part="leading-icon"/);
...
test("danger tone preserves destructive semantics independently from presentation", async () => {
  ...
  assert.match(styles, /\[data-bitfun-tone=danger\]/);
  assert.match(styles, /--bitfun-color-status-danger-content/);
  assert.match(styles, /--bitfun-color-status-danger-surface/);
...
test("fill styles bind to the public variant attribute", async () => {
  ...
  assert.match(styles, /\[data-bitfun-variant=fill\]/);
...
test("primary and text variants expose semantic emphasis without changing button anatomy", async () => {
  ...
  assert.match(primaryMarkup, /data-bitfun-variant="primary"/);
  assert.match(textMarkup, /data-bitfun-variant="text"/);
  assert.match(styles, /--bitfun-component-button-primary-background/);
  assert.match(styles, /--bitfun-color-action-primary-content/);
  assert.match(styles, /--bitfun-component-button-text-content/);
  assert.match(styles, /--bitfun-component-button-text-content-disabled/);
...
test("secondary exposes the filled secondary-action contract", async () => {
  ...
  assert.match(markup, /data-bitfun-variant="secondary"/);
  assert.match(styles, /--bitfun-color-surface-tertiary/);
  assert.match(styles, /--bitfun-color-border-default/);
  assert.match(styles, /--bitfun-color-border-strong/);
...
test("outline and text composite over the caller surface without changing their hit targets", async () => {
  ...
  assert.match(
    styles,
    /\[data-bitfun-variant="text"\]\s*\{[^}]*--_button-background:\s*transparent/s,
  );
  const textRule = styles.match(/\[data-bitfun-variant="text"\]\s*\{([^}]+)\}/)?.[1] ?? "";
  ...
  assert.doesNotMatch(styles, /\[data-bitfun-variant="text"\][^}]*padding-inline:\s*0/s);
});

test("Button owns the reference pill geometry and typography", async () => {
  const styles = await readFile(new URL("../dist/styles.css", import.meta.url), "utf8");

  assert.match(styles, /letter-spacing:var\(--bitfun-type-label-md-letter-spacing\)/);
  assert.match(styles, /border-radius:var\(--bitfun-radius-pill\)/);
  assert.match(styles, /--_button-height:\s*var\(--bitfun-control-height-md\)/);
  assert.match(styles, /--_button-leading-icon-size:\s*16px/);
  assert.match(styles, /--_button-trailing-icon-size:\s*14px/);
  assert.match(styles, /--_button-font-size:\s*var\(--bitfun-type-label-md-font-size\)/);
  assert.match(styles, /font-family:var\(--bitfun-type-label-md-font-family\)/);
  assert.match(styles, /font-weight:var\(--bitfun-type-label-md-font-weight\)/);
  assert.match(styles, /padding-block:var\(--_button-padding-block\)/);
  assert.match(styles, /padding-inline:var\(--_button-padding-inline\)/);
  assert.match(styles, /--_button-padding-inline:\s*var\(--bitfun-space-5\)/);
  assert.match(
    styles,
    /\[data-size=xs\]\{[^}]*--_button-height:\s*var\(--bitfun-control-button-xs-height\)/,
  );
  assert.match(
    styles,
    /\[data-size=xs\]\{[^}]*--_button-padding-inline:\s*var\(--bitfun-control-button-xs-padding-inline\)/,
  );
  assert.match(
    styles,
    /\[data-size=sm\]\{[^}]*--_button-padding-inline:\s*var\(--bitfun-space-4\)/,
  );
  assert.match(
    styles,
    /\[data-size=lg\]\{[^}]*--_button-padding-inline:\s*var\(--bitfun-space-6\)/,
  );
  assert.match(
    styles,
...
test("Button keeps mixed child content vertically centered", async () => {
  ...
  assert.match(styles, /\.label\s*\{[^}]*gap:\s*var\(--bitfun-space-1\)/);
...
test("real and preview hover states share the component rule", async () => {
  ...
  assert.match(
    styles,
    /:is\(:hover,\s*\[data-bitfun-preview-state=(?:"hover"|hover)\]\):not\(:disabled\)/,
  );
});
...
test("real and preview active states share the semibold component rule", async () => {
  ...
  assert.match(
    styles,
    /:is\(:active,\s*\[data-bitfun-preview-state=(?:"active"|active)\]\):not\(:disabled\)/,
  );
  assert.match(styles, /font-weight:\s*var\(--bitfun-type-label-selected-font-weight\)/);
  assert.equal((styles.match(/--bitfun-type-label-selected-font-weight/g) ?? []).length, 1);
});

test("fill uses Button state colors and icons inherit content color", async () => {
  const styles = await readFile(new URL("../dist/styles.css", import.meta.url), "utf8");

  assert.match(styles, /--_button-background:\s*var\(--bitfun-component-button-fill-background\)/);
  assert.match(styles, /--_button-background-hover:\s*var\(--bitfun-component-button-fill-background-hover\)/);
  assert.match(styles, /--_button-background-active:\s*var\(--bitfun-component-button-fill-background-pressed\)/);
...
test("native disabled and loading use variant-specific disabled content with a neutral danger fallback", async () => {
  ...
  for (const variant of ["primary", "text"]) {
    const rule = styles.match(new RegExp(`\\[data-bitfun-variant="${variant}"\\]\\s*\\{([^}]+)\\}`))?.[1] ?? "";
    assert.match(rule, new RegExp(`--_button-content-disabled:\\s*var\\(--bitfun-component-button-${variant}-content-disabled\\)`));
    const markup = renderToStaticMarkup(createElement(Button, { variant, loading: true }, "Save"));
    assert.match(markup, /disabled=""/);
    assert.match(markup, /aria-busy="true"/);
  }
  assert.match(styles, /\[data-bitfun-tone="danger"\]\s*\{[^}]*--_button-content-disabled:\s*var\(--bitfun-color-action-neutral-content-disabled\)/);
});
