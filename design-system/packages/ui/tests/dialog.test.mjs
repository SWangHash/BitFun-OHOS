...
test("Dialog and Sheet compose the shared overlay kernel and compound anatomy", async () => {
  ...
  for (const part of [
    ...
    assert.match(source, new RegExp(`data-bitfun-part=\\"${part}\\"`));
  }
...
test("Dialog geometry and typography use public design tokens", async () => {
  ...
  for (const token of [
    ...
    "--bitfun-overlay-dialog-footer-padding-inline",
    "--bitfun-overlay-dialog-max-inline-size-xxlarge",
    "--bitfun-type-heading-dialog-font-size",
    "--bitfun-type-heading-dialog-font-weight",
    "--bitfun-color-overlay-scrim",
    "--bitfun-color-surface-raised",
    "--bitfun-shadow-overlay",
  ]) {
...
test("DialogFooter exposes a centered frosted floating action layer", async () => {
  ...
  assert.match(styles, /\[data-appearance=floating\]:before\{[^}]*backdrop-filter:var\(--bitfun-overlay-dialog-footer-blur\)/);
  assert.match(styles, /\[data-appearance=floating\]\{[^}]*pointer-events:auto/);
  assert.match(styles, /--bitfun-overlay-dialog-footer-action-min-width/);
});
