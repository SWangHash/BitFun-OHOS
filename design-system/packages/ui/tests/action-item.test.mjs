...
test("ActionItem exposes native trigger semantics and independent content areas", () => {
  ...
  assert.match(markup, /data-bitfun-component="action-item"/);
  assert.match(markup, /<button[^>]+role="menuitem"/);
  assert.match(markup, /data-bitfun-part="leading"/);
  assert.match(markup, /data-bitfun-part="label"[^>]*data-overflow-behavior="marquee"[^>]*><span[^>]*>AI Assistant<\/span><\/span>/);
  assert.match(markup, /data-bitfun-part="shortcut"/);
...
test("ActionItem renders trailing metadata between the label and the shortcut", () => {
  ...
  const label = markup.indexOf('data-bitfun-part="label"');
  const metadata = markup.indexOf('data-bitfun-part="metadata"');
  const shortcut = markup.indexOf('data-bitfun-part="shortcut"');

  assert.match(markup, /data-bitfun-part="metadata"[^>]*>12<\/span>/);
  assert.doesNotMatch(markup, /data-bitfun-part="metadata"[^>]*aria-hidden/);
...
test("ActionItem can reserve an empty leading gutter for aligned lists", () => {
  ...
  assert.match(markup, /data-bitfun-part="leading"><\/span>/);
});
...
test("ActionItem exposes a danger tone for destructive rows", () => {
  ...
  assert.match(neutralMarkup, /data-bitfun-tone="neutral"/);
  assert.match(dangerMarkup, /data-bitfun-tone="danger"/);
});
