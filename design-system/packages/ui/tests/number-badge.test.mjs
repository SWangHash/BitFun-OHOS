...
test("NumberBadge renders caller-owned values, including zero and long counts", () => {
  for (const value of [0, 18, 1234, "99+"]) {
    ...
    assert.match(markup, /data-bitfun-component="number-badge"/);
...
test("NumberBadge uses a 24px filled surface and 11px control typography without clipping", async () => {
  ...
  for (const token of ["space-6", "type-meta-font-size", "type-meta-font-family", "type-meta-font-weight", "color-action-neutral-content", "color-number-badge-background"]) assert.ok(source.includes(`--bitfun-${token}`), token);
