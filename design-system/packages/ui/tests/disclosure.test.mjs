...
test("Disclosure connects its trigger and region with accessible state", () => {
  ...
  assert.match(markup, /data-bitfun-component="disclosure"/);
  assert.match(markup, /data-open="true"/);
  assert.match(markup, /aria-expanded="true"/);
  assert.match(markup, /aria-controls="bitfun-disclosure-[^"]+-content"/);
  assert.match(markup, /role="region"/);
  assert.match(markup, /aria-labelledby="bitfun-disclosure-[^"]+-trigger"/);
...
test("Disclosure uses public geometry tokens and honors reduced motion", async () => {
  ...
  assert.match(styles, /--bitfun-layout-disclosure-trigger-min-block-size/);
  assert.match(styles, /--bitfun-layout-disclosure-content-padding-inline/);
