...
export function MobileBadge({ children, className, dot = false, leading, tone = "neutral", ...props }: MobileBadgeProps) {
  return <span {...props} className={classNames(styles.root, className)} data-bitfun-component="mobile-badge" data-tone={tone}>{dot && <span aria-hidden="true" className={styles.dot} />}{leading}<span>{children}</span></span>;
}
