...
export function MobileBanner({ action, children, className, icon, role, tone = "neutral", ...props }: MobileBannerProps) {
  return <div {...props} className={classNames(styles.root, className)} data-bitfun-component="mobile-banner" data-tone={tone} role={role ?? (tone === "danger" ? "alert" : "status")}>{icon && <span className={styles.icon}>{icon}</span>}<div className={styles.content}>{children}</div>{action && <div className={styles.action}>{action}</div>}</div>;
}
