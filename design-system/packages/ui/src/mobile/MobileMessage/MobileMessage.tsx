...
export function MobileMessage({ actions, children, className, roleType, ...props }: MobileMessageProps) {
  return <article {...props} className={classNames(styles.root, className)} data-bitfun-component="mobile-message" data-role={roleType}><div className={styles.content} data-bitfun-part="content">{children}</div>{actions && <div className={styles.actions}>{actions}</div>}</article>;
}
