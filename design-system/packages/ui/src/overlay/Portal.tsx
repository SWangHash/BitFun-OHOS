...
export function getOverlayHost(ownerDocument: Document): HTMLDivElement {
  const existing = ownerDocument.querySelector<HTMLDivElement>('[data-bitfun-overlay-host="true"]');
  if (existing) return existing;
  const host = ownerDocument.createElement("div");
  host.setAttribute("data-bitfun-overlay-host", "true");
  Object.assign(host.style, {
    position: "fixed", inset: "0", zIndex: "var(--bitfun-layer-overlay-host)", pointerEvents: "none",
  });
...
function LayerMount({
  ...
  const [ticket] = useState(() => ({
    id: Symbol("bitfun-overlay"), sequence: stack.reserveSequence(),
    ...
    <LayerStackContext.Provider value={stack}>
      <ParentLayerContext.Provider value={{ id: ticket.id, stack }}>
        <FieldSurfaceContext.Provider value="default">
          <div ref={slotRef} className={inline ? styles.inlineLayer : styles.layer} data-bitfun-overlay-layer=""
            data-bitfun-overlay-modal={modal || undefined} {...(!open ? { inert: "" } : {})}>
            {children}
          </div>
