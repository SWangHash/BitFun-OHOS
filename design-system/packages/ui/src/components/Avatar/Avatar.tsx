...
export function Avatar({ alt = "", children, className, icon, onError, shape = "circle", size = "md", src, ...props }: AvatarProps) {
  ...
  return (
    <span {...props} className={classNames(styles.root, className)} data-bitfun-component="avatar" data-bitfun-shape={shape} data-size={size}>
      {src && !imageFailed
        ? <img alt={alt} className={styles.image} data-bitfun-part="image" onError={() => { setImageFailed(true); onError?.(); }} src={src} />
        : icon !== undefined
          ? <span className={styles.content} data-bitfun-part="icon">{icon}</span>
          : <span className={styles.content} data-bitfun-part="text">{children}</span>}
    </span>
  );
}
...
export function AvatarGroup({ children, className, maxCount = 5, ...props }: AvatarGroupProps) {
  ...
  return <div {...props} className={classNames(styles.group, className)} data-bitfun-component="avatar-group">{visible}{hiddenCount > 0 && <Avatar aria-label={`${hiddenCount} more`}>+{hiddenCount}</Avatar>}</div>;
}
