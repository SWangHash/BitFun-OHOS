...
export function VoiceCallTranscript({ entries, status, header, className, compact, presentation = "voice", onLoadEarlier }: VoiceCallTranscriptProps) {
  ...
  return <div ref={viewportRef} className={classNames(styles.conversation, compact && styles.compactTranscript, className)}
    data-bitfun-component="voice-call-panel" data-bitfun-part="conversation"
    ...
    <div ref={contentRef} className={styles.entries} data-bitfun-part="entries">
      {header}
      {entries.map(entry => <div key={entry.id} data-transcript-id={entry.id} className={styles.entry}>
        <div className={styles[entry.role]} data-bitfun-part={entry.role === "status" ? "status" : `${entry.role}Transcript`}>
          {entry.content}
        </div>
        {entry.activity && <div className={styles.activity} data-bitfun-part="entryActivity" data-role={entry.role}>
          {entry.activity}
        </div>}
      </div>)}
