...
`default.css` synchronously imports the system token contract and all built-in theme variants. Theme selection is scoped to a `data-bitfun-design-system-root` element and uses independent `data-color-scheme` and `data-contrast` axes.
