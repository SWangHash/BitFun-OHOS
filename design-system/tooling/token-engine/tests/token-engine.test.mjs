...
test("resolves aliases and emits stable CSS variable names", () => {
  ...
  assert.equal(tokenNameToCssVariable("color.action.primary"), "--bitfun-color-action-primary");
  assert.match(renderCss(tokens), /--bitfun-color-action-primary: #146c4b;/);
});
...
test("can preserve exact and embedded references in generated CSS", () => {
  ...
  assert.match(css, /--bitfun-type-body-font-size: var\(--bitfun-font-size-base\);/);
  assert.match(
    css,
    /--bitfun-type-body-fluid-size: clamp\(var\(--bitfun-font-size-base\), 2vw, 18px\);/,
  );
});
