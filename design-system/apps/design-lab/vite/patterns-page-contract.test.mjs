...
test("Patterns composes only public non-FlowChat contracts", async () => {
  ...
  assert.match(page, /data-bitfun-pattern="settings-form"/);
  assert.match(page, /data-bitfun-pattern="navigation-panel"/);
  assert.match(page, /data-bitfun-pattern="search-command-surface"/);
  assert.match(page, /data-bitfun-pattern="device-card"/);
...
test("Patterns remain responsive and consume theme contracts", async () => {
  ...
  assert.match(styles, /\.pattern-settings \[data-bitfun-component="field"\]\[data-orientation="horizontal"\]\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/s);
