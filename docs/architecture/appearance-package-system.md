...
编译器将全局 token 投影为 `--bitfun-appearance-*` 变量。`materials` 是带视觉角色的可复用样式定义：
...
```css
:root[data-bitfun-appearance="example.aurora"]
  [data-bitfun-component="button"][data-bitfun-part="root"][data-bitfun-variant="primary"]:hover:not(:disabled) { ... }
```

产品控件复用组件库时，可在 descriptor 中声明
`componentAttribute: 'data-bitfun-product-component'`，由编译器匹配
`data-bitfun-product-component` / `data-bitfun-product-part`，避免覆盖库组件自己的标识。
...
`data-bitfun-component` / `data-bitfun-scene` 与 `data-bitfun-part` DOM 契约。审计同时验证 descriptor、
DOM 和 registry 三者闭合。
...
Scene 使用与组件相同的规则，但稳定根属性是 `data-bitfun-scene`。每个场景必须登记自己的
...
- `<style data-bitfun-appearance-runtime="revision">`；
- `data-bitfun-appearance`、`data-bitfun-appearance-mode`、`data-bitfun-appearance-revision`；
- `ResolvedAppearance`；
- renderer adapter 状态；
- 当前包的 blob URL 生命周期。
...
Skin runtime、manifest 或文件格式。公网入口固定为 `market.bitfun.com/skin/`，稳定 API 为
...
每个可见 owner 使用专属 surface 或明确的共享组件 surface。`data-bitfun-kind`、按源码路径生成的 kind
