...
OpenCode `v1.18.4` 已包含独立终端插件入口，依据稳定版
...
```mermaid
flowchart LR
  Config["TUI source"] --> Source["Source control"] --> Client["PluginRuntimeClient"]
...
```mermaid
flowchart LR
  Client["PluginRuntimeClient"] <--> Adapter["TUI adapter"]
  Adapter <--> Input["Input"]
  Adapter <--> State["State"]
  Adapter <--> View["View"]
  Adapter <--> Theme["Theme"]
...
TUI 与服务插件的兼容接口共享来源、依赖、Client、事件、期限和错误基础设施；入口启停、贡献注册和普通错误分别
管理，但兼容进程级事实相同时默认复用同一个 Plugin Host。

### 3.1 开发视图

| 开发部分 | 负责 | 不能承担 |
|---|---|---|
| Plugin Host 内的 TUI 适配库 | 加载 `@opencode-ai/plugin/tui` 入口，跟踪插件注册项并提供兼容接口 | 持有 Rust 终端句柄或直接绘制 Ratatui Frame |
...
- Internal plugins 先加载；已获来源许可的外部模块可以并行解析，但按固定顺序 import，保持命令、路由和副作用顺序。
- 代码更新只有在来源身份/完整性和更新策略允许，且 import 前可见的运行条件未扩大时，才能自动完成静态检查；加载新代码前先停止并确认旧 Host。软件包版本或完整性
...
- 只有 file plugin 在动态 import 之前的 resolve/install 阶段出现可重试错误（固定版具体为
...
| `api.client` | 复用 OpenCode 插件客户端兼容接口。 |
...
| `api.lifecycle.signal` | 插件停用或 TUI 退出前触发取消 | 先触发 signal，再执行清理。 |
| `api.lifecycle.onDispose(fn)` | 在 Plugin Host 内登记清理函数并返回取消登记函数 | 清理有期限；Host 失去健康时回收完整进程树，不能阻塞终端恢复。 |
...
| 基础 `Dialog`、`ui.dialog.replace` 或任意 children/render | 无法从任意 JSX 推导结构，返回 `unsupported(renderer-required)` | 不打开空白对话框，也不固定 modal mode。 |
...
同一稳定提交的规范文字与 `tui.ts` 类型若存在属性差异，以固定源码和可执行样例为验收依据，并记录差异，
不能把两边字段直接并集后宣称兼容。
...
- OpenCode Client 复用插件兼容接口；TUI 专用方法映射到 TUI Effect 归属模块。
- TUI Plugin 使用 v2 SDK Event union，服务插件使用 v1 Event union；两者分别生成版本化事件清单。
- 固定清单中的已知事件按版本化 schema 转发，并在进入插件前执行大小、深度和敏感字段处理。
...
- panic、Plugin Host 丢失和用户中断都必须恢复终端状态，插件 dispose 不得阻塞恢复。
...
独立 OpenTUI 子进程中离屏渲染受限区域，将 cell buffer、输入和焦点桥接到 Ratatui 区域。该方案需要解决双渲染树、
...
1. server/tui 两类入口、default export、入口回退、插件 id、版本范围和依赖安装脚本禁用。
2. Internal、软件包和文件插件的去重、并行解析、顺序 import、串行激活和确定性冲突。
...
5. 插件初始化回滚、运行崩溃、事件风暴、慢 handler、超大状态和 Plugin Host 丢失。
