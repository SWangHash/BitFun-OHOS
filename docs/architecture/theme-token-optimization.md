...
而是让设计系统成为普通 UI 颜色的唯一公共来源：组件只消费 canonical `--bitfun-*`，产品层只选择、
...
flowchart LR
  ...
  Runtime --> Root["canonical root --bitfun-*"]
  Runtime --> Scope["canonical scoped --bitfun-*"]
...
1. **System / semantic**：`--bitfun-color-*`、`--bitfun-shadow-*`、`--bitfun-effect-*`、`--bitfun-opacity-*`
   等由设计系统发布的公共变量，是普通 UI 的默认消费层。
2. **Component**：`--bitfun-component-*`。仅用于跨消费方稳定存在、又无法由 semantic Token 准确表达的
   组件差异；必须在 `appearanceTokenContract.ts` 中登记。
3. **Product domain**：`--bitfun-domain-*`。用于 Git lane、语言身份、syntax、inspector、工具类别等明确的
...
3. 存在独立、稳定、可说明的组件语义，在最窄 owner 中新增 `--bitfun-component-*`。
4. 属于产品或渲染专用域，进入 `--bitfun-domain-*` 或 renderer payload。
...
对话注释由 Web UI Appearance 的 `--bitfun-component-conversation-excerpt-accent` 承载 UIKit 青色，
...
并用 `--bitfun-component-update-material-cyan` 保留青色衬光；该组件 Token 复用公开浅色主题的
...
- 不定义 `--color-*`、`--lab-*` 或 `--bitfun-appearance-token-*` 等局部/历史公共前缀。
...
```ts
{
  "theme-tokens": {
    version: 1,
    settings: {
      tokens: { "--bitfun-color-*": "...", "--bitfun-component-*": "...", "--bitfun-domain-*": "..." },
      scopes: {
        chrome: { "--bitfun-color-*": "..." }
      }
    }
...
- `scopes.chrome` 只能重绑定设计系统已有的 canonical theme Token，并应用到
  `[data-bitfun-theme-scope="chrome"]`；scope 内不发明另一套 chrome 名称。
...
旧 CSS-token adapter、Token 投影层、`css-tokens` renderer 和 `--bitfun-appearance-token-*`
...
`data-bitfun-design-system-root`、`data-color-scheme`、`data-contrast` 和 `data-density` 选择已发布主题。
...
`--bitfun-*`，不得内联另一套启动色。该 CSS 和两个 Appearance manifest 一起由
`generate-startup-appearance-bootstrap.mjs` 从正式主题/Appearance 源生成；统一颜色审计执行 `--check`，
生成物漂移直接失败。

Native Mobile 预览的工具 chrome 消费 canonical `--bitfun-*`；设备画布消费 `--mobile-*` 这一受登记的 scoped
...
baseline 约束。运行时只把选中 preset 投影到 canonical `--bitfun-color-*` 子集。
...
稳定语义。它们必须留在对应 renderer/domain owner 中，并通过明确 payload 或 `--bitfun-domain-*` 消费；不得
