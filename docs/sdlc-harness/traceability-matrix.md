...
| 阶段性用户收益 | PRD-20 | 阶段收益编排、质量一致性检查、阶段风险副作用分析 | 实施计划、指标规格、质量数据面 | P-1、P0-P4 | P0/P1 回放低风险任务、安全确认、远程/不可支持状态和 Harness 主动配置发现；PR/团队场景作为“不显露、不阻断”的负向用例。P2 起再回放 PR/团队正向闭环 |
...
| 能力与操作判定 | PRD-09、PRD-16、PRD-17 | 未声明能力受限、策略不写死工具名、所有执行面复用现有安全 owner | 安全边界、OpenCode 扩展消费、质量数据面 | P-1、P0、P1 | tool、MCP、skills、插件、hook、内置工具回放用例；验证能力、目标对象、数据类别、来源、副作用信息和未知能力受限 |
| 工具复写安全 | PRD-17 | 工具复写显式授权、复写表按项目执行域生效 | 工具归属模块、OpenCode 扩展消费、安全边界 | P2 | 内置工具复写、同名工具冲突、权限撤销、hash 变化、远程执行主机变化用例；验证复写不跨项目且仍经安全边界 |
| 关键用例体验 | PRD-18 | 体验投影层、页面/面板布局、跨入口语义一致 | 产品需求、配置化策略画像、质量数据面 | P0、P1、P2 | P0/P1 回放质量保障要求较低的探索性改动、文档改动、安全确认、远程容器和 Harness 主动配置发现；P2 回放 PR 就绪和受管路径 |
