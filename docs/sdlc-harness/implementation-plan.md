...
| 安全常驻先行 | prompt 注入、凭据、网络、shell、删除、发布、hook/MCP 和 Harness 主动配置风险不等待后续治理阶段 |
...
| 扩展与插件 | 记录现有插件来源与静态诊断，不新建 Harness 扩展协议 | 消费归属模块提供的可用性、故障和降级诊断 | Harness 主动配置审核与受控工具复写 | 真实插件执行由产品架构和扩展计划独立交付；Harness 只在能力可用后消费 |
...
为准。实施时不能用 SDLC Harness 的空接口、静态候选或测试桩替代真实生产入口与端到端插件调用。
...
| P0 | 用户能低摩擦完成低风险任务，并看懂安全范围和未验证项 | 轻量项目理解、任务短摘要、安全确认、执行位置、Harness 主动配置发现、最小 QDP | 项目扫描、基础安全决策、事件记录、摘要视图、关键回放用例 | 普通任务可完成；安全越界不静默；低风险任务不被重流程拖慢 |
| P1 | 风险、远程和不可支持状态变得可理解，用户知道该补什么验证 | 弱提示、推荐检查、远程/沙箱降级、插件可用性与故障诊断、证据引用摘要 | 风险分类器、提示降噪、sandbox capability 事件、类型化插件诊断消费 | 高风险解释清楚；低风险仍快速；远程和不可支持状态有替代路径 |
...
| P0 | 项目打开、项目上下文条、任务短摘要、安全确认、远程执行位置、Harness 主动配置发现、最小 QDP | 低风险任务能完成；用户看到已验证/未验证、安全范围、执行位置和下一步 | 默认体验被安全/摘要/扫描拖慢；项目扫描误判后形成强策略；安全确认过多；沙箱能力被夸大；Harness 主动配置发现误阻断普通任务 | 回放低风险任务、安全高敏动作、远程容器、Harness 主动配置发现；摘要必须包含未验证项；安全与质量事件分开审计 | 首次有用动作以 P0 基线为准；项目扫描分层、可取消、可延后；安全确认和摘要渲染不阻塞工具结果展示；QDP 只采最小事件 |
| P1 | 风险提示、推荐检查、远程不可支持状态、沙箱降级说明、插件可用性与故障诊断、证据引用摘要 | 用户能理解风险和能力缺口，并知道补什么验证 | 风险提示泛化成噪音；推荐检查不可运行；远程缺口被本地验证掩盖；静态诊断被误认为插件可执行；证据引用缺少新鲜度 | 中/高风险用例必须说明原因、证据、建议和跳过后果；推荐检查要有不可运行分支；插件结果必须由对应 owner 校验和提交 | 风险分类和提示生成后台化；同类提示合并；诊断消费保留版本/epoch 与陈旧结果保护；证据引用只保存摘要和引用 |
...
|---|---|---|---|
| 默认体验变重 | P0/P1 把证据包、图谱、严格审查或团队规则提前显露 | 低风险任务完成变慢，用户认为产品只适合强流程项目 | 默认只展示短摘要和必要安全确认；复杂能力按 PR、团队、发布、事故和合规触发；打断率和首次有用动作耗时超基线时降级 |
...
| 风险漏报 | 小 diff 命中安全、权限、schema、远程、发布、Harness 主动配置或外部 owner 风险事实 | 高风险变更通过轻量路径，后续缺陷或安全事件返工 | 路径、动作、owner 风险事实和历史信号优先于 diff 行数；高风险低置信进入降级状态和推荐检查 |
...
| 插件和工具复写越权 | 外部插件、OpenCode 适配层或工具复写绕过归属模块直接影响权威状态 | 执行链不可信，插件语义与内置工具分裂 | 插件工具、Hook、事件和权限请求必须进入现有类型化 owner；工具复写绑定项目、来源、hash、执行域和撤销入口；插件或 Harness 不直接写通过、失败、阻断、审计或授权状态 |
...
| 关键用例清单 | 低风险开发、安全确认、远程上下文、PR 就绪、受管路径、Harness 主动配置审核的回放用例 | 每个 PRD-18 用例都有入口、默认状态、体验收益和验证方法 |
| 安全边界接口 | 权限、执行位置、沙箱等级组合、网络、凭据、Harness 主动配置审核、应急放行 | 安全决策不依赖门禁或质量策略 |
| 能力与操作判定 | tool、MCP、skills、插件、hook 和内置能力向现有安全 owner 提供能力、目标、数据类别、来源和副作用信息 | 新扩展复用同一策略判定；未声明或无法验证的能力默认受限 |
| 执行与沙箱矩阵 | 只固化 P0 必需的本地、远程、ACP、MCP、插件执行域和浏览器/桌面执行位置、隔离等级、降级原因 | `allow_in_sandbox` 只在真实隔离可用时出现；完整矩阵按执行面进入 P1+ |
| 扩展消费边界 | 类型化工具、Hook、公开事件、权限结果和插件诊断的现有接口 | 每项都能指出真实生产者、消费方和状态 owner；Harness 不新增跨能力统一消息或效果协议 |
...
| Harness 主动配置默认安全 | 发现 Harness 将直接执行的项目脚本、自定义工具和工作流配置，记录来源、hash、权限与执行域 | 未审核时不执行；不改变 OpenCode、MCP 等外部能力 owner 的自动发现和有效策略语义 |
| 低风险任务也有一致质量说明 | 低风险任务摘要必须包含已验证、未验证、跳过原因和建议补证路径 | 不要求严格审查、强制检查或完整需求分析 |
...
| 用户可见收益 | 必要实现 | 明确不做或延期 |
|---|---|---|
| 中风险动作有行内弱提示 | 路径、模块、操作、历史信号和安全敏感标签进入风险提示 v0 | 不默认弹窗，不默认严格审查 |
...
| 扩展诊断可消费但不干扰 | 消费归属模块发布的来源、可用性、故障、策略限制和降级诊断 | 不把来源或静态候选显示为插件可执行；Harness 不直接写插件、权限或工具权威状态 |
...
- 扩展无适配器、不可用或返回过期诊断时，不影响内核权威状态和无关任务。
...
| Harness 主动配置可先审后用 | Harness 直接执行的项目脚本、自定义工具和工作流配置记录来源、hash、权限、执行域和审核状态 | 不改变外部生态 owner 的加载语义 |
| 工具复写有边界 | 复用工具注册表、权限校验和撤销入口，复写绑定项目执行域 | 不建立第二套工具 ABI；外部插件执行按独立计划交付 |
...
- PR 就绪、受管路径、风险接受和 Harness 主动配置审核用例可回放。
...
| 项 | 进入实现前必须保持的边界 |
|---|---|
| 用户收益 | Review 后台运行并在父任务留下可恢复记录；复审形成同一记录的新修订；空白 child 被明确的准备、加载或失败状态替代 |
| 持久化与查询 | 现有 session-history owner 保存一个记录锚点和每次修订的有界投影，并提供按父任务或精确 PR identity 查询的只读摘要；UI 不以当前已加载 session 充当全量索引；归档、保留和删除按整条记录执行，断链历史只能只读降级，不能伪造锚点或用户处置 |
| 状态正确性 | 执行阶段、结果可用性、问题结论、证据覆盖和目标新鲜度分别投影；后端结构化结果必须独立保留 coverage/freshness 或等价原因，不能让 stale 覆盖 limited；只有同一幂等请求的启动恢复复用修订，已开始、失败、取消或过期后的再次审查创建新修订 |
| 问题连续性 | 分组键只关联同类问题，证据指纹保护用户处置；只有两者完全一致时才继承已解决或已忽略，本轮未观察到不等于已解决 |
| 范围诚实 | 本地变更能够可靠归因时使用受限复审；来源不确定时回退当前工作区完整目标；PR 复审重新检查当前完整 base/head，不宣称未证明的 provider delta 节省 |
| 明确不做 | 自动 Review、自动评论/批准/合并、自动 checkout、Reviewer 命令执行、固定专家入口、Review 专用遥测存储和组织分析 |
| 验证方向 | 元数据恢复、幂等启动、并发记录更新、记录级归档/删除、断链只读降级、workspace/Git range/PR 新鲜度、limited+stale 与问题同时保留、问题处置继承、remote persistence、无额外模型调用和无重复内容读取 |
...
| 模型和工具成本 | 严格审查、影响分析、评测回放、插件适配评估默认不进入低风险路径 | 改为用户触发、团队配置触发或离线任务 |
