...
请求携带的项目、信任、策略和工具目录版本只描述 BitFun 原生包的当时状态；OpenCode 的来源、执行、
...
- 已建立 `bitfun-services-core`、`bitfun-services-integrations`、`bitfun-agent-tools`、`tool-runtime`、`bitfun-tool-packs`、`bitfun-agent-runtime`、`bitfun-agent-workflows`、`bitfun-runtime-services`、`bitfun-product-domains`、`bitfun-product-capabilities` 等归属 crate。早期 `bitfun-harness` 描述符原型没有真实执行消费方，后续已删除；命名工作流策略由最小的 `bitfun-agent-workflows` 承载。
...
- Public API、Tool ABI 和事件转换的基础流程已建立：Agent Runtime SDK 只暴露 preview 接口；`bitfun-agent-tools` 提供已准备快照、默认副作用信息和旧调用拒绝；`bitfun-events` 提供 Desktop 与 Peer Host 共用的事件视图；Core 兼容路径不再重复映射字段，Tauri adapter 只负责交付。

## 4. 适配器边界与后续专项

- `bitfun-core` 仍承载兼容接口 / `product-full` 组装和少量迁移期适配器；不应继续新增归属逻辑。
