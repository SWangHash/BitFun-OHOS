...
“受管包”、`ProjectionOnlyPluginRuntimeClient`（仅提供静态视图，不提供 runtime read/dispatch）、`idempotency_key` 和
请求携带的项目、信任、策略和工具目录版本只描述 OpenBitFun 原生包的当时状态；OpenCode 的来源、执行、
...
- 已抽取 `openbitfun-core-types`、`openbitfun-events`、`openbitfun-runtime-ports`、`openbitfun-agent-stream` 等基础契约。
- 已建立 `openbitfun-services-core`、`openbitfun-services-integrations`、`openbitfun-agent-tools`、`tool-runtime`、`openbitfun-tool-packs`、`openbitfun-agent-runtime`、`openbitfun-agent-workflows`、`openbitfun-runtime-services`、`openbitfun-product-domains`、`openbitfun-product-capabilities` 等归属 crate。早期 `openbitfun-harness` 描述符原型没有真实执行消费方，后续已删除；命名工作流策略由最小的 `openbitfun-agent-workflows` 承载。
...
- `openbitfun-events` 已承接 backend event DTO、agentic event DTO、framework-neutral Agentic frontend event projection 和 platform-neutral `EventEmitter` trait；当前 Tauri transport 只负责 delivery。
...
- `openbitfun-core` 的 function-agent AI concrete acquisition 已从旧 `runtime_services` 路径收拢到明确的 core port adapter；Git / AI compatibility re-export 仍保留旧 public path。
- 产品组装已承接 `DeliveryProfile`、当前交付形态入口矩阵、`CapabilitySet`、feature group matrix、profile-scoped capability plan、product-full provider plan、service availability report、profile-scoped harness registry 入口与 legacy-route 行为保护，以及 `ProductAssembler` 对 explicit profile input、runtime services、harness registry 和 service requirement 的验证；core 只保留兼容 re-export。ProductFull / Desktop 保留完整能力；CLI / ACP / SDK 只注册 CodeAgent 清单和七个原子工具组，内置 Harness registry 为空但 SDK 显式注入接口保留；Server / Remote / Web / MobileWeb 不再 materialize product-full capability packs、feature groups、runtime services、tool groups 或 harness routes。
...
- 归属 crate 不得依赖回 `openbitfun-core`。
- `product-full` 保持兼容入口的完整产品能力集合，产品形态差异仍以产品组装的 capability plan 表达。
- 边界检查覆盖归属 crate 禁止依赖、旧路径仅作兼容入口、特性开关、六层路径解析、产品组装、session/config/context fact 归属、tool confirmation gate 归属和高风险归属回流。
- focused tests 覆盖当前 delivery profile 能力裁剪、ProductAssembler 缺失 service 报告、无直接 core 入口的空 capability plan、SDK fake provider / services / tool / harness / hook / workspace-scoped agent registry 完整流程，以及 runtime hook 顺序、timeout、错误策略和重复 id 拦截。
...
- Public API、Tool ABI 和事件转换的基础流程已建立：Agent Runtime SDK 只暴露 preview 接口；`openbitfun-agent-tools` 提供已准备快照、默认副作用信息和旧调用拒绝；`openbitfun-events` 提供 Desktop 与 Peer Host 共用的事件视图；Core 兼容路径不再重复映射字段，Tauri adapter 只负责交付。

## 4. 适配器边界与后续专项

- `openbitfun-core` 仍承载兼容接口 / `product-full` 组装和少量迁移期适配器；不应继续新增归属逻辑。
- 产品入口的能力裁剪已由产品组装 profile plan 表达；后续新增入口必须先明确 `ProductCoreDependencyMode`、unsupported / temporarily-unavailable 语义和兼容性测试；内部 `NotAvailable` 类错误进入 Server/API 前必须转换为稳定状态词。
- H1 剩余 owner 决策已迁出：dialog start route / outcome lifecycle 继续由 `agent-runtime` 给出可测试决策，tool pipeline 的 Task batch 策略由 `tool-execution` 持有，prompt runtime / workspace / user-context 组合由 `agent-runtime` 持有，AI model selector / cache-key 解析由 `openbitfun-ai-adapters` 持有。`openbitfun-core` 仍只保留 coordinator 调用、config IO、credential overlay、prompt 事实收集和 prompt-cache persistence IO 等 concrete adapter。
...
- Agent Runtime SDK 已具备 v1 preview workspace 内公开边界、最小 fake-provider 完整流程、runtime services / tool / harness / hook / workspace-scoped agent registry 注入基线、最小 feature 证明和外部 embedder 示例。若后续要独立发布为外部包，需要单独评审发布流程、crate packaging、semver 承诺和长期兼容策略。
- Skill registry 主体 owner 已收口到 `agent-runtime`：`openbitfun-core` 保留本地/远端扫描、config/registry IO、缓存和加载错误映射；内置 skill 分组、root/slot/key 事实、mode default/override、visible resolution、shadow 标记、mode skill info 和加载后 assistant payload 由 runtime 统一给出。
- Workspace runtime provider owner 已继续外移：本地 workspace FS/shell 实现由 `openbitfun-services-core` 的 `workspace-runtime` feature 持有，远端 SSH workspace FS/shell adapter 由 `openbitfun-services-integrations::remote_ssh` 持有；`openbitfun-core::agentic::workspace` 保留 binding、session storage 解析和旧路径 re-export。
- Review-platform 主体 owner 已从 `openbitfun-core` 外移到 `services-integrations`：provider detection、remote discovery、token store、provider DTO mapping、pagination、CI log extraction 和 GitHub / GitLab / GitCode HTTP/Git 集成由 `review-platform` feature 持有；core 只保留旧 public path、产品 data-dir 注入和 remote SSH unsupported 分类。
