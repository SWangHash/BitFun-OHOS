...
- 新接口必须有当前生产消费方、版本边界、验证方式和删除条件；空 profile、re-export、测试桩或未来矩阵不算消费方。
...
| Agent Runtime SDK | 已有无 `bitfun-core` 依赖的 v9 preview 接口和 smoke test；未接入真实执行路径的通用 Harness registry 已删除 | 发布边界仍需真实嵌入方证明 |
...
富历史及 Peer Host/ACP 的其余维护等未迁移能力继续集中在一个 Core 兼容接口。Agentic Event Queue 仍是唯一
...
这两类外部服务。结构化输出复用现有 Agent 事件对象；会话 ID 与
...
2. ACP 完整历史继续使用单次 Core 兼容恢复；在出现第二个真实消费者及经过授权的附件读取能力前，不为协议回放扩张通用 transcript。模型/模式目录与配置读取、MCP、ACP stdio 和协议转换生命周期继续留在各自现有归属模块。
...
编译门禁存在，启动页 PTY/ConPTY 生命周期、Chat resize/取消、`exec` Ctrl+C、本地模型 403/断流失败最终状态、Patch I/O 失败与发布归档冒烟测试已接入门禁。
CLI-P0 整体退出条件尚未满足；真实供应商审批流、OS 级终端初始化故障注入、兼容接口退出以及 ACP/Desktop 切换仍需分别验收。

### 4.3 依次切换 ACP 与 Desktop

- ACP 在 CLI 之后迁移，优先简化协议转换和权限/会话桥接，不把 ACP 生命周期下沉到 Agent Runtime。
...
首个可执行切片应只完整流程一种 standalone custom tool：真实来源 → worker → 原始校验 → Tool Runtime → 调用结果。
在该切片完成前：
...
2. 选择一个用户可见端到端能力，写清当前 owner、目标 owner、唯一写入方和删除条件。
3. 先补行为等价与边界失败用例，再切换生产调用方。
4. 删除或固定被替代路径，复核 Remote、错误、取消和恢复语义。
...
- 同一事实在兼容接口与目标 owner 中同时计算或写入；
- 通用消息对象、候选效果或描述符开始承载工具、Hook、Client、TUI 等不同语义；
- Remote 不支持时静默回本机，或 SDK 仍需要 `product-full` 却被描述为独立可用；
- 为迁移一次性重写全部 CLI、Desktop 或 Core，而没有可单独验收的端到端能力。
