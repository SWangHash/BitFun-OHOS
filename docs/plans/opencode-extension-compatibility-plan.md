...
Product Assembly registers adapter implementations with ExternalSourceControlPlane
```

- 通用层只认识开放的 `ecosystem_id`、来源限定身份、使用范围、执行域、状态、诊断和能力专属贡献，不按
...
- 来源目录按“提供者 + 来源限定身份”协调版本，provider discovery 独立并发且有期限；某个适配器解析失败、升级
  或删除来源时，只影响其自己的来源和贡献，不能阻塞或清空其他生态。目录型来源进一步记录命令粒度的读取失败，
  避免一个坏文件复活同目录中已稳定删除的命令。
- 文件观察是 OS 服务事实，候选版本与来源合并是协调器事实，OpenCode 路径和语义是 OpenCode adapter 事实，
  最终调用仍属于 Command/Tool/Subagent owner。

### 1.2 产品基线

- 用户全局和当前项目来源后台发现，不阻塞项目打开、TUI 输入或普通会话。
- 设置中提供统一“外部 AI 应用”入口，显示来源、使用范围、实际生效状态、受限原因和最近刷新结果；默认聚合，不要求
  用户理解文件级实现。
- 当前安全支持的内容可在用户明确调用时直接使用；尚缺运行能力的字段只显示“已识别但当前不可用”，不伪装成功。
- 来源可按当前执行域抑制、恢复和重新加载；观察器更新不得绕过用户的抑制选择。
- 更新先生成不可变候选版本，通过解析和校验后在同一次状态提交中切换。解析失败保留仍然合规的上一有效版本；稳定删除、显式停用
...
|---|---|---|---|
| PR1：来源目录 + OpenCode Command（已实现） | Desktop 可查看、抑制/恢复并刷新全局/项目 OpenCode 来源；交互式 TUI（ChatMode）可列出并执行支持的 `/command`；运行中修改、删除、恢复后自动刷新 | 通用来源目录与 `ExternalSourceControlPlane`；Prompt Command 契约；OpenCode Command adapter | JS/TS Tool 执行、Hook、MCP、OpenCode Client/Server、Subagent 执行、复制式导入 |
...
| PR3：OpenCode Subagent（已实现） | 全局/项目 agent 定义经一次非阻塞确认后进入现有 Subagent 归属模块，可选择、单次调用、更新和撤下；同名冲突由用户选择，unsupported 字段有明确诊断 | 现有 Subagent 归属模块 + 独立 Subagent 兼容接口 + 调用期间固定版本 | 原始 OpenCode 会话内核、完整 primary-agent 替换、外部 agent 续接、跨产品通用 agent JSON |
...
Tool 与 Subagent 不复用 Command 的贡献对象，只复用来源身份、状态、版本、诊断和观察生命周期。Codex 与
...
## 3. PR1：来源目录与 OpenCode Command 纵向完整流程
...
| 位置 | PR1 责任 | 不得承担 |
|---|---|---|
| `contracts/product-domains` | 开放生态 ID、来源限定身份、使用范围、状态/诊断、来源快照、Prompt Command 定义和 provider 端口 | OpenCode 路径、文件 IO、UI、具体适配器选择 |
...
| `assembly/external-sources` | 与 provider 无关的一次性版本切换、隔离降级、同名冲突目录和版本敏感选择 | 注册具体 adapter、按生态分支或解释生态文件 |
| `assembly/core` | 注册 adapter、按工作区协调刷新、定义偏好 schema/路径并通过服务原语持久化、连接 watcher 与产品入口 | 实现文件锁/原子写、复制 OpenCode parser、按生态分支能力行为 |
| `apps/cli` | 将可用外部 Command 以原生 `/command` 显示在 TUI 菜单并接入输入分发；来源标签及内部 candidate id 处理同名候选 | 解析 OpenCode 文件、注册假工具或公开生态限定命令 |
...
2. 各 provider discovery 由 Core 独立调度，当前期限为 5 秒；超时只回退该 provider。每个 provider 同时最多一个
   正在运行的 discovery，后续刷新复用该任务，防止不可取消的阻塞扫描耗尽线程池。
...
5. 外部命令执行前刷新，并以候选 ID + 命令内容版本校验菜单/冲突选择；显示后发生更新时拒绝旧选择，不得执行新版本。
...
8. 两个 provider 具有同名命令时，目录生成版本敏感的待选择项；用户选择前不激活任何候选。刷新、更新或移除
   ...
   候选也重新进入待确认状态，不能静默切换实现。偏好按执行域/命令族保存单个当前内容摘要和去重的曾冲突候选身份，
   不按内容版本累计无界历史。
9. Desktop 首屏使用非强制快照并后台刷新；首次发现完成前短轮询并保持“正在检查”，已完成选择的冲突不再停留在
   “需要你的选择”区块。工作区切换、轮询和设置写响应按请求使用范围、请求序号与目录快照版本校验；
...
- OpenCode fixture 固定 XDG 全局/项目、`config.json`、单复数目录、JSON/JSONC、Markdown、路径去重、覆盖、参数展开、
  大小/数量上限和受限字段。
- watcher 覆盖创建、连续写入、同卷原子替换、稳定删除与重新出现；目录切换不阻塞 TUI 输入。
...
- 用户确认后，每个 standalone Tool 启动独立 Node.js worker，提供 `load/invoke/cancel/dispose`。合作式取消先传递
  `AbortSignal`；脚本阻塞事件循环时在短宽限期后终止整个 worker。Tool 的 schema、调用权限、审计和
  模型暴露继续走现有 Tool Runtime，不建立第二套路由。

### 4.2 产品与决策语义

- Desktop 在“外部 AI 应用”中显示来源、文件、工作目录、工具名和直接文件/网络/环境/进程能力，并明确提示当前 worker
  不是 OS 沙箱。交互式 TUI（ChatMode）使用同一快照：`/extensions` 提供共享状态、刷新和 Safe Mode，状态栏只做一次非阻塞提醒，通用 `/tools` 入口以“外部 AI 应用”
...
- Desktop IPC 和 TUI 快照只包含静态摘要与决策 key，不传输模块源码。用户选择落盘使用 PR1 的跨进程锁、锁内
  合并和同卷原子替换；Desktop 与交互式 TUI（ChatMode）不分别维护偏好。

### 4.3 更新、删除与降级

1. watcher 对全局与项目 Tool 目录去抖后重扫；准备前再次发现并核对脚本身份和内容版本，预览后被替换的文件按
   stale revision 拒绝，不能执行未确认的新内容。
2. 已批准脚本在来源身份、runtime 和能力集合不变时后台重载；导出、schema 或 load 校验失败时撤下该脚本，
   显示 `load_failed`，不保留对已变化原位源码的旧 worker。PR2 尚无完整旧版本文件，因此拒绝继续加载，
   不伪装成“沿用上一版本”。
3. 稳定删除、来源抑制、主动停用或审批撤销会撤下 Tool Runtime 路由并 dispose worker；在途调用先收到取消，阻塞
   worker 在宽限期后终止。一个脚本的失败不得清空同来源 Command、其他脚本或其他生态 adapter。
4. 每脚本持久 worker 的普通请求 30 秒无响应时终止；合作取消在 500 ms 后硬终止，输出限制 1 MiB、协议帧
...
- 将 route、并发属性和 timeout owner 固定为单个 invocation plan，消除并发切换期间的二次读取窗口。
- 为来源目录瞬时不可读增加明确的 `unknown/last-good` 状态；只有内容摘要匹配的完整文件仍可校验时才继续服务，明确删除、
  停用和权限收紧仍立即撤下。
...
- Desktop 的“外部 AI 应用”设置和 TUI `/agents` 中的“外部 AI 应用”选项使用同一 Subagent 当前版本和
  `preference_revision` 校验的
  非阻塞审批与冲突操作；同一工作区的 Agent 决策串行提交，完成后刷新权威状态并说明具体对象。冲突候选在选择前原位展示模型、
  工具、执行域、安全来源标签、兼容影响和恢复动作；一次点击即可在同一受锁提交中完成“选择并批准”。Agents 场景只显示已激活的
  只读 `External · provider` 视图、`Single-run` 标记，并可跳转到统一管理入口；外部 prompt、绝对用户路径和内部行为摘要
  不进入 IPC 或普通列表摘要。
...
- `NodeScriptToolRuntime` 在共享外部 Tool 运行时首次初始化时解析 Node.js 可执行文件，并在当前进程内持有成功结果；
  只有缓存仍为空时，后续可用性检查或加载才重新执行当前进程可见范围内的 `which(node)`；可用性还会在 2 秒期限内
  实际执行 `node --version`，并从探测和 worker 环境移除 `NODE_OPTIONS/NODE_PATH`。该过程不会替换已持有路径、worker
...
明确延期到独立、由证据驱动的后续工作：OS/容器沙箱和 CPU/内存等硬资源限制、worker/prompt 全局预算、运行中 Tool
版本切换期间的调用保护、通用 watcher 事件限流、偏好记录压缩、完整 metrics/打点平台。它们分别涉及平台执行、安全模块、
...
- Skills 场景和设置列表沿用现有卡片/列表，在使用范围旁显示来源。普通项不增加确认步骤；按默认优先关系被覆盖的项
...
- 交互式 TUI 不提供 `/external-tools`、`/external-agents` 等按能力分裂的命令。共享控制使用 `/extensions`，工具使用通用 `/tools` 入口，并在内容中以
  ...
  状态栏提醒计数；编号操作绑定当前视图和稳定 key，改选仍由归属模块校验对应的当前能力版本和偏好版本。
...
- GUI/TUI 都保持原生异常语义：读取失败不假定变更成功，刷新失败保留已知可用内容并提示状态可能过期，存储异常使受影响
  名称直接拒绝，Remote 明确不支持且不回退本机。模型设置读取失败只要求修复设置并刷新，不把重启当作通用恢复动作。
