...
/// Inject data-bitfun-appearance-mode on the first <html> tag.
fn inject_data_appearance_mode(html: &str, appearance_mode: &str) -> String {
    ...
    if let Some(idx) = html.find("<html") {
        ...
        if let Some(close) = rest.find('>') {
            ...
            if tag.contains("data-bitfun-appearance-mode=") {
                return html.to_string();
            }
            let insert = format!(" data-bitfun-appearance-mode=\"{}\"", safe);
            return format!(
                "{}{}>{}",
...
mod tests {
    ...
    fn compile_with_request_preserves_legacy_compile_output() {
        ...
        assert!(compiled.contains("data-bitfun-appearance-mode=\"dark\""));
    ...
    fn inject_data_appearance_mode_skips_existing_attribute() {
        let source = MiniAppSource {
            html:
                r#"<!DOCTYPE html><html lang="zh-CN" data-bitfun-appearance-mode="dark"><body></body></html>"#
                    .to_string(),
            css: String::new(),
            ui_js: String::new(),
        ...
        assert_eq!(
            compiled
                .matches("data-bitfun-appearance-mode=\"dark\"")
                .count(),
            1
        );
    }
