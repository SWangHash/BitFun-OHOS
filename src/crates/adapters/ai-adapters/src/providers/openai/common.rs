...
async fn fetch_codex_chatgpt_models(
    client: &AIClient,
    url: &str,
...
async fn fetch_codex_chatgpt_models_with_retry(
    client: &AIClient,
    url: &str,
) -> Result<Vec<RemoteModelInfo>> {
    for attempt in 1..=CODEX_MODEL_DISCOVERY_MAX_ATTEMPTS {
        let result = tokio::time::timeout(
            ...
            fetch_codex_chatgpt_models(client, url),
        )
        .await
...
async fn list_codex_chatgpt_models(
    ...
    fetch_codex_chatgpt_models_with_retry(client, &url).await
}
...
mod tests {
    use super::{
        attach_tools, codex_subscription_model_allowed, is_known_codex_reasoning_model,
