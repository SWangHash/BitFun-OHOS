...
form?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (!((authorization.origin === 'https://github.com' && authorization.pathname === '/login/oauth/authorize') || (authorization.origin === 'https://auth.bitfun.com' && authorization.pathname === '/sign-in')) || !!authorization.username || !!authorization.password) {
      throw new Error(message('登录地址无效。', 'The sign-in URL is invalid.'));
    }
    if (authorization.origin === 'https://auth.bitfun.com') authorization.searchParams.set('locale', navigator.language);
