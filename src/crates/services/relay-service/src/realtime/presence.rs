...
pub(super) async fn register(
    ...
    tokio::spawn(async move {
        ...
        broadcast(&io, &state, &account);
    });
    true
}
pub(super) fn broadcast(io: &SocketIo, state: &AppState, account: &str) {
    let devices: Vec<_> = state
        .device_manager
        .online_devices(account)
        .into_iter()
        .map(|(id, name)| json!({"device_id":id,"device_name":name}))
        .collect();
