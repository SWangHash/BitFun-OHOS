...
fn abnormal_process_exit_releases_the_lease() {
    ...
    let mut child = command
        ...
        .arg("--nocapture")
        .env("BITFUN_EXCLUSIVE_FILE_LEASE_CHILD", "1")
        .env("BITFUN_EXCLUSIVE_FILE_LEASE_LOCK_PATH", &lock_path)
        .env("BITFUN_EXCLUSIVE_FILE_LEASE_READY_PATH", &ready_path)
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
...
fn abnormal_exit_child_holds_lease() {
    if std::env::var_os("BITFUN_EXCLUSIVE_FILE_LEASE_CHILD").is_none() {
        return;
    }
    let lock_path = std::path::PathBuf::from(
        std::env::var_os("BITFUN_EXCLUSIVE_FILE_LEASE_LOCK_PATH").expect("child lock path"),
    );
    let ready_path = std::path::PathBuf::from(
        std::env::var_os("BITFUN_EXCLUSIVE_FILE_LEASE_READY_PATH").expect("child ready path"),
    );
