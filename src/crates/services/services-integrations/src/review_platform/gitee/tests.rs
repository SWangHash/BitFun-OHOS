...
fn server(responses: Vec<Response>) -> (ProviderContext, Receiver<Vec<Request>>) {
    server_with_list_confirmations(responses, false)
}

fn list_server(responses: Vec<Response>) -> (ProviderContext, Receiver<Vec<Request>>) {
    server_with_list_confirmations(responses, true)
}

fn server_with_list_confirmations(
    responses: Vec<Response>,
    confirm_list: bool,
) -> (ProviderContext, Receiver<Vec<Request>>) {
    let listed: HashMap<String, Value> = responses
        .iter()
        .filter_map(|response| serde_json::from_str::<Value>(&response.body).ok())
        .filter_map(|value| value.as_array().cloned())
        .flatten()
        .filter_map(|value| {
            value
                .get("number")
                .map(|number| (number.to_string(), value.clone()))
        })
        .collect();
    ...
    std::thread::spawn(move || {
        ...
        while !responses.is_empty() || !pending.is_empty() {
            ...
            let request_path = requests.last().unwrap().url.path();
            let number = request_path.rsplit('/').next().unwrap();
            let response = if confirm_list && pending.remove(number) {
                ok(listed
                    .get(number)
                    .expect("confirmation must match a listed PR")
                    .clone())
            } else {
                let response = responses.pop_front().expect("unexpected mock request");
                if confirm_list && request_path.ends_with("/files") && response.status == 200 {
                    let files: Value = serde_json::from_str(&response.body).unwrap();
...
async fn list_uses_gitee_headers_and_documented_auth_parameters() {
    ...
    let (ctx, receiver) = list_server(vec![response, ok(json!([changed_file()]))]);
...
fn assert_stat_requests(requests: &[Request], numbers: &[&str]) {
    ...
    for request in requests {
        ...
        if request.url.path().ends_with("/files") {
...
async fn list_returns_statistics_for_every_row_without_reordering() {
    ...
    let (ctx, receiver) = list_server(responses);
    let page = GiteeProvider
...
async fn unavailable_list_statistics_preserve_the_original_row() {
    for known in [false, true] {
        for response in [
            ...
            let (ctx, receiver) = list_server(vec![counted_page(vec![value], 1), response]);
            let page = GiteeProvider
                .list_pull_requests(
                    &ctx,
                    PullRequestPagination {
    ...
    for confirmation in [
        ...
        let expected = serde_json::to_value(pull_request(&pr()).unwrap()).unwrap();
...
async fn list_does_not_certify_missing_or_invalid_file_line_counts() {
    for invalid in [
        ...
        let (ctx, receiver) = list_server(vec![counted_page(vec![pr()], 1), ok(json!([value]))]);
        let page = GiteeProvider
            .list_pull_requests(
                &ctx,
                PullRequestPagination {
...
async fn list_does_not_certify_overflowing_line_totals() {
    ...
    let (ctx, receiver) = list_server(vec![
        counted_page(vec![pr()], 1),
...
async fn draft_filter_scans_past_pages_without_matches_and_rejects_incomplete_empty_results() {
    ...
    assert_eq!(scanned_requests.len(), 4);
    assert_stat_requests(&scanned_requests[2..], &["101"]);

    let (ctx, receiver) = list_server(vec![counted_page(Vec::new(), 101)]);
    assert!(matches!(
        GiteeProvider
            .list_pull_requests_with_state(
                &ctx,
...
async fn public_gitee_initial_list_statistics() {
    let project = std::env::var("GITEE_TEST_PROJECT")
        .expect("set GITEE_TEST_PROJECT to a public owner/repository");
...
async fn public_gitee_readonly_smoke() {
    let project = std::env::var("GITEE_TEST_PROJECT")
        .expect("set GITEE_TEST_PROJECT to a public owner/repository");
    let number = std::env::var("GITEE_TEST_PULL_REQUEST")
        .expect("set GITEE_TEST_PULL_REQUEST to a PR with a text diff");
    let mut ctx = provider_context_for_identity(
        ReviewPlatformKind::Gitee,
        "gitee.com",
        &project,
    ...
    let target = GiteeProvider
        .pull_request_review_target(&ctx, number.as_str())
        .await
        .unwrap();
    assert_eq!(target.pull_request.id, number.as_str());
    ...
    let ci = GiteeProvider
        ...
        .await
        .unwrap();
    assert_eq!(ci.pull_request.id, number.as_str());
    let detail = GiteeProvider
        .pull_request_detail(&ctx, number.as_str())
        .await
        .unwrap();
    assert_eq!(detail.pull_request.id, number.as_str());
