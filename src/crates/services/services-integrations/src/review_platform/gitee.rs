    ...
    let futures = pull_requests
        ...
        .map(|mut pull_request| async move {
            if let Ok(files) = files(ctx, &pull_request.id).await {
                // The files endpoint follows the live PR. Bind its statistics to
                // the list revision before allowing the UI to cache them.
                if let Ok(value) = detail(ctx, &pull_request.id).await {
                    if let Ok(confirmed) = self::pull_request(&value) {
                        if require_revisions(&pull_request).is_ok()
                            && ensure_pull_request_revisions_stable(&pull_request, &confirmed)
                                .is_ok()
                        {
