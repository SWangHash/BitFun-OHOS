...
const BITFUN_SEARCH_TIMEOUT_SECS: u64 = 60;
const BITFUN_SEARCH_MEDIA_TYPE: &str = "application/vnd.bitfun.web-search.v1+json";

pub enum BitFunSearchHttpAuth {
    None,
    Bearer(String),
    Header { name: String, value: String },
}

pub struct BitFunSearchHttpProvider {
    endpoint: Url,
    auth: BitFunSearchHttpAuth,
}

impl BitFunSearchHttpProvider {
    pub fn new(
        endpoint: impl AsRef<str>,
        auth: BitFunSearchHttpAuth,
    ) -> Result<Self, WebSearchError> {
        let provider = WebSearchProviderId::new(WebSearchProviderId::BITFUN_SEARCH_HTTP);
        let endpoint = Url::parse(endpoint.as_ref()).map_err(|error| {
            WebSearchError::new(
                provider.clone(),
                WebSearchErrorKind::InvalidConfiguration,
                format!("BitFun Search endpoint is invalid: {error}"),
            )
        })?;
...
#[async_trait]
impl WebSearchProvider for BitFunSearchHttpProvider {
    fn id(&self) -> WebSearchProviderId {
        WebSearchProviderId::new(WebSearchProviderId::BITFUN_SEARCH_HTTP)
    }
    ...
    async fn search(&self, request: WebSearchRequest) -> Result<WebSearchResponse, WebSearchError> {
        ...
        let client = search_client(provider.clone(), BITFUN_SEARCH_TIMEOUT_SECS)?;
        let mut builder = client
            .post(self.endpoint.clone())
            .header(ACCEPT, BITFUN_SEARCH_MEDIA_TYPE)
            .json(&ProtocolRequest {
                query: &request.query,
                max_results: request.max_results,
            });
        builder = match &self.auth {
            BitFunSearchHttpAuth::None => builder,
            BitFunSearchHttpAuth::Bearer(secret) => builder.bearer_auth(secret),
            BitFunSearchHttpAuth::Header { name, value } => {
                let name = HeaderName::from_bytes(name.as_bytes()).map_err(|error| {
                    WebSearchError::new(
                        provider.clone(),
                        WebSearchErrorKind::InvalidConfiguration,
                        format!("BitFun Search auth header name is invalid: {error}"),
                    )
                })?;
                let value = HeaderValue::from_str(value).map_err(|error| {
                    WebSearchError::new(
                        provider.clone(),
                        WebSearchErrorKind::InvalidConfiguration,
                        format!("BitFun Search auth header value is invalid: {error}"),
                    )
                })?;
        ...
        let response = builder.send().await.map_err(|error| {
            transport_error(
                provider.clone(),
                "Failed to call BitFun Search HTTP endpoint",
                error,
            )
        })?;
        ...
        let response: ProtocolResponse = serde_json::from_slice(&body).map_err(|error| {
            WebSearchError::new(
                ...
                format!("Failed to parse BitFun Search HTTP response: {error}"),
            )
        })?;
...
fn validate_endpoint(endpoint: &Url, provider: WebSearchProviderId) -> Result<(), WebSearchError> {
        ...
        || endpoint.fragment().is_some()
    {
        return Err(WebSearchError::new(
            provider,
            WebSearchErrorKind::InvalidConfiguration,
            "BitFun Search endpoint cannot contain user info or a fragment",
        ));
    }
    ...
    if endpoint.scheme() != "http" || !is_loopback_host(endpoint.host_str()) {
        return Err(WebSearchError::new(
            provider,
            WebSearchErrorKind::InvalidConfiguration,
            "BitFun Search endpoint must use HTTPS; HTTP is allowed only for localhost or loopback IPs",
        ));
    }
    Ok(())
}
...
fn validate_auth(
    ...
    match auth {
        BitFunSearchHttpAuth::None => Ok(()),
        BitFunSearchHttpAuth::Bearer(value) if !value.trim().is_empty() => Ok(()),
        BitFunSearchHttpAuth::Header { name, value } if !value.trim().is_empty() => {
            let name = HeaderName::from_bytes(name.as_bytes()).map_err(|error| {
                WebSearchError::new(
                    provider.clone(),
                    WebSearchErrorKind::InvalidConfiguration,
            ...
            if forbidden.contains(&name.as_str()) {
                return Err(WebSearchError::new(
                    provider,
                    WebSearchErrorKind::InvalidConfiguration,
                    "BitFun Search auth header cannot override a protocol-managed header",
                ));
            }
        ...
        _ => Err(WebSearchError::new(
            provider,
            WebSearchErrorKind::InvalidConfiguration,
            "BitFun Search authentication secret is not configured on this device",
        )),
    }
}
...
fn validate_content_type(
    ...
    if content_type.starts_with("application/json")
        || content_type.starts_with(BITFUN_SEARCH_MEDIA_TYPE)
    {
        return Ok(());
    }
    Err(WebSearchError::new(
        provider,
        WebSearchErrorKind::InvalidResponse,
        "BitFun Search endpoint did not return a supported JSON content type",
    ))
}
