...
```json
{
  "--bitfun-color-surface-scene": "rgba(8, 17, 31, 0.08)",
  "--bitfun-color-surface-raised": "rgba(24, 43, 60, 0.72)",
  "--bitfun-color-surface-subtle": "rgba(11, 25, 40, 0.52)",
  "--bitfun-color-action-quiet-hover": "rgba(16, 34, 53, 0.60)",
  "--bitfun-color-action-neutral-surface": "rgba(22, 43, 61, 0.66)",
  "--bitfun-color-action-neutral-surface-pressed": "rgba(23, 54, 74, 0.74)",
  "--bitfun-color-border-subtle": "rgba(82, 229, 245, 0.08)",
  "--bitfun-color-border-default": "rgba(82, 229, 245, 0.12)",
  "--bitfun-color-border-strong": "rgba(82, 229, 245, 0.18)"
}
```

Tune the RGB values to the source palette while preserving the alpha hierarchy.

Insights report cards currently derive their local surfaces from `--bitfun-color-surface-raised` and canonical action/border tokens. They do not expose individual registered Parts. Keep `insights.content` transparent and use these tokens for glass-card behavior.
...
| collapsed navigation has an opaque 80x40 block | child `nav-bar.root` paints `--bitfun-color-surface-canvas` | force the base root transparent and use a low-alpha collapsed-state glass material |
