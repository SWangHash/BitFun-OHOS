...
class ManifestValidatorTests(unittest.TestCase):
    ...
    def test_theme_token_renderer_accepts_only_canonical_root_and_scope_tokens(self) -> None:
        ...
        self.assertEqual([], validator.validate_theme_tokens({
            "tokens": {"--bitfun-color-surface-canvas": "#101820"},
            "scopes": {"chrome": {"--bitfun-color-content-primary": "#f5f7fa"}},
        }))

        errors = validator.validate_theme_tokens({
            "tokens": {"--bitfun-appearance-token-color-bg-primary": "#101820"},
        })
        self.assertIn(
            "Unsupported root token name: --bitfun-appearance-token-color-bg-primary",
            errors,
        )
