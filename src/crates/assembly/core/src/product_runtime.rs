...
) -> BitFunResult<crate::agentic::WorkspaceBinding> {
    let mut config = session.config.clone();
    crate::agentic::workspace::normalize_session_workspace(&mut config).await?;
    let id = config
        .workspace_id
        .as_deref()
        .ok_or_else(|| BitFunError::Validation("Session workspace ID is required".into()))?;
...
impl CoreAgentRuntimeCompatibility {
    pub async fn ensure_control_conversation(
        &self,
    ) -> BitFunResult<crate::agentic::coordination::ControlConversation> {
    ...
    async fn resolve_fork_workspace_id(
        ...
        let service =
            crate::service::workspace::get_global_workspace_service().ok_or_else(|| {
                runtime_port_error(BitFunError::service("Workspace service is unavailable"))
            })?;
