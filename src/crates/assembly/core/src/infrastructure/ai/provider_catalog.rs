...
/// mixed even when the caller passed the BitFun-only source.
fn promoted_catalog_source(
    ...
    if has_bound_catalog_data && source == ProviderCatalogSource::BitFun {
...
mod tests {
    ...
    fn overlay_is_valid_and_keeps_product_endpoint_decisions() {
        ...
        let bitfun = overlay
            .providers
            .iter()
            .find(|provider| provider.id == "bitfun")
            .expect("bitfun");
        assert_eq!(
            bitfun.model_policy.curated_models,
        ...
        assert_eq!(
            bitfun
                ...
                .collect::<Vec<_>>(),
            [
                ("default", "https://api.bitfun.com", "anthropic", true,),
                ("openai", "https://api.bitfun.com/v1", "openai", false,),
            ]
        );
        let qwen = overlay
            .providers
    ...
    fn trusted_endpoint_resolves_catalog_identity_but_custom_gateway_does_not() {
        ...
        assert_eq!(
            trusted_models_dev_binding(
                "anthropic",
                "https://api.bitfun.com/v1/messages",
                "GLM-5.2",
                &relayed,
            ),
        ...
        assert_eq!(
            trusted_models_dev_binding(
                "openai",
                "https://api.bitfun.com/v1/chat/completions",
                "deepseek-v4-flash",
                &relayed,
            ),
            Some(("deepseek".to_string(), "deepseek-v4-flash".to_string()))
