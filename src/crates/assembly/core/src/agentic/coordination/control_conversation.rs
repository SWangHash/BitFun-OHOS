...
const CONTROL_SESSION_ID: &str = "bitfun-control";
...
impl ConversationCoordinator {
    pub async fn ensure_control_conversation(&self) -> BitFunResult<ControlConversation> {
    ...
    ) -> BitFunResult<ControlConversation> {
    ...
    pub(super) async fn select_control_conversation_in_workspace(
        ...
        let session_id = if create_new {
            format!("bitfun-control-{}", uuid::Uuid::new_v4())
        } else {
        ...
        if manager.get_session(&session_id).is_none() {
                ...
                .is_some()
            {
                ...
                return Err(BitFunError::session(
...
mod tests {
    ...
    fn legacy_selection_and_future_fields_round_trip() {
        ...
        let payload = serde_json::json!({"sessionId": "bitfun-control-new", "futureField": {"keep": true}});
