...
impl InsightsService {
    ...
    async fn generate_inner(
        ...
        let ai_factory = get_global_ai_client_factory().await.map_err(|e| {
            BitFunError::service(format!("Failed to get AI client factory: {}", e))
        })?;
    ...
    async fn analyze_interaction_style(
        ...
        let value: Value = serde_json::from_str(&json_str).map_err(|e| {
            BitFunError::Deserialization(format!(
                "Failed to parse interaction style JSON: {}",
                e
            ))
        })?;
    ...
    pub async fn load_report(path: &str) -> BitFunResult<InsightsReport> {
        ...
        let canonical_usage_dir = tokio::fs::canonicalize(&usage_dir).await.map_err(|e| {
            BitFunError::io(format!("Failed to resolve insights directory: {}", e))
        })?;
        let canonical_path = tokio::fs::canonicalize(&requested_path)
            .await
            .map_err(|e| {
                BitFunError::io(format!("Failed to resolve insights report: {}", e))
            })?;
        ...
        let report: InsightsReport = serde_json::from_str(&json_str).map_err(|e| {
            BitFunError::Deserialization(format!("Failed to parse report: {}", e))
        })?;
        Ok(report)
    }
