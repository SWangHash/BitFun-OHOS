    ...
    async fn call_impl(
        ...
        if context.is_remote() {
            return Err(BitFunError::tool(
                "FinalizeMiniApp reads product-host-local files and cannot be used in a remote workspace. Use BitFunControl get feature.miniapps and its structured update-app operation instead".to_string(),
            ));
        }
        ...
        let app = manager
            ...
            .await
            .map_err(|error| {
                BitFunError::tool(format!("Failed to finalize MiniApp: {error}"))
            })?;
