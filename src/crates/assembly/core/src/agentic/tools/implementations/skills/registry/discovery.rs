                        ...
                        if let Err(error) = read {
                            ...
                            match Self::parse_skill_markdown(
                                ...
                                Ok(mut data) => {
                                    ...
                                    // wc matrix market provenance: a `.market-source`
                                    // marker recorded at install time identifies the
                                    // market package this skill came from.
                                    if let Ok(market_source) =
                                        fs::read_to_string(path.join(".market-source")).await
                                    {
                                        let trimmed = market_source.trim();
                                        if !trimmed.is_empty() {
                                            data.market_install_id =
