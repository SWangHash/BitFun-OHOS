        ...
        match params.action {
            ...
            CronAction::List => {
                let cron_service = get_global_cron_service().ok_or_else(|| {
                    BitFunError::tool("cron service not initialized".to_string())
                })?;
                let session_id =
                ...
                if !cron_service.is_scheduling_owner() {
                    ...
                    result_for_assistant.push_str(
                        "\n\nScheduling note: another running BitFun instance owns scheduled job execution right now, so these jobs run there and edits or manual runs are refused in this instance.",
                    );
                }
            ...
            CronAction::Update => {
                ...
                let patch = params.patch.ok_or_else(|| {
                    BitFunError::tool("patch is required for update".to_string())
                })?;
                if patch.is_empty() {
            ...
            CronAction::Run => {
                ...
                let job_id = params.job_id.ok_or_else(|| {
                    BitFunError::tool("job_id is required for run".to_string())
                })?;
                Self::validate_job_id(&job_id).map_err(BitFunError::tool)?;
        ...
        let validation = tool
            .validate_input(
                &json!({
                    "action": "list",
                    "session_id": "worker_1",
                    "workspace": "E:/Projects/BitFun/BitFun",
                }),
                Some(&empty_context()),
            )
            .await;
