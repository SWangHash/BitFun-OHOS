...
<html data-bitfun-appearance-mode="dark">
    ...
    async fn call_impl(
        ...
        if context.is_remote() {
            return Err(BitFunError::tool(
                "InitMiniApp returns product-host-local files and cannot be used in a remote workspace. Use BitFunControl get feature.miniapps and its structured create-app operation instead".to_string(),
            ));
        }
