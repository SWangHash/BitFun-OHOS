...
impl Tool for AnalyzeImageTool {
    ...
    async fn call_impl(
        ...
        let client = get_global_ai_client_factory()
            .await
            .map_err(|err| {
                BitFunError::service(format!("AI client factory unavailable: {err}"))
            })?
...
mod image_path_tests {
    ...
    async fn analyze_image_reads_uploaded_runtime_pixels_in_an_ssh_session() {
        ...
        context
            .custom_data
            .insert("__bitfun_test_runtime_root".into(), json!(root.path()));
        ...
        assert!(reference.starts_with("bitfun://runtime/remote-workspace/"));
