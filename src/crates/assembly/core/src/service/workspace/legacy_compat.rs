...
pub(crate) fn resolve_legacy_workspace_reference(
    ...
    ssh_host: Option<&str>,
) -> BitFunResult<Option<WorkspaceInfo>> {
    ...
    let path_matches = |workspace: &&WorkspaceInfo| {
        if workspace.workspace_kind == WorkspaceKind::Remote {
            bitfun_services_core::workspace_identity::normalize_remote_workspace_path(
                &workspace.root_path.to_string_lossy(),
            ) == bitfun_services_core::workspace_identity::normalize_remote_workspace_path(path)
        } else {
    ...
    if let Some(id) = workspace_id {
        ...
        return records
            ...
            .map(Some)
            .ok_or_else(|| {
                BitFunError::service(format!("Workspace ID is unavailable on this host: {id}"))
            });
    }
...
mod tests {
    ...
    async fn id_only_restore_uses_record_type_and_separates_execution_from_storage() {
        ...
        use bitfun_runtime_ports::{SessionStorageKind, SessionStorePort};
...
pub(crate) async fn upgrade_session_workspace_reference(
    ...
    if config.workspace_id.is_none() {
        let path = config
            ...
            .ok_or_else(|| BitFunError::service("Session has no workspace reference"))?;
        let record = service
            .resolve_legacy_workspace_reference(
                None,
                path,
...
impl WorkspaceService {
    ...
    pub async fn upgrade_legacy_session_binding(
    ...
    ) -> BitFunResult<bitfun_runtime_ports::AgentSessionWorkspaceBinding> {
        ...
        Ok(bitfun_runtime_ports::AgentSessionWorkspaceBinding {
...
impl WorkspaceService {
    ...
    pub async fn upgrade_legacy_file_connection(
        ...
        if providers.len() > 1 {
            return Err(BitFunError::service("Legacy filesystem path matches multiple workspace providers; select the workspace ID"));
        }
...
) -> BitFunResult<crate::service::cron::CronJobTarget> {
    use crate::service::cron::CronJobTarget;
        ...
        if connection.is_none() && host.is_none() {
                ...
                .await
            {
                if let Some(id) = binding.workspace_id.as_deref() {
                    use bitfun_runtime_ports::SessionStorePort;
