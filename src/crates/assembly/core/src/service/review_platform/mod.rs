    ...
    async fn execute_remote_git_command(
        ...
        #[cfg(not(feature = "remote-workspace"))]
        {
            ...
            Err(ReviewPlatformError::InvalidRepository(format!(
                "Remote workspaces are not compiled into this BitFun host (feature `remote-workspace`); refusing to run Git against the local filesystem for remote workspace {}",
                target.workspace_id
            )))
        }
    }
...
mod tests {
    ...
    async fn remote_git_execution_fails_loudly_without_remote_workspace_capability() {
        ...
        assert!(error
            .to_string()
            .contains("Remote workspaces are not compiled into this BitFun host"));
    }
