...
async fn load_remote_session_metadata_for_workspace(
    ...
    use bitfun_services_core::session::SessionRelationshipKind;
    ...
    Ok(metadata
        ...
        .map(|session| {
            ...
            let relationship =
                bitfun_services_core::session::normalized_session_relationship(&session);
...
impl CoreServiceAgentRuntime {
    ...
    pub(crate) async fn synchronize_relay_session(
        hub: &bitfun_services_integrations::remote_connect::host_stream::HostStreamHub,
        session_id: &str,
        turn_id: Option<&str>,
    ) -> Result<(), String> {
        hub.synchronize_records(session_id.to_owned(),turn_id.is_none(),||async {
            ...
            tokio::task::spawn_blocking(move||{
                bitfun_services_integrations::remote_connect::session_records::records_from_turns(&turns,&read_remote_chat_image_pixels)
            }).await?
...
mod tests {
    ...
    use bitfun_services_integrations::remote_connect::no_host_image_pixels;
