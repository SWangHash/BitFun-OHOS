...
export const SnapshotCard: React.FC<SnapshotCardProps> = ({
  ...
  const getOperationInfo = () => {
    const operationMap = {
      'write': { name: t('flowChatCards.snapshotCard.writeFile'), color: 'var(--bf-appearance-token-color-success)' },
      'edit': { name: t('flowChatCards.snapshotCard.editFile'), color: 'var(--bf-appearance-token-color-warning)' },
      'delete': { name: t('flowChatCards.snapshotCard.deleteFile'), color: 'var(--bf-appearance-token-color-error)' },
      'multi-edit': { name: t('flowChatCards.snapshotCard.multiEdit'), color: 'var(--bf-appearance-token-color-warning)' }
    };
  ...
  if (displayMode === 'compact') {
    return (
      ...
      >
        ...
        <div className="snapshot-card__actions" onClick={(e) => e.stopPropagation()} data-bitfun-component="flow-chat-card" data-bitfun-part="actions">
          <button
  ...
  return (
    ...
    >
      ...
      {(status === 'running' || status === 'streaming') && (
        <div className="snapshot-card__processing" data-bitfun-component="flow-chat-card" data-bitfun-part="processing">
      ...
      {status === 'error' && result?.error && (
        <div className="snapshot-card__error" data-bitfun-component="flow-chat-card" data-bitfun-part="error">
          <XCircle size={14} />
