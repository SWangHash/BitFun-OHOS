...
function pkg(id: string, background: string): AppearancePackage {
  return {
    ...
    renderers: {
      'theme-tokens': {
        version: 1,
        settings: { tokens: { '--bitfun-color-surface-canvas': background } },
      },
    },
  };
}
...
describe('AppearanceRuntime', () => {
  ...
  it('commits revisions atomically and rolls renderer adapters back on failure', async () => {
    ...
    const adapter: AppearanceRendererAdapter<'theme-tokens'> = {
      ...
      apply: async next => {
        ...
        if (next?.tokens['--bitfun-color-surface-canvas'] === 'fail') throw new Error('renderer failed');
      },
    };
    ...
    expect(document.documentElement.getAttribute('data-bitfun-appearance')).toBe('test.first');
    expect(document.querySelectorAll('style[data-bitfun-appearance-runtime]')).toHaveLength(1);
    ...
    expect(document.documentElement.getAttribute('data-bitfun-appearance')).toBe('test.first');
    expect(document.querySelectorAll('style[data-bitfun-appearance-runtime]')).toHaveLength(1);
    expect(calls.at(-1)).toEqual({ tokens: { '--bitfun-color-surface-canvas': 'first' } });
  ...
  it('creates host-owned blob URLs for package assets and revokes replaced URLs', async () => {
    ...
    expect(document.head.textContent).toContain('--bitfun-appearance-asset-background: url("blob:test-background")');
  ...
  it('resolves video background URLs without exposing the video as a CSS image variable', async () => {
    ...
    expect(snapshot.cssText).not.toContain('--bitfun-appearance-asset-motion');
    expect(snapshot.cssText).toContain('--bitfun-appearance-asset-poster');
  });
  ...
  it('keeps a staged revision inactive until renderer transactions commit', async () => {
    ...
    const adapter: AppearanceRendererAdapter<'theme-tokens'> = {
      ...
      validate: () => [],
      apply: next => {
        if (next?.tokens['--bitfun-color-surface-canvas'] !== 'second') return;
        const stagedStyle = document.querySelector<HTMLStyleElement>(
          'style[data-bitfun-appearance-runtime="2"]',
        );
        observations.push({
          rootRevision: document.documentElement.getAttribute('data-bitfun-appearance-revision'),
    ...
    expect(observations[0].stagedCss).toContain(
      '[data-bitfun-appearance="test.same-id"][data-bitfun-appearance-revision="2"]',
    );
    expect(document.documentElement.getAttribute('data-bitfun-appearance-revision')).toBe('2');
    expect(document.querySelectorAll('style[data-bitfun-appearance-runtime]')).toHaveLength(1);
  });
  ...
  it('applies normal cascade after legacy styles and reserves important for explicit overrides', async () => {
    ...
    document.body.innerHTML = '<div class="legacy-gallery" data-bitfun-component="gallery-layout" data-bitfun-part="root">Run</div>';
    const registry = new AppearanceRegistry()
    ...
    const gallery = document.querySelector('[data-bitfun-component="gallery-layout"]') as HTMLDivElement;
