...
describe('annotation highlight paint colors', () => {
  const accent = '--bitfun-component-conversation-excerpt-accent';
  const paintStyle = () => document.querySelector<HTMLStyleElement>('style[data-bitfun-appearance-highlight-paint]');
  ...
  it('materializes indirect search/selection mixes in both root and chrome without mutating packages', async () => {
    ...
    const tokens = {
      '--bitfun-color-accent-border-subtle': value,
      '--bitfun-color-selection-surface': value,
      '--bitfun-color-action-secondary-pressed': value,
    };
    ...
    const scope = document.querySelector('style[data-bitfun-appearance-theme-scopes]')!;
    expect(scope.textContent).not.toContain('color-mix(');
    expect(scope.textContent).toContain('[data-bitfun-theme-scope="chrome"]');
