...
export const SPEECH_TRANSCRIPTION_RESULT_EVENT = 'speech://transcription-result';
...
export interface SpeechTranscriptionResultEvent {
  sessionId: string;
  text: string;
  isFinal: boolean;
}
...
export class SpeechAPI {
  ...
  onTranscriptionResult(callback: (result: SpeechTranscriptionResultEvent) => void): () => void {
    return api.listen(SPEECH_TRANSCRIPTION_RESULT_EVENT, callback);
  }
}
