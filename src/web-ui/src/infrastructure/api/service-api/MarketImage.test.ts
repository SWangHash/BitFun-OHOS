...
import { marketImageSrcSet, marketImageUrl, retryOriginalMarketImage } from './MarketImage';

describe('market image URLs', () => {
  it('adds a versioned image variant without losing existing URL state', () => {
    expect(marketImageUrl('/market/image?token=one#focus', 'compact-v1'))
      .toBe('/market/image?token=one&variant=compact-v1#focus');
  });
