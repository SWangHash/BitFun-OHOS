...
describe('temporary Skill receipt upgrade', () => {
  ...
  const oldKey = 'bitfun:external-skill-import:' + JSON.stringify(['/repo', '/source']);
  const newKey = 'bitfun:external-skill-import:v2:' + JSON.stringify(['local', 'parent-id', '/source']);
  ...
  it('does not turn an old project receipt with a missing root into a global receipt', () => {
    const data = storage();
    const source = '/source';
    const oldGlobal = 'bitfun:external-skill-import:' + JSON.stringify(['', source]);
    const target = 'bitfun:external-skill-import:v2:' + JSON.stringify(['local', '', source]);
