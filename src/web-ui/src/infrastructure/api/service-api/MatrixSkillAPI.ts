...
class MatrixSkillAPI {
  ...
  async installSkill(enName: string): Promise<MatrixSkillInstallResult> {
    try {
      return await api.invoke<MatrixSkillInstallResult>('install_matrix_skill', { enName });
    } catch (error) {
      throw createTauriCommandError('install_matrix_skill', error, { enName });
    }
  }
