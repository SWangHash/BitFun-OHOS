...
describe('FontPreferenceService', () => {
  ...
  it('applies only the canonical design-system font-size foundation', () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--bitfun-font-size-base')).toBe('14px');
    expect(document.documentElement.style.getPropertyValue('--bitfun-font-size-meta')).toBe('11px');
    // typography-audit: negative-test-start -- verifies retired Appearance and FlowChat variables stay unwritten
    expect(document.documentElement.style.getPropertyValue('--bitfun-appearance-token-font-size-base')).toBe('');
    expect(document.documentElement.style.getPropertyValue('--bitfun-appearance-token-flowchat-font-size-base')).toBe('');
  ...
  it('drops retired FlowChat font settings while loading persisted preferences', async () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--bitfun-font-size-base')).toBe('16px');
  ...
  it('adjusts a saved preset, applies typography, and persists the existing config shape', async () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--bitfun-font-size-base')).toBe('17px');
