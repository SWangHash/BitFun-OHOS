...
it('synchronizes skips in development without starting discovery or dismissing another version', async () => {
  ...
  try {
    ...
    act(() => {
      localStorage.setItem('bitfun:update:skippedVersion', '1.5.0');
      window.dispatchEvent(new StorageEvent('storage', { key: 'bitfun:update:skippedVersion' }));
    });
    expect(useUpdateInstallStore.getState()).toMatchObject({ skippedVersion: '1.5.0', notice: 'available' });
    act(() => {
      localStorage.setItem('bitfun:update:skippedVersion', '2.0.0');
      window.dispatchEvent(new StorageEvent('storage', { key: 'bitfun:update:skippedVersion' }));
    });
