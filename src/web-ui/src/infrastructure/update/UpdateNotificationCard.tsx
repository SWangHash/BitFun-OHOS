  ...
  return (
    ...
    >
      <Card appearance="raised" padding="md" gap="md" radius="md"
        className="bitfun-update-notice__surface" data-bitfun-native-webview-occlusion>
        <div data-bitfun-component="update" data-bitfun-part="availableRoot" data-bitfun-variant="daily">
          <div data-bitfun-component="update" data-bitfun-part="lead">
            <CardHeader
              align="center"
              actions={(
            ...
            >
              <span className="bitfun-update-notice__heading"
                data-bitfun-component="update" data-bitfun-part="noticeTitle">
                <ActionItem className="bitfun-update-notice__title"
                  ...
                  <OverflowText className="bitfun-update-notice__title-text">{title}</OverflowText>
                </ActionItem>
              </span>
            </CardHeader>
          </div>
        </div>
        <CardBody className="bitfun-update-notice__body">
          {introduction ? (
            <OverflowText as="p" lines={notice === 'error' ? 1 : 3} className="bitfun-update-notice__introduction"
              data-testid="app-update-introduction"
              data-bitfun-component="update" data-bitfun-part="subtitle">
              {introduction}
            </OverflowText>
          ) : (
          ...
          {notice === 'error' && <div className="bitfun-update-notice__feedback">
            <p className="bitfun-update-notice__message" role="status" aria-live="polite"
              data-bitfun-component="update" data-bitfun-part="subtitle">
              <span className="bitfun-update-notice__status-icon" aria-hidden="true"
                data-bitfun-component="update" data-bitfun-part="leadIcon">
          ...
          {notice === 'downloading' && (
            <div className="bitfun-update-progress__bar bitfun-update-notice__progress"
              role="progressbar" aria-label={t('update.downloadingTitle')}
              aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent ?? undefined}
              data-bitfun-component="update" data-bitfun-part="progressBar">
              <div
                className={'bitfun-update-progress__fill' + (percent == null ? ' bitfun-update-progress__fill--indeterminate' : '')}
                style={percent == null ? undefined : { transform: `scaleX(${percent / 100})` }}
                data-bitfun-component="update" data-bitfun-part="progressFill"
                data-bitfun-state={percent == null ? 'indeterminate' : undefined} />
            </div>
          )}
        </CardBody>
        {notice !== 'downloading' && (
          <div className="bitfun-update-notice__footer" data-bitfun-component="update" data-bitfun-part="actions">
            <CardFooter align={notice === 'available' ? 'between' : 'end'} className="bitfun-update-notice__actions">
              {notice === 'available' && version && (
                <Button className="bitfun-update-notice__action" size="sm" variant="outline" onClick={() => state.skipVersion(version)}>
                  {t('update.skipVersion')}
                </Button>
              )}
              {notice === 'available' && <Button className="bitfun-update-notice__action" size="sm" variant="primary" onClick={event => startDownload(event, Boolean(state.version), version ?? undefined)}>
                {t('update.downloadUpdate')}
              </Button>}
              {notice === 'error' && <Button className="bitfun-update-notice__action" size="sm" variant="primary" onClick={event => {
