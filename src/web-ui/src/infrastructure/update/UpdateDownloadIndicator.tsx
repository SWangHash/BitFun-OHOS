import { createOverlayPortal, Icon, IconButton, Tooltip } from '@bitfun/ui';
...
export function UpdateDownloadIndicator() {
  ...
  return (
    <div className="bitfun-update-download" data-testid="nav-update-download"
      data-bitfun-component="update" data-bitfun-part="downloadControl" data-download-status={status}>
      <Tooltip content={status === 'ready' ? installLabel : label} placement="top">
        <IconButton ref={buttonRef} size="sm" shape="circle" variant="quiet" className="bitfun-update-download__button"
          ...
          icon={(
            <span ref={vesselRef} className="bitfun-update-download__vessel" data-download-status={status}
              data-bitfun-component="update" data-bitfun-part="downloadVessel">
              {status === 'downloading' && <span className="bitfun-update-download__fill"
                style={fraction == null ? undefined : { transform: `translateY(${(1 - fraction) * 100}%)` }}
                data-bitfun-component="update" data-bitfun-part="downloadFill"
                data-bitfun-state={fraction == null ? 'indeterminate' : undefined} />}
              {(status === 'ready' || status === 'installing') && <Icon name="check-line" size="xs" className="bitfun-update-download__complete" />}
            </span>
          )} />
      </Tooltip>
      {status === 'downloading' ? (
      ...
      {status === 'downloading' && createOverlayPortal(
        <span ref={setTransferElement} className="bitfun-update-download__transfer" aria-hidden="true"
          data-bitfun-component="update" data-bitfun-part="downloadTransfer" />,
