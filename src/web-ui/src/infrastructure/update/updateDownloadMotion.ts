...
export function runUpdateDownloadHandoff(
  ...
  const duration = milliseconds(style.getPropertyValue('--bitfun-motion-duration-slow'));
  const easing = style.getPropertyValue('--bitfun-motion-easing-smooth').trim() || 'ease-in-out';
  ...
  void Promise.all(animations.map(animation => animation.finished)).then(() => {
    ...
    const landing = vessel.animate([
      ...
      { transform: 'scale(1)' },
    ], { duration: milliseconds(style.getPropertyValue('--bitfun-motion-duration-base')), easing });
