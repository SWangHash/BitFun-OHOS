...
it('shows explicit release details without the general manual-check action', async () => {
  ...
  const actions = container.querySelector('[data-bitfun-part="updateActions"]');
  expect(actions?.closest('[data-bitfun-part="footer"]')).not.toBeNull();
  expect(actions?.closest('[data-bitfun-part="body"]')).toBeNull();
...
it('keeps release notes directly readable from both details entry points', async () => {
  await act(async () => state().checkForUpdates());
  const notes = container.querySelector('[data-bitfun-part="notes"]');
  expect(notes?.textContent).toContain('Release changes');
  expect(container.querySelector('details')).toBeNull();
  act(() => state().openReleaseNotes());
  expect(container.querySelector('[data-bitfun-part="notes"]')).toBe(notes);
