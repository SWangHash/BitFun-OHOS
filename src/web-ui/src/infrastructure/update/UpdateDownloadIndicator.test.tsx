...
it('hands reminder attention to bottom-up progress and reopens the progress card on activation', () => {
  ...
  expect(control()?.querySelector<HTMLElement>('[data-bitfun-part="downloadFill"]')?.style.transform).toBe('translateY(75%)');
...
it('hands the shallow wave to real progress without a spinner or fabricated percentage', () => {
  ...
  const fill = control()?.querySelector<HTMLElement>('[data-bitfun-part="downloadFill"]');
  expect(progress()?.hasAttribute('aria-valuenow')).toBe(false);
  expect(control()?.querySelector('[data-bitfun-component="spinner"]')).toBeNull();
  expect(fill?.getAttribute('data-bitfun-state')).toBe('indeterminate');
  ...
  expect(progress()?.getAttribute('aria-valuenow')).toBe('25');
  expect(control()?.querySelector('[data-bitfun-part="downloadFill"]')).toBe(fill);
  expect(fill?.hasAttribute('data-bitfun-state')).toBe(false);
...
it('hands a download to navigation and opens installation confirmation from the completed checkmark', async () => {
  ...
  expect(container.querySelector('[data-testid="app-update-notice"]')).toBeNull();
  expect(control()?.querySelector('[data-bitfun-part="downloadFill"]')).toBeNull();
  expect(control()?.querySelector('.bitfun-update-download__complete')).not.toBeNull();
