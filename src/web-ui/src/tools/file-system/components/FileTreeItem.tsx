...
import { OverflowText, Icon, Input } from '@bitfun/ui';
...
export const FileTreeItem: React.FC<FileTreeItemProps> = ({
  ...
  const handleDragStart = (event: React.DragEvent) => {
    ...
    dragImage.style.background = 'color-mix(in srgb, var(--bitfun-color-content-on-light) 80%, transparent)';
    dragImage.style.color = 'var(--bitfun-color-content-on-dark)';
  ...
  return (
    ...
    >
      ...
      {isRenaming ? (
        ...
        <OverflowText className={`bitfun-file-explorer__node-name ${isCompressed ? 'bitfun-file-explorer__compressed-path' : ''}`}>
          {node.name}
        </OverflowText>
      )}
