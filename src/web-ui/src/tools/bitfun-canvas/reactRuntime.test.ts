...
describe('React Canvas runtime bridge', () => {
  ...
  it('uses a light centered standalone shell by default', () => {
    ...
    expect(html).toContain('.bitfun-canvas-stack{max-width:min(100%,980px);margin-inline:auto}');
  });
  ...
  it('bridges host appearance variables into the iframe runtime', () => {
    ...
    expect(html).toContain('--bitfun-color-surface-canvas');
  });
...
window.BitFunCanvasRuntime.mount(Canvas);
    ...
    try {
      ...
      expect(dom.window.document.querySelector('.bitfun-chart')).toBeTruthy();
      expect(dom.window.document.querySelector('.bitfun-collapsible-section')).toBeTruthy();
      expect(dom.window.document.querySelector('.bitfun-diagram')).toBeTruthy();
      expect(dom.window.document.querySelector('.bitfun-canvas-adapter-tabs')).toBeTruthy();
      expect(dom.window.document.querySelector('[data-bitfun-component="tab-group"]')).toBeTruthy();
      expect(dom.window.document.querySelector('[data-bitfun-component="input"]')).toBeTruthy();
      expect(dom.window.document.querySelector('[data-bitfun-component="empty"]')).toBeTruthy();
    } finally {
      dom.window.close();
    }
  });
