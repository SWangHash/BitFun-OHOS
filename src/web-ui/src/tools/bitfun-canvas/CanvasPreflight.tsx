...
export function CanvasPreflight({
  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (!type.startsWith('bitfun-canvas-')) return;
      try {
        if (type === 'bitfun-canvas-load-state') {
        ...
        if (type === 'bitfun-canvas-module-started') {
          iframe.contentWindow.postMessage({
            type: 'bitfun-canvas-appearance',
            appearance: readHostAppearancePayload(),
          }, '*');
          return;
        }
        if (type === 'bitfun-canvas-save-state') {
          iframe.contentWindow.postMessage({
        ...
        if (type === 'bitfun-canvas-action') {
          iframe.contentWindow.postMessage({
            type: 'bitfun-canvas-error',
            requestId: message?.requestId,
            error: 'Canvas host actions are disabled during preflight validation',
          }, '*');
          return;
        }
        if (type === 'bitfun-canvas-ready') {
        ...
        if (type === 'bitfun-canvas-runtime-error' || type === 'bitfun-canvas-early-error') {
