...
function DiagramShell({
  ...
  return (
    <div
      {...props}
      className={['bitfun-diagram', className].filter(Boolean).join(' ')}
      style={{
        minWidth: 0,
        overflow: 'auto',
        border: '1px solid color-mix(in srgb, var(--bitfun-color-border-subtle) 78%, transparent)',
        borderRadius: 8,
        background: 'color-mix(in srgb, var(--bitfun-color-surface-raised) 56%, transparent)',
        padding: 14,
        ...style,
      }}
    >
      {title ? (
        <div style={{ marginBottom: 10, color: 'var(--bitfun-color-content-primary)', fontSize: 'var(--bitfun-type-label-sm-font-size)', fontWeight: 'var(--bitfun-type-label-selected-font-weight)', lineHeight: 'var(--bitfun-type-label-sm-line-height)' }}>
          {title}
        </div>
      ) : null}
      ...
      <g aria-hidden="true">
        {layout.ranks.map((rank, index) => (
          <rect
            ...
            rx={8}
            fill={index % 2 === 0 ? 'var(--bitfun-color-surface-subtle)' : 'var(--bitfun-color-surface-chrome)'}
            ...
            <g key={`${edge.from}-${edge.to}-${index}`}>
              ...
              {meta?.label ? (
                <text
                  ...
                  textAnchor="middle"
                  fill="var(--bitfun-color-content-muted)"
        ...
        return (
          <g key={layoutNode.id} transform={`translate(${layoutNode.x} ${layoutNode.y})`}>
            ...
            <rect
              ...
              rx={6}
              fill="var(--bitfun-color-surface-raised)"
              stroke="var(--bitfun-color-border-subtle)"
              strokeWidth={1}
            />
            <rect
              width={4}
              ...
              opacity={0.78}
            />
            <text x={14} y={description ? 18 : layoutNode.height / 2 + 4} fill="var(--bitfun-color-content-primary)" fontSize="var(--bitfun-type-support-font-size)" fontWeight="var(--bitfun-type-label-selected-font-weight)">
              {String(label).slice(0, 22)}
            </text>
            {description ? (
              <text x={14} y={34} fill="var(--bitfun-color-content-muted)" fontSize="var(--bitfun-type-micro-font-size)">
  ...
  return (
    <DiagramShell {...props} title={title} height={resolvedHeight} style={style} className={className}>
      {safeNodes.length ? (
        ...
        <div style={{ color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-support-font-size)' }}>No graph nodes</div>
      )}
    </DiagramShell>
  );
}
  ...
  return (
    <DiagramShell {...props} title={title} height={height ?? layout.height} style={style} className={className}>
      {resolvedNodes.length ? (
        ...
        <div style={{ color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-support-font-size)' }}>No flow steps</div>
      )}
    </DiagramShell>
  );
}
