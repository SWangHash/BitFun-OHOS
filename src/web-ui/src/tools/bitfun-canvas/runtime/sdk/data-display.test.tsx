...
describe('BitFun Canvas data display components', () => {
  it('renders callouts as lightweight themed notes', () => {
    ...
    expect(markup).toContain('bitfun-callout');
    expect(markup).toContain('bitfun-callout-title');
    expect(markup).toContain('bitfun-callout-body');
    expect(markup).toContain('--bitfun-callout-accent:var(--bitfun-canvas-info)');
  ...
  it('renders sandbox-native alerts without host i18n requirements', () => {
    ...
    expect(markup).toContain('bitfun-alert');
  ...
  it('renders collapsible sections from the bundled SDK', () => {
    ...
    expect(markup).toContain('bitfun-collapsible-section');
  ...
  it('renders key value lists from records', () => {
    ...
    expect(markup).toContain('bitfun-key-value-list');
  ...
  it('renders timeline events with time and description', () => {
    ...
    expect(markup).toContain('bitfun-timeline');
  ...
  it('renders nested file trees', () => {
    ...
    expect(markup).toContain('bitfun-file-tree');
  ...
  it('uses dedicated code-change colors for diff counts and lines', () => {
    ...
    expect(markup).toContain('--bitfun-color-code-change-added');
    expect(markup).toContain('--bitfun-color-code-change-removed');
    expect(markup).not.toContain('--bitfun-color-status-success-content');
    expect(markup).not.toContain('--bitfun-color-status-danger-content');
  });
  ...
  it('renders progress bars with bounded values', () => {
    ...
    expect(markup).toContain('bitfun-progress');
  ...
  it('renders swatches and usage bars from palette tokens', () => {
    ...
    expect(markup).toContain('bitfun-swatch');
    expect(markup).toContain('bitfun-usage-bar');
    expect(markup).toContain('role="progressbar"');
  ...
  it('renders todo lists and cards without host services', () => {
    ...
    expect(markup).toContain('bitfun-todo-list');
    expect(markup).toContain('bitfun-todo-list-card');
