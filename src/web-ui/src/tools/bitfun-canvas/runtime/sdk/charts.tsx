...
const chartColors = [
  'var(--bitfun-color-accent-default)',
  'var(--bitfun-color-status-success-content)',
  'var(--bitfun-color-status-warning-content)',
  'var(--bitfun-color-status-info-content)',
  'var(--bitfun-color-accent-hover)',
  'var(--bitfun-color-status-danger-content)',
  'var(--bitfun-color-content-muted)',
  'color-mix(in srgb, var(--bitfun-color-accent-default) 40%, transparent)',
];
...
function ChartShell({
  ...
  return (
    <div {...props} className={['bitfun-chart', props.className].filter(Boolean).join(' ')} style={style}>
      {title || legend.length > 1 ? (
        <div className="bitfun-chart__header">
          {title ? <div className="bitfun-chart__title"><OverflowText>{title}</OverflowText></div> : <span />}
          {legend.length > 1 ? (
            <div className="bitfun-chart__legend">
              {legend.map((item, index) => (
                <span key={item.name + index} className="bitfun-chart__legend-item">
                  <span className="bitfun-chart__swatch" style={{ background: item.color }} />
                  {item.name}
                </span>
              ))}
            </div>
...
function chartGrid(left: number, top: number, plotWidth: number, plotHeight: number, max: number) {
  return Array.from({ length: 5 }, (_, index) => {
    ...
    return (
      <g key={`grid-${index}`}>
        <line x1={left} y1={y} x2={left + plotWidth} y2={y} stroke="var(--bitfun-color-border-subtle)" strokeWidth={1} />
        <text x={left - 8} y={y + 4} textAnchor="end" fill="var(--bitfun-color-content-muted)" fontSize="var(--bitfun-type-micro-font-size)">
          {label}
        </text>
      </g>
    );
    ...
    <ChartShell title={props.title} legend={series} height={height} style={props.style}>
      <svg viewBox={`0 0 ${width} ${height}`} height={height} role="img" aria-label={String(props.title || 'Bar chart')}>
        ...
        {labels.map((label, index) => (
          <text key={`label-${index}`} x={left + index * groupWidth + groupWidth / 2} y={height - 10} textAnchor="middle" fill="var(--bitfun-color-content-muted)" fontSize="var(--bitfun-type-micro-font-size)">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} height={height} role="img" aria-label={String(props.title || 'Line chart')}>
        ...
        {series.flatMap(item =>
          item.values.map((value, index) => (
            <circle key={`point-${item.name}-${index}`} cx={xFor(index)} cy={yFor(value)} r={3} fill={item.color} stroke="var(--bitfun-color-surface-panel)" strokeWidth={1.5} />
          )),
        )}
        {labels.map((label, index) => (
          <text key={`label-${index}`} x={xFor(index)} y={height - 10} textAnchor="middle" fill="var(--bitfun-color-content-muted)" fontSize="var(--bitfun-type-micro-font-size)">
            <title>{String(label)}</title>
            {String(label).slice(0, 14)}
          </text>
        ))}
...
export function PieChart(props: CanvasChartProps = {}) {
  ...
  const paths = slices.map((item, index) => {
    ...
    angle = nextAngle;
    return <path key={item.label + index} d={path} fill={item.color} stroke="var(--bitfun-color-surface-panel)" strokeWidth={1.5} />;
  });
  return (
      ...
      <svg viewBox={`0 0 ${width} ${height}`} height={height} role="img" aria-label={String(props.title || 'Pie chart')}>
        ...
        {slices.slice(0, 6).map((item, index) => (
          <g key={`label-${index}`} transform={`translate(225 ${52 + index * 23})`}>
            ...
            <text x={16} y={0} fill="var(--bitfun-color-content-secondary)" fontSize="var(--bitfun-type-meta-font-size)">
              {String(item.label).slice(0, 20)}
            </text>
            <text x={118} y={0} fill="var(--bitfun-color-content-muted)" fontSize="var(--bitfun-type-meta-font-size)" textAnchor="end">
