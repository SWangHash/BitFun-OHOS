...
import { Alert as DesignAlert, Disclosure as DesignDisclosure, OverflowText } from '@bitfun/ui';
...
export function Callout({ children, tone = 'info', title, style, ...props }: CanvasCalloutProps) {
  ...
  return (
    <section
      {...props}
      className={['bitfun-callout', props.className].filter(Boolean).join(' ')}
      style={{ '--bitfun-callout-accent': accent, ...style } as React.CSSProperties}
    >
      {title ? (
        <div className="bitfun-callout-title">
          {title}
        </div>
      ) : null}
      <div className="bitfun-callout-body">{children}</div>
    </section>
  );
}
...
export function Alert({
  ...
  return (
    <DesignAlert
      {...props}
      className={['bitfun-alert', props.className].filter(Boolean).join(' ')}
    ...
    <div {...props} style={{ display: 'grid', gap: 3, ...style }}>
      <strong
        style={{
          ...
          fontSize: 'var(--bitfun-type-heading-dialog-font-size)',
          fontWeight: 'var(--bitfun-type-label-selected-font-weight)',
          lineHeight: 'var(--bitfun-type-display-sm-line-height)',
          fontVariantNumeric: 'tabular-nums',
        }}
      >
        {value}
      </strong>
      <span style={{ color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-support-font-size)' }}>{label}</span>
    </div>
  );
}
...
export function Table({
        ...
        {safeRows.length ? (
          safeRows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              style={{
                background: striped && rowIndex % 2 === 1 ? 'var(--bitfun-color-surface-subtle)' : undefined,
              }}
            >
          ...
          <tr>
            <td colSpan={safeHeaders.length || 1} style={{ color: 'var(--bitfun-color-content-muted)' }}>
              {emptyMessage}
            </td>
          </tr>
        )}
  ...
  return framed ? (
    <div {...props} className={['bitfun-table-wrap', props.className].filter(Boolean).join(' ')} style={style}>
      {table}
    </div>
  ) : (
...
export function CollapsibleSection({
  ...
  return (
    <DesignDisclosure
      {...props}
      actions={trailing}
      className={['bitfun-collapsible-section', props.className].filter(Boolean).join(' ')}
      leading={leading}
      ...
      summary={(
        <>
          {title}
          {count !== undefined ? <span className="bitfun-collapsible-section__count">{count}</span> : null}
        </>
      )}
    >
      {children}
  ...
  return (
    <span
      ...
      style={{
        ...
        gap: 7,
        alignItems: 'center',
        fontSize: 'var(--bitfun-type-support-font-size)',
        fontVariantNumeric: 'tabular-nums',
        ...style,
      }}
    >
      {addCount ? <span style={{ color: 'var(--bitfun-color-code-change-added)' }}>+{addCount}</span> : null}
      {delCount ? <span style={{ color: 'var(--bitfun-color-code-change-removed)' }}>-{delCount}</span> : null}
    </span>
  );
}
...
export function DiffView({
  ...
  return (
    <div {...props} className={['bitfun-diff', props.className].filter(Boolean).join(' ')} style={style}>
      {normalizeDiffLines(lines).map((line, index) => {
        const type = line?.type;
        const accent =
          type === 'added' || type === 'addition'
            ? 'var(--bitfun-color-code-change-added)'
            : type === 'removed' || type === 'removal'
              ? 'var(--bitfun-color-code-change-removed)'
              : 'transparent';
        const bg =
          accent === 'var(--bitfun-color-code-change-added)'
            ? 'color-mix(in srgb, var(--bitfun-color-code-change-added) 12%, transparent)'
            : accent === 'var(--bitfun-color-code-change-removed)'
              ? 'color-mix(in srgb, var(--bitfun-color-code-change-removed) 12%, transparent)'
              : 'transparent';
        return (
          <div
            key={index}
          ...
          >
            ...
            {showLineNumbers ? (
              <span
                style={{
                  color: coloredLineNumbers && accent !== 'transparent' ? accent : 'var(--bitfun-color-content-muted)',
                  textAlign: 'right',
                  padding: '0 8px',
                  userSelect: 'none',
                }}
            ...
            <span
              style={{
                color: accent === 'transparent' ? 'var(--bitfun-color-content-muted)' : accent,
                userSelect: 'none',
              }}
            >
              {accent === 'var(--bitfun-color-code-change-added)' ? '+' : accent === 'var(--bitfun-color-code-change-removed)' ? '-' : ' '}
            </span>
            <span style={{ paddingRight: 10, color: 'var(--bitfun-color-content-primary)' }}>
              {line?.content || ''}
            </span>
          </div>
        );
...
export function KeyValueList({
  ...
  return (
    <dl
      {...props}
      className={['bitfun-key-value-list', props.className].filter(Boolean).join(' ')}
      style={{
        display: 'grid',
    ...
    >
      {entries.length ? (
        entries.map((item, index) => (
          <div
            ...
            style={{
              ...
              borderBottom: '1px solid var(--bitfun-color-border-subtle)',
            }}
          >
            <dt
              style={{
                ...
                fontSize: 'var(--bitfun-type-meta-font-size)',
                lineHeight: 'var(--bitfun-type-meta-line-height)',
              }}
            >
              {item.label}
            </dt>
            <dd
              style={{
                ...
                fontSize: compact ? 'var(--bitfun-type-body-xs-font-size)' : 'var(--bitfun-type-body-sm-font-size)',
                fontWeight: 'var(--bitfun-type-label-selected-font-weight)',
                lineHeight: 'var(--bitfun-type-meta-line-height)',
        ...
        <div style={{ color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-support-font-size)' }}>{emptyMessage}</div>
      )}
    </dl>
  );
}
...
export function Timeline({
  ...
  return (
    <ol
      {...props}
      className={['bitfun-timeline', props.className].filter(Boolean).join(' ')}
    ...
    >
      {safeItems.length ? (
            ...
            >
              <span
                ...
                style={{
                  ...
                  color,
                  fontSize: 'var(--bitfun-type-micro-font-size)',
                  fontWeight: 'var(--bitfun-type-heading-page-font-weight)',
                }}
              >
                {item.icon ?? ''}
              </span>
              <span style={{ minWidth: 0, display: 'grid', gap: 2 }}>
                ...
                >
                  <strong style={{ minWidth: 0, color: 'var(--bitfun-color-content-primary)', fontSize: 'var(--bitfun-type-body-sm-font-size)' }}>
                    {item.title}
                  </strong>
                  {item.time ? (
                    <time style={{ flex: '0 0 auto', color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-meta-font-size)' }}>
                      {item.time}
                    </time>
                  ) : null}
                </span>
                {item.description ? (
                  <span style={{ color: 'var(--bitfun-color-content-secondary)', fontSize: 'var(--bitfun-type-support-font-size)', overflowWrap: 'anywhere' }}>
                    {item.description}
                  </span>
                ) : null}
              </span>
        ...
        <li style={{ color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-support-font-size)' }}>{emptyMessage}</li>
      )}
    </ol>
  );
}
...
function renderFileTreeItems(items: CanvasFileTreeItem[], depth: number, defaultExpanded: boolean) {
  return items.map((item, index) => {
    ...
    const row = (
      ...
      >
        <span style={{ flex: '0 0 auto', width: 14, color: isFolder ? 'var(--bitfun-color-accent-default)' : 'var(--bitfun-color-content-muted)' }}>
          {isFolder ? '▸' : '•'}
        </span>
        <OverflowText
          style={{
            minWidth: 0,
            color: toneColor(item.tone),
            fontFamily: 'var(--bitfun-type-code-sm-font-family)',
        ...
        {item.meta ? (
          <span style={{ flex: '0 0 auto', marginLeft: 'auto', color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-meta-font-size)' }}>
            {item.meta}
          </span>
        ) : null}
      </span>
...
export function FileTree({
  ...
  return (
    <div
      {...props}
      className={['bitfun-file-tree', props.className].filter(Boolean).join(' ')}
      style={{
        minWidth: 0,
        overflow: 'auto',
        ...
        borderRadius: 8,
        padding: '8px 10px',
        background: 'color-mix(in srgb, var(--bitfun-color-surface-panel) 70%, transparent)',
        ...style,
      }}
    >
...
export function ProgressBar({
  ...
  return (
    <div {...props} className={['bitfun-progress', props.className].filter(Boolean).join(' ')} style={style}>
      {label || showValue ? (
        <div
          style={{
            display: 'flex',
...
export function Swatch({
  ...
  return (
    <span
      {...props}
      className={['bitfun-swatch', props.className].filter(Boolean).join(' ')}
...
export function UsageBar({
  ...
  return (
    <div {...props} className={['bitfun-usage-bar', props.className].filter(Boolean).join(' ')} style={style}>
...
function todoStatusColor(status: CanvasTodoItem['status']) {
  if (status === 'completed') return 'var(--bitfun-color-status-success-content)';
  if (status === 'in_progress') return 'var(--bitfun-color-status-warning-content)';
  if (status === 'cancelled') return 'var(--bitfun-color-content-muted)';
  return 'var(--bitfun-color-content-muted)';
}
  ...
  return (
    <span
      ...
      style={{
        ...
        color: 'var(--bitfun-color-surface-canvas)',
        fontSize: 'var(--bitfun-type-micro-font-size)',
        lineHeight: 'var(--bitfun-type-modifier-leading-none-line-height)',
        fontWeight: 'var(--bitfun-type-heading-page-font-weight)',
      }}
    >
      {isCompleted ? '✓' : ''}
...
export function TodoList({
  ...
  return (
    <div
      {...props}
      className={['bitfun-todo-list', props.className].filter(Boolean).join(' ')}
      style={{
        display: 'grid',
        gap: 4,
        ...style,
        ...
        const body = (
          <>
            ...
            <span style={{ minWidth: 0, display: 'grid', gap: 2 }}>
              <span
                style={{
                  color: todo.status === 'completed' ? 'var(--bitfun-color-content-secondary)' : 'var(--bitfun-color-content-primary)',
                  fontSize: 'var(--bitfun-type-support-font-size)',
                  lineHeight: 'var(--bitfun-type-support-line-height)',
              ...
              <span style={{ color: todoStatusColor(todo.status), fontSize: 'var(--bitfun-type-micro-font-size)', lineHeight: 'var(--bitfun-type-micro-line-height)' }}>
...
export function TodoListCard({
  ...
  return (
    <section
      {...props}
      className={['bitfun-todo-list-card', props.className].filter(Boolean).join(' ')}
      style={{
        border: '1px solid var(--bitfun-color-border-subtle)',
        borderRadius: 8,
        background: 'var(--bitfun-color-surface-raised)',
        overflow: 'hidden',
        ...style,
      }}
    >
      <button
        ...
        style={{
          ...
          gap: 8,
          border: 0,
          borderBottom: open ? '1px solid var(--bitfun-color-border-subtle)' : 0,
          background: 'transparent',
          color: 'var(--bitfun-color-content-primary)',
          padding: '8px 10px',
          font: 'inherit',
      ...
      >
        ...
        <span style={{ fontWeight: 'var(--bitfun-type-label-selected-font-weight)', fontSize: 'var(--bitfun-type-support-font-size)' }}>Tasks</span>
        <span style={{ marginLeft: 'auto', color: 'var(--bitfun-color-content-muted)', fontSize: 'var(--bitfun-type-support-font-size)' }}>
