...
function installBitFunCanvasRuntime(initialRevision: string): void {
  ...
  function makeAppearance(tokens: CanvasRuntimeRecord): CanvasRuntimeRecord {
    ...
    const codeChangeAdded = readToken(
      appearanceVars['--bitfun-color-code-change-added'],
      'var(--bitfun-color-code-change-added)',
    );
    const codeChangeRemoved = readToken(
      appearanceVars['--bitfun-color-code-change-removed'],
    ...
    const semanticBg = {
      editor: bg,
      chrome: 'var(--bitfun-color-surface-subtle)',
      elevated: panel,
    };
    const semanticText = {
      primary: fg,
      ...
      quaternary: muted,
      link: accent,
      onAccent: 'var(--bitfun-color-content-on-dark)',
    };
    const semanticFill = {
      primary: panel,
      secondary: 'var(--bitfun-color-action-neutral-surface)',
      tertiary: 'var(--bitfun-color-action-quiet-hover)',
      quaternary: 'var(--bitfun-color-surface-subtle)',
    };
    const semanticStroke = {
      primary: border,
      secondary: 'var(--bitfun-color-border-default)',
      tertiary: 'var(--bitfun-color-border-subtle)',
      focused: accent,
    };
  ...
  function applyAppearance(nextAppearance: CanvasRuntimeRecord): void {
    ...
    if (nextAppearance.type === 'dark' || nextAppearance.type === 'light') {
      ...
      document.querySelectorAll('[data-bitfun-design-system-root]').forEach(element => {
  ...
  function elementComponentName(element: Element): string | undefined {
    const className = Array.from(element.classList).find(name => name.startsWith('bitfun-') || name.startsWith('bitfun-'));
  ...
  function ErrorPanel({ error }: CanvasRuntimeRecord = {}) {
    return React.createElement('main', { style: { maxWidth: 860, margin: '0 auto', padding: 12, border: '1px solid var(--bitfun-color-border-default)', borderRadius: 8 } }, [
      React.createElement('h1', {
        key: 'title',
        style: {
          fontSize: 'var(--bitfun-type-heading-page-font-size)',
          fontWeight: 'var(--bitfun-type-heading-page-font-weight)',
          lineHeight: 'var(--bitfun-type-heading-page-line-height)',
          margin: '0 0 8px',
        },
