...
import { buildBitFunControlCardModel, getBitFunControlInput } from './bitFunControlCardModel';

function call(input: unknown, result?: unknown, status: FlowToolItem['status'] = 'completed'): FlowToolItem {
  return {
    id: 'control-1', type: 'tool', toolName: 'BitFunControl', timestamp: 0, status,
    toolCall: { id: 'call-1', input },
    toolResult: result === undefined ? undefined : { success: true, result },
  };
}

describe('BitFun control card projection', () => {
  it.each([
    ['list', { items: [] }, 'ambient'],
    ...
    expect(buildBitFunControlCardModel(item, 'en-US')).toMatchObject({ action, confirmed: true, failed: false });
  ...
  it.each([
    ...
    const model = buildBitFunControlCardModel(reloaded, 'en-US');
  ...
  it('uses executing-host labels first and preserves unknown remote IDs', () => {
    ...
    const model = buildBitFunControlCardModel(item, 'en-US', { id: 'feature.browser', titleEn: 'Local browser' });
    expect(model).toMatchObject({ target: 'Peer browser', confirmed: true, failed: false, availability: 'unavailable' });
    expect(buildBitFunControlCardModel(call({ action: 'open', capability_id: 'peer.future-feature' }), 'en-US',
  ...
  it('does not equate a completed invocation with an applied change or invent an effective value', () => {
    for (const result of [undefined, 'not JSON', [], { success: true }, { futureOutcome: true }]) {
      const model = buildBitFunControlCardModel(call({ action: 'configure', value_boolean: true }, result), 'en-US');
      expect(model.confirmed).toBe(false);
      expect(model.effectiveValue).toBeUndefined();
    }
    expect(buildBitFunControlCardModel(call({ action: 'configure' }, { configured: false }), 'en-US'))
    ...
    expect(buildBitFunControlCardModel(failed, 'en-US').confirmed).toBe(false);
  });

  it.each(['cancelled', 'rejected'] as const)('preserves %s instead of showing a failure or success', status => {
    const model = buildBitFunControlCardModel(call({ action: 'execute' }, { executed: false, success: false }, status), 'en-US');
  ...
  it('keeps discovery pagination and partial presentation sync explicit', () => {
    const page = buildBitFunControlCardModel(call({ action: 'search', query: 'theme' },
    ...
    const saved = buildBitFunControlCardModel(call({ action: 'configure', value_string: 'dark' },
  ...
  it('classifies deferred and streamed inputs consistently without losing finalized values', () => {
    const gateway = { ...call({ tool_name: 'BitFunControl', args: { action: 'search', query: 'theme' } }),
    ...
    projected.partialParams = { tool_name: 'BitFunControl', args: { action: 'configure', value_boolean: false } };
    expect(getBitFunControlInput(projected)).toMatchObject({ action: 'configure', value_boolean: false });
    ...
    expect(buildBitFunControlCardModel(projected, 'en-US').action).toBe('open');
    expect(getBitFunControlInput(projected)).not.toHaveProperty('value_boolean');
