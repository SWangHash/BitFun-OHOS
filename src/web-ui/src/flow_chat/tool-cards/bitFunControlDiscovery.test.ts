...
import { loadAllBitFunControlResults } from './bitFunControlDiscovery';
...
describe('complete BitFun discovery reads', () => {
  it('loads all 46 matches from a recorded 20-item page, preserving the query and history', async () => {
    ...
    const loaded = await loadAllBitFunControlResults({ action: 'search', query: 'appearance' }, original, scope, signal(), async request => {
  ...
  it('accepts legacy id aliases and absent optional metadata', async () => {
    await expect(loadAllBitFunControlResults({ action: 'list' }, {}, scope, signal(), async () => ({
  ...
  it('retains the recorded total when a response omits optional pagination metadata', async () => {
    await expect(loadAllBitFunControlResults({ action: 'list' }, { totalCount: 46 }, scope, signal(), async () => ({
      items: [{ id: 'first' }],
    }))).rejects.toMatchObject({ code: 'invalid-page' });
  });

  it('refuses to mix catalogs when the host version changes between pages', async () => {
    await expect(loadAllBitFunControlResults({ action: 'list' }, { catalogDigest: 'original' }, scope, signal(), async request => ({
  ...
  it.each([
    ...
    await expect(loadAllBitFunControlResults({ action: 'list' }, {}, scope, signal(), async () => page))
      .rejects.toMatchObject({ code: 'invalid-page' });
  });
  ...
  it('stops before the next read if the card is closed or its device surface changes', async () => {
    ...
    let reads = 0;
    await expect(loadAllBitFunControlResults({ action: 'list' }, {}, scope, controller.signal, async () => {
      reads++;
      controller.abort();
    ...
    let current = true;
    await expect(loadAllBitFunControlResults({ action: 'list' }, {}, {
