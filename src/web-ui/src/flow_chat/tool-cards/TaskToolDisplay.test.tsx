...
describeWithJsdom('TaskToolDisplay', () => {
  ...
  it('shows partial-timeout Review results without exposing partial output', async () => {
    ...
    expect(container.textContent).not.toContain('src/private.ts');
    expect(container.querySelector('[data-bitfun-part="agentStatus"]')?.textContent)
  ...
  it('shows a safe timeout outcome for a Review check', async () => {
    ...
    expect(container.querySelector('[data-bitfun-part="agentStatus"]')?.textContent).toBe('Timed out');
  });
  ...
  it('shows a stopped outcome for a cancelled Review check', async () => {
    ...
    expect(container.querySelector('[data-bitfun-part="agentStatus"]')?.textContent).toBe('Stopped');
  });
  ...
  it.each([
    ...
    expect(container.querySelector('[data-bitfun-part="processing"]')).toBeTruthy();
    expect(container.querySelector('[data-bitfun-part="interruptAgentButton"]')).toBeTruthy();
    expect(container.querySelector('[data-bitfun-part="agentStatus"]')).toBeNull();
  ...
  it.each([
    ...
    expect(container.querySelector('[data-bitfun-part="interruptAgentButton"]')).toBeNull();
  ...
  it('reacts when a running background child transitions to error', async () => {
    ...
    expect(container.querySelector('[data-bitfun-part="processing"]')).toBeFalsy();
    expect(container.textContent).toContain('toolCards.taskTool.failed');
    expect(container.querySelector('[data-completed-status="error"]')).toBeTruthy();
  });
  ...
  it('opens the real subagent session in the aux pane when the task card rail is clicked', async () => {
    ...
    const openButton = container.querySelector<HTMLButtonElement>('[data-bitfun-part="openAgentButton"]');
    expect(openButton).toBeTruthy();
    expect(container.querySelector('[data-bitfun-component="subagent-avatar"][data-bitfun-avatar-id]'))
      .toBeTruthy();
    expect(container.textContent).not.toContain('repo-investigator');
    expect(container.querySelector('[data-bitfun-part="subagentName"]')).toBeNull();
  ...
  it('renders spawn task cards from the result subagent session metadata', async () => {
    ...
    const summary = container.querySelector<HTMLElement>('[data-bitfun-tool-card="agent-control"] [data-bitfun-part="summary"]');
  ...
  it('stops a running foreground subagent from the task summary actions', async () => {
    ...
    const stopButton = container.querySelector<HTMLButtonElement>('[data-bitfun-part="interruptAgentButton"]');
  ...
  it('does not report a stop failure when the Review check already completed', async () => {
    ...
    await act(async () => {
      container.querySelector<HTMLButtonElement>('[data-bitfun-part="interruptAgentButton"]')!
  ...
  it('renders cancelled foreground subagent results as cancelled instead of failed', async () => {
    ...
    expect(container.querySelector('[data-bitfun-part="agentStatus"]')).toBeTruthy();
    expect(container.querySelector(
      '[data-bitfun-component="flow-chat-tool-card"][data-bitfun-part="surface"][data-bitfun-status="cancelled"]',
    )).toBeTruthy();
  ...
  it('keeps cancel task cards collapsed and disables opening the subagent session', async () => {
    ...
    expect(container.querySelector('[data-bitfun-part="openAgentButton"]')).toBeNull();
    expect(container.querySelector('[data-bitfun-part="surface"][data-bitfun-attention="ambient"]')).toBeTruthy();
