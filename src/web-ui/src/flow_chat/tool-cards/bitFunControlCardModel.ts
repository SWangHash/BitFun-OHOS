...
export type BitFunControlAction = 'list' | 'search' | 'get' | 'open' | 'execute' | 'configure';
...
export function getBitFunControlInput(toolItem: Pick<FlowToolItem, 'toolCall' | 'partialParams' | 'isParamsStreaming'>): ControlRecord {
  ...
  const params = partial.tool_name === 'BitFunControl' ? controlRecord(partial.args) : partial;
  return toolItem.isParamsStreaming ? { ...input, ...params }
    : Object.keys(input).length > 0 ? input : params;
}

export function isBitFunControlDiscovery(input: unknown): boolean {
  return ['list', 'search', 'get'].includes(String(controlRecord(input).action ?? '').trim());
}

function actionOf(input: ControlRecord): BitFunControlAction | undefined {
  const action = text(input.action);
  return action && ['list', 'search', 'get', 'open', 'execute', 'configure'].includes(action)
    ? action as BitFunControlAction
    : undefined;
}
...
export function buildBitFunControlCardModel(
  toolItem: FlowToolItem,
  language: string,
  localCapability?: unknown,
) {
  const input = getBitFunControlInput(toolItem);
...
export type BitFunControlCardModel = ReturnType<typeof buildBitFunControlCardModel>;
