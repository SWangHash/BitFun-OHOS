...
import { buildBitFunControlCardModel } from './bitFunControlCardModel';
...
const item: FlowToolItem = {
  id: 'control', type: 'tool', toolName: 'BitFunControl', timestamp: 0, status: 'completed',
...
describe('discovery view state', () => {
  ...
  let output: ReturnType<typeof useBitFunControlDiscovery>;
  ...
  function Probe({ expanded, toolItem = item }: { expanded: boolean; toolItem?: FlowToolItem }) {
    const model = useMemo(() => buildBitFunControlCardModel(toolItem, 'en-US'), [toolItem]);
    output = useBitFunControlDiscovery(toolItem, model, expanded);
    return null;
  }
