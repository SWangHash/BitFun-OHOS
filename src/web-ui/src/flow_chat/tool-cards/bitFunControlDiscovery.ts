...
export async function loadAllBitFunControlResults(
  ...
  for (;;) {
    scope.assertCurrent('Read BitFun discovery results');
    signal.throwIfAborted();
    const page = controlRecord(await readPage({ ...request, cursor }));
    scope.assertCurrent('Apply BitFun discovery results');
    signal.throwIfAborted();
