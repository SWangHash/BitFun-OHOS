...
import { Button, Dialog, IconButton, Tooltip, VoiceCallTranscript, type VoiceTranscriptEntry } from '@bitfun/ui';
...
export function ControlConversation({ session, sessionRef, voiceTarget, active, renderHeader, onClose, onVoiceViewChange, onNewConversation }: ControlConversationProps) {
  ...
  for (const row of visibleRows) {
    ...
    if (turn.userMessage.content || turn.userMessage.images?.length) entries.push({ id: `${row.id}:user`, role: 'user', content: <>
      ...
      {!!turn.userMessage.images?.length && <div className="bitfun-conversation-mode-surface__images"
        data-bitfun-component="conversation-mode-surface" data-bitfun-part="images">
  ...
  const historyFeedback = failedHistory || historyBusy ? <div className="bitfun-conversation-mode-surface__history" role="status"
    data-bitfun-component="conversation-mode-surface" data-bitfun-part="history">
  ...
  return <>
    <ConversationModeSurface voiceTarget={voiceTarget} renderHeader={renderHeader} active={active}
      ...
      transcript={mode => <VoiceCallTranscript compact presentation={mode} entries={entries} status={status || sessionError} header={historyFeedback}
        ...
        className="bitfun-conversation-mode-surface__transcript" />}>
    ...
    {preview && <Dialog open aria-label={t('context.image')} onOpenChange={() => setPreview(null)}>
      <img className="bitfun-conversation-mode-surface__image-preview" src={preview} alt={t('context.image')}
        data-bitfun-component="conversation-mode-surface" data-bitfun-part="imagePreview" />
    </Dialog>}
  </>;
}