...
import { VoiceCallTranscript } from '@bitfun/ui';
...
describe('ConversationModeSurface', () => {
  ...
  it('places the logo control in the host header without a footer and preserves the call back button', async () => {
    ...
    expect(container.querySelector('header .bitfun-conversation-mode-surface__logo')).not.toBeNull();
  ...
  it('switches to text without hanging up or moving the call to another conversation', async () => {
    ...
    expect(container.querySelector('[data-testid="voice-panel"]')).toBeNull();
    expect(container.querySelector<HTMLButtonElement>('[data-bitfun-part="modeSwitchButton"]')?.disabled).toBe(true);
    expect(mocks.controller.start).not.toHaveBeenCalled();
  });
  ...
  describe('persistent identity and transcript', () => {
    ...
    const identity = () => container.querySelector<HTMLButtonElement>('[data-bitfun-part="identity"] button[aria-expanded]')!;
    const back = () => container.querySelector<HTMLButtonElement>('[data-bitfun-part="voiceHeader"] button[aria-label="voiceCall.call.switchToChat"]')!;
    ...
    it.each([false, true])('keeps the reading viewport through entry, return and hangup (mode-aware: %s)', async modeAware => {
      ...
      const record = container.querySelector('[data-bitfun-part="conversation"]');
      const userMessage = container.querySelector('[data-transcript-id="user"]');
      expect(record?.getAttribute('data-bitfun-presentation')).toBe(modeAware ? 'chat' : 'voice');
      const input = container.querySelector<HTMLInputElement>('input')!;
      await act(async () => identity().click());
      expect(mocks.controller.start).toHaveBeenCalledWith(miniAppTarget);
      mocks.controller.phase = 'live'; mocks.controller.target = miniAppTarget;
      await act(async () => renderIntegrated(false, modeAware));
      expect(record?.getAttribute('data-bitfun-presentation')).toBe('voice');
      expect(identity().hidden).toBe(true);
      expect(container.querySelector('[data-bitfun-part="voiceHeader"]')?.getAttribute('aria-hidden')).toBe('false');
      expect(back().querySelector('.lucide-arrow-left')).not.toBeNull();
      expect(container.querySelector('[data-bitfun-part="composer"]')?.getAttribute('aria-hidden')).toBe('true');
      ...
      expect(container.querySelector('[data-bitfun-part="conversation"]')).toBe(record);
    ...
    it('keeps a blocking response reachable without ending the live call', async () => {
      ...
      expect(container.querySelector('[data-bitfun-part="composer"]')?.getAttribute('aria-hidden')).toBe('false');
      expect(mocks.controller.end).not.toHaveBeenCalled();
    });
