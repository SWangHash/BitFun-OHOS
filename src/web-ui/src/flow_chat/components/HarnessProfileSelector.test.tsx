...
describe('HarnessProfileSelector', () => {
  ...
  it('portals the menu-item picker outside its parent scroll boundary and closes it after an Agent choice', async () => {
    ...
    const selectorRoot = container.querySelector<HTMLElement>(
      '[data-bitfun-component="harness-selector"][data-bitfun-part="root"]',
    );
    const trigger = container.querySelector<HTMLButtonElement>(
      '[data-testid="harness-profile-selector"]',
    );
    expect(selectorRoot?.dataset.bitfunPresentation).toBe('menu-item');
    expect(selectorRoot?.dataset.bitfunProfile).toBe('Standard');
    ...
    expect(triggerMark?.querySelector('[data-bitfun-name="standard"][data-size="md"]')).not.toBeNull();
    expect(
      trigger?.querySelector('[data-bitfun-part="label"]')?.textContent,
    ...
    expect(menu?.dataset.bitfunPlacement).toBe('side');
    ...
    expect(
      menu?.querySelector('[data-testid="harness-agent-DeepResearch"] [data-bitfun-name="user"][data-size="md"]'),
    ).not.toBeNull();
  ...
  it('offers three Harness gears, Creative, and the second-level Agents entry', async () => {
    ...
    const menu = document.querySelector<HTMLElement>('.bitfun-harness-selector__menu');
    expect(menu).not.toBeNull();
    const rows = Array.from(menu!.querySelectorAll<HTMLElement>('[data-bitfun-part="profile"]'));
    expect(rows.map(row => row.dataset.bitfunProfile)).toEqual([
      'Minimal',
      'Standard',
      'Ultimate',
      'Creative',
    ...
    expect(rows[1]?.dataset.bitfunState).toBe('current');
  });
  ...
  it('creates a new Session from a different main Agent without mutating the current Agent', async () => {
    ...
    expect(document.querySelector('.bitfun-harness-selector__menu')?.getAttribute('data-bitfun-page'))
      .toBe('profiles');
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[data-testid="harness-profile-other"]')
        ?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    ...
    expect(
      document.querySelector<HTMLElement>('[data-testid="harness-agent-Plan"]')?.dataset.bitfunState,
    ).toBe('available');
    expect(
      document.querySelector<HTMLElement>('[data-testid="harness-agent-Cowork"]')?.dataset.bitfunState,
    ).toBe('available');
  ...
  it.each(['Creative'] as const)(
    ...
    async (profileId) => {
      ...
      expect(profile?.dataset.bitfunState).toBe('current');
