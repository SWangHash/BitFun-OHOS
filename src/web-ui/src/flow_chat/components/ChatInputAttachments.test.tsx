...
describe('numbered composer attachments', () => {
  ...
  const chip = () => container.querySelector<HTMLButtonElement>('[data-bitfun-product-part="attachment"]')!;
  ...
  afterEach(() => {
    ...
    document.querySelectorAll('[data-bitfun-overlay-host]').forEach(host => host.remove());
    vi.unstubAllGlobals();
  });

  it('groups annotations in a capsule, previews on hover, and removes only the chosen detail', () => {
    act(() => root.render(<Composer />));
    const strip = container.querySelector('[data-bitfun-part="imageStrip"]')!;
    expect(strip.querySelector('img')?.alt).toBe('Photo.png');
    expect(strip.contains(chip())).toBe(true);
    expect(document.querySelector('[data-bitfun-product-part="details"]')).toBeNull();
    act(() => chip().dispatchEvent(new MouseEvent('mouseover', { bubbles: true })));
    flushFrames();
    expect(document.querySelector('[data-bitfun-product-part="details"]')?.textContent).toContain(excerpt.fragments[0].text);
    expect(document.querySelector('[data-bitfun-product-part="details"]')?.textContent).toContain('First comment');
  ...
  it('keeps the sent snapshot read-only when the input contains an edited copy with the same ID', () => {
    ...
    expect(dialog()!.querySelector('[data-bitfun-product-part="comment"]')!.textContent).toBe('First comment');
    ...
    click(container.querySelector<HTMLButtonElement>('[data-bitfun-product-part="attachment"]')!);
    click(trigger(1));
  ...
  it('shows only the source quote and locate action for a sent annotation without a comment', () => {
    ...
    click(container.querySelector<HTMLButtonElement>('button[aria-label="Annotation 1"]')!);
    expect(dialog()!.querySelector('textarea, input, [data-bitfun-product-part="comment"]')).toBeNull();
    expect(dialog()!.querySelector('[data-bitfun-product-part="quote"]')!.textContent).toBe(excerpt.fragments[0].text);
  ...
  it('opens the same editor from source and attachment and reads the latest draft from either entry', () => {
    ...
    const source = container.querySelector<HTMLButtonElement>('[data-bitfun-product-part="superscript"]')!;
    const attachment = () => container.querySelector<HTMLButtonElement>('[data-bitfun-product-part="attachment"]')!;
    click(source);
    const actions = [...dialog()!.querySelectorAll('button')].map(button => button.textContent);
    expect(dialog()!.dataset.bitfunProductPart).toBe('dialog');
  ...
  it('removes from a source marker dialog and clears the linked draft and composer attachment', () => {
    ...
    act(() => root.render(<LinkedComposer />));
    click(container.querySelector<HTMLButtonElement>('[data-bitfun-product-part="superscript"]')!);
