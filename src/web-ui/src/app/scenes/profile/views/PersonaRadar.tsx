    ...
    >
      ...
      {ringPaths.map((d, i) => (
        <path
          ...
          d={d}
          style={{
            fill: i === RING_FRACTIONS.length - 1 ? 'var(--bitfun-color-surface-subtle)' : 'none',
      ...
      {vertices.map((v, i) => (
        <line
          ...
          style={{ stroke: 'var(--bitfun-color-border-subtle)', strokeWidth: 0.7 }}
        />
      ))}
      ...
      <path
        d={scorePath}
        style={{
          fill: 'var(--bitfun-color-accent-default)',
          fillOpacity: 0.12,
          stroke: 'var(--bitfun-color-accent-default)',
          strokeWidth: 1.2,
      ...
      {/* Score dots */}
      {dots.map((p, i) => (
        <circle key={`dot-${i}`} cx={p.x} cy={p.y} r={2.2} style={{ fill: 'var(--bitfun-color-accent-default)' }} />
      ))}
        ...
        return (
          <g key={`label-${i}`}>
            ...
            <text
              ...
              style={{
                fontSize: 'var(--bitfun-type-overline-sm-font-size)',
                fill: hasClick ? 'var(--bitfun-color-content-secondary)' : 'var(--bitfun-color-content-muted)',
