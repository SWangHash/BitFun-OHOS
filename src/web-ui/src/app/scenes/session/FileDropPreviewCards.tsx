...
function PreviewTile({ file }: { file: FileDropPreviewItem }) {
  ...
  return (
    <div className="bitfun-chat-pane__file-tile" data-kind={kind}>
...
export function FileDropPreviewCards({ preview }: { preview: FileDropPreview }) {
  ...
  return (
    <div className="bitfun-chat-pane__file-preview" data-multiple={preview.count > 1}
      data-stack-depth={Math.min(preview.count, 3)}>
      <div className="bitfun-chat-pane__file-front">
        {firstFile && <PreviewTile key={firstFile.name} file={firstFile} />}
        <div className="bitfun-chat-pane__file-caption">
          <OverflowText>{firstFile?.name}</OverflowText>
        </div>
      </div>
      {preview.count > 1 && <span className="bitfun-chat-pane__file-count"
