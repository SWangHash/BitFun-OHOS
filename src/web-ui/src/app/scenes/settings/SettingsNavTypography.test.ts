...
describe('SettingsNav typography and layout ownership', () => {
  it('uses the shared navigation semantic text roles without a local font scale', () => {
    ...
    expect(stylesheet).not.toContain('--bitfun-appearance-token-font-size-');
    // typography-audit: negative-test-end
    expect(stylesheet).toContain('font-size: var(--bitfun-type-meta-font-size);');
    expect(stylesheet).toContain('line-height: var(--bitfun-type-meta-line-height);');
  ...
  it('keeps search and body separated by the navigation spacing token', () => {
    ...
    expect(root.getPropertyValue('gap')).toBe('var(--bitfun-layout-navigation-panel-content-gap)');
    expect(header.getPropertyValue('display')).toBe('flex');
    expect(header.getPropertyValue('flex-direction')).toBe('column');
    expect(header.getPropertyValue('gap')).toBe('var(--bitfun-layout-navigation-panel-content-gap)');
    expect(header.getPropertyValue('padding')).toBe('0px');
    expect(root.getPropertyValue('padding-inline-end')).toBe('0px');
    expect(header.getPropertyValue('padding-inline-end')).toBe('var(--bitfun-space-2)');
  });

  it('resolves shared navigation text against the current chrome instead of secondary action ink', () => {
    const root = declarations('.bitfun-settings-nav');
    expect(root.getPropertyValue('--bitfun-color-content-primary')).toBe('var(--bitfun-color-content-primary)');
    expect(root.getPropertyValue('--bitfun-color-action-neutral-content')).toBe('');
  });
  ...
  it('leaves group spacing, heading typography, and item states to NavigationPanel while indenting child items', () => {
    ...
    expect(content.getPropertyValue('padding-inline-end')).toBe('var(--bitfun-space-2)');
    expect(content.getPropertyValue('gap')).toBe('');
    expect(item.getPropertyValue('padding-inline-start')).toBe(
      'calc(var(--bitfun-layout-navigation-panel-item-padding-inline) + var(--bitfun-control-icon-button-xs-size) + var(--bitfun-space-2))',
    );
    expect(readSettingsNavStylesheet()).not.toContain(
      'padding-inline-start: calc(var(--bitfun-layout-navigation-panel-item-padding-inline) + 30px)',
    );
  ...
  it('stacks search copy inside the shared label without overriding the selected item', () => {
    ...
    expect(highlight?.style.getPropertyValue('background')).toBe('var(--bitfun-color-action-neutral-surface)');
  });
});
