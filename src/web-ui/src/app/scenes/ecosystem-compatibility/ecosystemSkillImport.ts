...
import { isBitFunManagedSkill } from '@/infrastructure/config/skillSourcePresentation';

export function suggestSkillImportName(skill: SkillInfo, level: SkillLevel, skills: SkillInfo[], reserved: string[] = []) {
  const names = new Set([...skills.filter((entry) => isBitFunManagedSkill(entry) && entry.level === level)
