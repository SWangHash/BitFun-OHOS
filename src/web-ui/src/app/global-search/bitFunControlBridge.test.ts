...
describe('BitFunControl presentation bridge', () => {
  ...
  it('advertises Creation API after listener setup and cleans up a failed handshake', async () => {
    ...
    await expect(initializeBitFunControlBridge()).rejects.toThrow('Host unavailable');
    expect(listeners.has('agentic://creation-runtime-request')).toBe(true);
    expect(listeners.has('agentic://bitfun-control-request')).toBe(true);
    expect(Math.max(...listen.mock.invocationCallOrder)).toBeLessThan(invoke.mock.invocationCallOrder[0]);
    expect(invoke).toHaveBeenCalledWith('mark_bitfun_control_surface_ready', {
