...
export function useSceneTabNavigation({
  ...
  useLayoutEffect(() => {
    ...
    getTabElements().forEach((tab) => {
      observer.observe(tab.closest<HTMLElement>('[data-bitfun-part="item"]') ?? tab);
    });
  ...
  useLayoutEffect(() => {
    const tabs = tabsRef.current;
    const activeTab = getTabElements().find(tab => tab.dataset.bitfunValue === activeTabId);
    const activeItem = activeTab?.closest<HTMLElement>('[data-bitfun-part="item"]') ?? activeTab;
