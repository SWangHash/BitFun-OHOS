...
describe('AssistantAvatar', () => {
  ...
  it('maps a retired SVG preset id to the new Claw artwork', () => {
    ...
    const avatar = container.querySelector('[data-bitfun-component="assistant-avatar"]');
    const image = container.querySelector('.assistant-avatar__image') as HTMLImageElement;
    expect(avatar?.getAttribute('data-bitfun-preset')).toBe('claw-orbit');
    expect(avatar?.getAttribute('data-bitfun-family')).toBe('clawOrbit');
  ...
  it('keeps the legacy emoji when a preset id is missing or unknown', () => {
    ...
    const avatar = container.querySelector('[data-bitfun-component="assistant-avatar"]');
    expect(avatar?.getAttribute('data-bitfun-family')).toBe('emoji');
  ...
  it('renders the new default avatar without discarding the accessible identity', () => {
    ...
    const avatar = container.querySelector('[data-bitfun-component="assistant-avatar"]');
    const image = container.querySelector('.assistant-avatar__image') as HTMLImageElement;
    expect(avatar?.getAttribute('data-bitfun-family')).toBe('claw');
    expect(avatar?.getAttribute('data-bitfun-preset')).toBe('claw');
