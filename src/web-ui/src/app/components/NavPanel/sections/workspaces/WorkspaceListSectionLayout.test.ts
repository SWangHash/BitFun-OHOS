...
describe('WorkspaceListSection layout styles', () => {
  it('keeps peer session groups airier than rows inside one group', () => {
    ...
    expect(workspaceList).toContain('gap: var(--bitfun-space-2);');
  ...
  it('keeps workspace rows constrained while only visible row actions reserve title space', () => {
    ...
    expect(workspaceItem).toContain('gap: calc(var(--bitfun-space-1) / 2);');
    ...
    expect(readWorkspaceItemSource()).toMatch(
      /<OverflowText\s+behavior="marquee"\s+className="bitfun-nav-panel__workspace-item-label"/,
    );
    ...
    expect(assistantItem).toContain('gap: calc(var(--bitfun-space-1) / 2);');
    ...
    expect(stylesheet).toContain('--bitfun-nav-session-rail: 30px;');
  ...
  it('keeps nested selection surfaces full-width while aligning their titles', () => {
    ...
    const sessionRails = stylesheet.match(/--bitfun-nav-session-rail: 30px;/g);
  ...
  it('keeps remote workspace directory and host metadata on one row with the status dot first', () => {
    ...
    const remoteNameRow = extractBlock(
      stylesheet,
      "&__workspace-item[data-bitfun-state~='remote'] &__workspace-item-name-row",
    );
    const remoteNameButton = extractBlock(
      stylesheet,
      "&__workspace-item[data-bitfun-state~='remote'] &__workspace-item-name-btn",
    );

    expect(remoteNameRow).toContain('gap: var(--bitfun-space-2);');
    expect(remoteNameButton).toContain('flex: 1 1 0;');
    expect(remoteNameButton).toContain('max-width: none;');
    expect(remoteChip).toContain('gap: var(--bitfun-space-2);');
    expect(remoteChip).toContain('flex: 0 0 auto;');
    expect(remoteChip).toContain('margin-left: auto;');
    expect(remoteChip).toContain('padding: 0 var(--bitfun-space-2);');
    expect(remoteChip).toContain('border-radius: var(--bitfun-radius-base);');
    expect(stylesheet).toContain(
      '&.is-connected {\n' +
      '      background: var(--bitfun-color-status-success-surface);\n' +
  ...
  it('keeps remote workspace icons aligned with the single-line row', () => {
    ...
    expect(stylesheet).not.toContain(
      "&__workspace-item[data-bitfun-state~='remote'] &__workspace-item-collapse-btn",
    );
  });
