...
describe('worktree isolated session row', () => {
  ...
  it('marks the row with an accessible worktree badge', () => {
    expect(sessionsSection).toContain('bitfun-nav-panel__inline-item-worktree-badge');
    expect(sessionsSection).toMatch(
  ...
  it('keeps the worktree badge icon-only', () => {
    ...
    expect(sessionsSection).toMatch(
      /inline-item-worktree-badge"[\s\S]{0,400}?<FolderGit2 className="bitfun-nav-panel__inline-item-worktree-icon" aria-hidden \/>\s*<\/span>/,
    );
    expect(sessionsSectionStyles).toMatch(
      /&__inline-item-worktree-badge \{[\s\S]*?inline-size: var\(--bitfun-control-icon-size-md\);/,
    );
    // The glyph keeps a normal icon size inside the badge circle.
    expect(sessionsSectionStyles).toMatch(
      /\.bitfun-nav-panel__inline-item-worktree-icon \{[\s\S]*?inline-size: var\(--bitfun-control-icon-size-xs\);/,
    );
  });
