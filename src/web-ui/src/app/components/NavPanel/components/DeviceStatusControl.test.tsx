...
describe('device status card', () => {
  ...
  it('names this machine instead of a bare in-use state, and keeps the action row either way', () => {
    ...
    const localActionRow = summary.querySelector('.bitfun-device-overview__connect-action');
    ...
    expect(localActionButton!.className).toContain('bitfun-device-overview__connect-reserved');
    ...
    expect(summary.querySelector('.bitfun-device-overview__connect-action')).not.toBeNull();
  });
  ...
  it('draws this machine as the attached desktop, by the system it reported', () => {
    ...
    const attachedDesktop = () => footer()
      .querySelector('[data-bitfun-device-kind="desktop"]');
    ...
    render();
    expect(footer().querySelector('[data-bitfun-device-kind="mobile"] [data-system]')).toBeNull();
  ...
  it('draws a group of headless hosts with the mark the list draws for them', () => {
    ...
    const headlessGroup = () => footer()
      .querySelector('[data-bitfun-device-kind="execution-host"]');
  ...
  it('marks a device name with the system that device runs', () => {
    ...
    const row = (text: string) => Array.from(
      rows.querySelectorAll('.bitfun-device-overview__device-row'),
  ...
  it('keeps an incompatible peer in the switch list but never connects to it', async () => {
    ...
    expect(document.querySelectorAll('.bitfun-device-overview__connect-action button')).toHaveLength(1);
