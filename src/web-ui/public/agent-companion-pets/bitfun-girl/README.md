...
Fangling is an original chibi adaptation of the BitFun Girl character, with a
...
Silver hair and BitFun ornaments preserve the character's identity: both the
...
cat. Fangling keeps the `bitfun-girl` package ID and paths so saved selections
continue to work after the display-name change. The earlier BitFun Bifang
