...
mod tests {
    ...
    fn installed_miniapp_crud_uses_isolated_product_storage() {
        ...
        let output = bitfun_core::util::process_manager::create_command(
            std::env::current_exe().unwrap(),
        )
        .args([
            "--ignored",
            "--exact",
            "bitfun_control_host::miniapps::tests::isolated_crud_child",
            "--nocapture",
        ])
        .env("BITFUN_CREATION_CRUD_TEST", "1")
        .env("BITFUN_USER_ROOT", root.path().join("user"))
        .env("BITFUN_HOME", root.path().join("home"))
    ...
    async fn isolated_crud_child() {
        assert_eq!(
            std::env::var("BITFUN_CREATION_CRUD_TEST").as_deref(),
            Ok("1")
        );
        let paths =
            std::sync::Arc::new(bitfun_core::infrastructure::PathManager::new().unwrap());
