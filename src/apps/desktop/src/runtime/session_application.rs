    ...
    async fn resolve(
        ...
        let workspace = if let Some(id) = request.workspace_id.as_deref() {
            ...
            self.workspace_service
                ...
                .await
                .and_then(|record| {
                    record.ok_or_else(|| BitFunError::service("Workspace ID is unavailable"))
                })
        }
        .map_err(desktop_core_session_error)?;
        use bitfun_runtime_ports::SessionStorePort;
        let storage = bitfun_core::agentic::session::CoreSessionStorePort::default()
            .resolve_workspace_storage(&workspace.id)
            .await
            .map_err(|error| DesktopSessionApplicationError::Core(error.to_string()))?;
        let remote =
            workspace.workspace_kind == bitfun_core::service::workspace::WorkspaceKind::Remote;
...
fn merge_ui_owned_session_metadata(
    ...
    if fields.contains(&UiSessionMetadataField::UnreadCompletion) {
        bitfun_core::service::session::apply_session_unread_completion(current, incoming);
    }
    ...
    if fields.contains(&UiSessionMetadataField::TitleMetadata) {
        bitfun_core::service::session::apply_session_title_metadata(current, incoming);
    }
}
