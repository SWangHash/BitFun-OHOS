...
fn check(pid: i32, window: u32) -> BitFunResult<()> {
...
pub(super) fn prepare(pid: i32) -> BitFunResult<()> {
    super::control_session::input_allowed().map_err(BitFunError::tool)?;
    let window = super::macos_capture::bound_window_id(pid).map_err(BitFunError::tool)?;
    check(pid, window)?;
    let foreground = super::macos_bg_input::frontmost_pid_macos().ok_or_else(|| {
        BitFunError::tool("[BACKGROUND_INPUT_UNAVAILABLE] Foreground ownership is unavailable")
    })?;
    ...
    let [x, y, width, height] =
        super::macos_capture::window_bounds(pid, window).map_err(BitFunError::tool)?;
    let mut state = FOCUS
        .lock()
        .map_err(|_| BitFunError::tool("Background input focus lock poisoned"))?;
    let focus = Focus {
        pid,
        identity,
        window,
    ...
    for kind in [CGEventType::LeftMouseDown, CGEventType::LeftMouseUp] {
        let event = CGEvent::new_mouse_event(source.clone(), kind, point, CGMouseButton::Left)
            .map_err(|_| BitFunError::tool("Background focus event unavailable"))?;
    ...
    *state = Some(focus);
    if !super::macos_skylight::set_local_activation(identity, window, true) {
        state.take();
        return Err(BitFunError::tool("[BACKGROUND_INPUT_UNAVAILABLE] Target-local input activation failed; no content input was sent"));
    }
