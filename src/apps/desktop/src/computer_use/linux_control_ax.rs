...
fn error(message: impl std::fmt::Display) -> BitFunError {
    BitFunError::tool(format!("[AT_SPI] {message}"))
}

async fn applications(
    connection: &AccessibilityConnection,
) -> BitFunResult<Vec<(AppInfo, ObjectRefOwned)>> {
    let root = connection
...
pub(super) async fn list_apps() -> BitFunResult<Vec<AppInfo>> {
...
fn bound_selector(target: Option<&str>) -> BitFunResult<AppSelector> {
...
pub(super) fn bound_app_selector() -> BitFunResult<AppSelector> {
...
pub(super) async fn bind_app_selector(selector: &AppSelector) -> BitFunResult<()> {
    let connection = AccessibilityConnection::new().await.map_err(error)?;
...
) -> BitFunResult<(AppInfo, ObjectRefOwned)> {
    let bound;
...
async fn cached_target(
    selector: &AppSelector,
    index: u32,
) -> BitFunResult<(AccessibilityConnection, CachedNode)> {
    let connection = AccessibilityConnection::new().await.map_err(error)?;
    let (app, root) = resolve(&connection, selector).await?;
    let pid = app
        .pid
...
pub(super) async fn press(selector: &AppSelector, index: u32) -> BitFunResult<()> {
...
mod native_tests {
    ...
    async fn atspi_semantic_fixture() {
        ...
        let pid: i32 = std::env::var("BITFUN_ATSPI_FIXTURE_PID")
