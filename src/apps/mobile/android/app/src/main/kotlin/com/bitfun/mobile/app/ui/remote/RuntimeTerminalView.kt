...
import com.bitfun.mobile.core.feature.workspace.RuntimeTerminalUiState
...
internal fun RuntimeTerminalView(state: RuntimeTerminalUiState, onInput: (String) -> Unit, onResize: (Int, Int) -> Unit, modifier: Modifier) {
    ...
    AndroidView(modifier = modifier, factory = { context ->
        WebView(context).apply {
            ...
            bridge = adapter
            addJavascriptInterface(adapter, "BitFunTerminalHost")
            webViewClient = object : WebViewClient() {
                ...
                override fun onPageFinished(view: WebView?, url: String?) { evaluateJavascript("window.BitFunTerminal.connect()", null) }
            }
...
private class TerminalBridge(private val view: WebView, private val input: (String) -> Unit, private val resize: (Int, Int) -> Unit) {
    ...
    private fun renderTheme() {
        if (ready && !disposed) theme?.let { view.evaluateJavascript("window.BitFunTerminal.setTheme($it)", null) }
    }
    ...
    private fun render(force: Boolean) {
        ...
        view.evaluateJavascript("window.BitFunTerminal.accept($frame)", null)
    ...
    fun dispose() { disposed = true; view.removeJavascriptInterface("BitFunTerminalHost"); view.destroy() }
}
