...
class AccountStoreTest {
    ...
    @Test fun oldOfficialRelayTokenIsRetainedButNeverSentToNewRelay() = runTest {
        ...
        val old = secure.read("github_device_session_v1")!!.decodeToString().replace(AccountDefaults.CLOUD_RELAY_URL, "https://remote.bitfun.com/v/retired")
    ...
    fun enrichesExistingAccountAndRetainsIdentityAndCacheOffline() = runTest {
        ...
        val backend = FakeAccountBackend().apply {
            userId = "42"
            profileResult = com.bitfun.mobile.core.transport.GitHubProfile(42, "octocat", "https://avatars.githubusercontent.com/u/42")
        }
    ...
    fun metadataForAnotherAccountCannotReplaceDisplayOrAuthority() = runTest {
        val backend = FakeAccountBackend().apply {
            userId = "42"
            profileResult = com.bitfun.mobile.core.transport.GitHubProfile(99, "other", null)
        }
        val store = AccountStore.create(this, backend, MemorySecureStore(), "phone-1", "Android")
        store.dispatch(AccountIntent.Login)
        advanceUntilIdle()
...
private class FakeAccountBackend : AccountBackend {
    ...
    var listRequests = 0
    var lastLoginRelay = ""
    var profileResult: com.bitfun.mobile.core.transport.GitHubProfile? = null
    var profileLoads = 0
    override suspend fun profile(userId: String): com.bitfun.mobile.core.transport.GitHubProfile? {
        profileLoads++
