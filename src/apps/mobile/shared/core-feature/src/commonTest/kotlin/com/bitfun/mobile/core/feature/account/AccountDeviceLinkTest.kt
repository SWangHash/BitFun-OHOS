...
class AccountDeviceLinkTest {
    @Test fun explicitLegacyConstructorsPreserveTheExistingDefaults() {
        ...
        assertEquals("https://remote.bitfun.com/v/1.0.1", legacy.relayUrl)
        assertEquals(false, legacy.refreshing)
        assertNull(legacy.refreshFailure)
    }

    private val link = "https://remote.bitfun.com/v/1.0.1/#/pair?did=desktop-1"
    private val ready = AccountUiState.Ready(userId = "user", username = "name", relayUrl = "https://remote.bitfun.com/v/1.0.1", devices = listOf(AccountDeviceUi("desktop-1", "Desktop", true, null)), selectedDeviceId = null, selectedDeviceName = null)
    ...
    @Test fun rejectsLegacyAndLookalikeLinksWithoutUsingTheirRoutingFields() {
        for (invalid in listOf(
            link.replace("https:", "http:"), link.replace("remote.bitfun.com", "evil.example"),
            link.replace("remote.bitfun.com", "user@remote.bitfun.com"),
