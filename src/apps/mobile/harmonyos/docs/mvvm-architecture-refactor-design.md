...
- bundle 名以 `AppScope/app.json5` 为准，和手表端共用且作为升级兼容标识保持稳定；BitFun 是产品展示名，不迁移 package identity、Preferences/HUKS/RDB/KV 的持久化标识。`distributedKVStore` 按 bundleName + storeId 隔离，任意改名都会让手机与手表落入互不相干的库，并让已安装用户丢失配对与登录恢复能力；
