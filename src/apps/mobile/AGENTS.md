...
- Build and test from `shared/`: `./gradlew jvmTest` for host logic,
  ...
  `xcodebuild -project src/apps/mobile/ios/BitFun.xcodeproj -scheme BitFun -configuration Debug -destination 'platform=iOS Simulator,name=iPhone 16 Plus' -parallel-testing-enabled NO -only-testing:BitFunUITests/StreamingPresentationUITests test`.
