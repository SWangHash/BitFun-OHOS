...
enum MobileLaunchConfiguration {
    ...
    static func makeModel() -> MobileAppModel {
        ...
        let model = MobileAppModel(
            ...
            messages: [
                ...
                ChatMessage(id: UUID(), role: .assistant, text: "这是 BitFun 的移动端会话界面。你可以从手机连接桌面端，查看工作区、会话和智能体的执行状态。")
            ],
    ...
    private static func configure(_ model: MobileAppModel) -> MobileAppModel {
        ...
        if arguments.contains("--download-preview") {
            let previewURL = FileManager.default.temporaryDirectory.appendingPathComponent("bitfun-download-preview.rs")
...
private extension MobileAppModel {
    func configureConnectedPreview() {
        ...
        workspaceCatalog = [(path: "/workspace/BitFun", name: "BitFun", selected: true, remoteConnectionId: nil)]
        remoteAssistants = [
