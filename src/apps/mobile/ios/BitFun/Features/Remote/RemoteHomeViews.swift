...
struct RemoteConnectedHomeView: View {
    ...
    private func homeButton(_ title: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(model.localized(title))
                ...
                .background(BitFunTheme.soft, in: RoundedRectangle(cornerRadius: 12))
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                ForEach(recent) { session in
                    ...
                    Rectangle().fill(BitFunTheme.line).frame(height: 0.5)
                }
                if recent.isEmpty && presentation == .ready {
                    Text(model.localized("还没有可继续的对话"))
                        .font(.system(size: 13)).foregroundStyle(BitFunTheme.muted).padding(.vertical, 18)
                }
        ...
        }.foregroundStyle(BitFunTheme.ink).background(BitFunTheme.page)
    }
}
...
struct RemoteConversationStatusBar: View {
    ...
    private var tone: Color { failed ? BitFunTheme.statusDanger : BitFunTheme.muted }
    private var title: String {
        model.localized(model.isSending ? "BitFun 正在执行..." :
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            HStack(spacing: 7) {
                ...
                Text(detail.isEmpty || detail == title ? title : "\(title) · \(detail)")
                    ...
                    .foregroundStyle(model.isSending ? BitFunTheme.muted : tone)
                    .lineLimit(1)
                    .truncationMode(.tail)
            }
            ...
            if model.isSending {
                Button(model.localized("停止"), action: model.stopSending)
                    ...
                    .background(BitFunTheme.soft, in: Capsule())
                    .buttonStyle(.plain)
            }
        }
        ...
        .overlay(alignment: .top) { BitFunTheme.line.frame(height: 1) }
        .overlay(alignment: .bottom) { BitFunTheme.line.frame(height: 1) }
...
struct WelcomeHomeView: View {
    ...
    private var phrases: [String] { ["BitFun", model.localized("干点活儿"), model.localized("玩点花样"), model.localized("随你发挥")] }
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 0) {
                    HStack {
                        HStack(spacing: 0) { ForEach(Array("BitFun".enumerated()), id: \.offset) { _, c in welcomeGlyph(c, size: MobileDesignGeometry.welcomeHeaderWordSize) } }
                        Spacer()
                    }
    ...
    private func duration(_ word: String) -> Double { 250+Double(word.count)*185+(word == "BitFun" ? 600 : 0)+1800+650 }
    private func phrase(at date: Date) -> (word: String,time: Double) {
        if reduceMotion { return ("BitFun",2700) }
