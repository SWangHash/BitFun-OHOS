...
private struct ConversationRowView: View, Equatable {
    ...
    private func assistantFailure(_ error: String) -> some View {
        ...
        .overlay(alignment: .leading) {
            Rectangle().fill(BitFunTheme.statusDanger).frame(width: 2)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Button { withAnimation(.easeOut(duration: 0.18)) { expanded.toggle() } } label: {
                ...
                .foregroundStyle(failed ? BitFunTheme.statusDanger : BitFunTheme.muted)
                .frame(minHeight: 32)
        ...
        .padding(.leading, 12)
        .overlay(alignment: .leading) { Rectangle().fill(BitFunTheme.line).frame(width: 2) }
    }
...
struct MarkdownMessageView: View {
    ...
    var body: some View {
        ...
        .environment(\.openURL, OpenURLAction { url in
            if url.scheme == nil || ["computer", "file", "bitfun"].contains(url.scheme?.lowercased() ?? "") {
    ...
    var body: some View {
        ...
        case "heading":
            Text(inlineString(block.inlines))
                .font(headingFont)
                .foregroundStyle(BitFunTheme.ink).textSelection(.enabled)
        case "quote":
            Text(inlineString(block.inlines))
                .font(MobileDesignTypography.bodyLarge.font).foregroundStyle(BitFunTheme.muted)
                ...
                ForEach(block.items, id: \.id) { item in
                    HStack(alignment: .firstTextBaseline, spacing: 7) {
                        if let checked = taskListState(item.text) {
                            ...
                            Text(item.marker).foregroundStyle(BitFunTheme.muted)
                                .frame(width: 20, alignment: .trailing)
                        }
                        Text(listItemInlineString(item)).foregroundStyle(BitFunTheme.ink)
                            .lineSpacing(MobileDesignTypography.bodyLarge.lineSpacing)
                            .textSelection(.enabled)
                    }
                    .font(MobileDesignTypography.bodyLarge.font)
        ...
        case "divider": Rectangle().fill(BitFunTheme.line).frame(height: 1).padding(.vertical, 3)
        default:
            Text(inlineString(block.inlines))
                .font(MobileDesignTypography.bodyLarge.font).foregroundStyle(BitFunTheme.ink)
...
private struct MarkdownTableView: View {
    ...
    var body: some View {
        ...
        .background(BitFunTheme.card)
        .clipShape(RoundedRectangle(cornerRadius: 10))
        .overlay(RoundedRectangle(cornerRadius: 10).stroke(BitFunTheme.line, lineWidth: 1))
    }
                    ...
                    ForEach(Array(row.cells.enumerated()), id: \.offset) { columnIndex, cell in
                        ...
                        .overlay(alignment: .trailing) {
                            if columnIndex < row.cells.count - 1 {
                                Rectangle().fill(BitFunTheme.line).frame(width: 1)
                            }
                        }
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text(language.isEmpty ? MobileLocalization.text("代码") : language)
                    .font(MobileDesignTypography.labelSmall.font).foregroundStyle(BitFunTheme.muted)
                Spacer()
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                Text(code).font(.system(size: 12.5, design: .monospaced))
                    .foregroundStyle(BitFunTheme.ink).padding(12).textSelection(.enabled)
            }
        }
        .background(BitFunTheme.soft).clipShape(RoundedRectangle(cornerRadius: 12))
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button { model.downloadRemoteFile(reference: reference.reference, label: reference.label) } label: {
                ...
                .foregroundStyle(BitFunTheme.muted).frame(width: 40, height: 40)
            }
            .buttonStyle(.plain)
            .accessibilityLabel(model.localizedFormat("下载 %@", reference.label))
        }
        .padding(.leading, 10).padding(.trailing, 4).padding(.vertical, 7)
        .background(BitFunTheme.card).clipShape(RoundedRectangle(cornerRadius: 14))
                ...
                case let .tool(tool):
                    if let path = tool.planPath {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            .background(BitFunTheme.soft).clipShape(RoundedRectangle(cornerRadius: 14))
        ...
        Button { withAnimation(.easeOut(duration: 0.18)) { expanded.toggle() } } label: {
            HStack(spacing: 8) {
                Image(systemName: "doc.on.doc").font(.system(size: 12, weight: .medium))
                    .frame(width: 20, height: 20).background(BitFunTheme.soft)
            ...
            .foregroundStyle(BitFunTheme.muted).frame(minHeight: 32)
                    .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
...
private struct ToolStatusRow: View {
    ...
    private var legacyAnswerPanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            TextField(model.localized("回复"), text: $answer, axis: .vertical)
                ...
                .background(BitFunTheme.card).clipShape(RoundedRectangle(cornerRadius: 11))
            ...
            ForEach(tool.questions) { question in
                VStack(alignment: .leading, spacing: 7) {
                    if !question.header.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        Text(question.header).font(MobileDesignTypography.labelMedium.font).foregroundStyle(BitFunTheme.muted)
                    }
                    Text(question.question).font(MobileDesignTypography.bodySmall.font).foregroundStyle(BitFunTheme.ink)
                    ForEach(options(for: question)) { option in
                        ...
                        Button { toggle(option.label, for: question) } label: {
                            HStack(alignment: .top, spacing: 9) {
                                Image(systemName: selected ? (question.multiSelect ? "checkmark.square.fill" : "largecircle.fill.circle") : (question.multiSelect ? "square" : "circle"))
                                    .foregroundStyle(selected ? BitFunTheme.accent : BitFunTheme.muted)
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(option.label).font(MobileDesignTypography.bodySmall.font).foregroundStyle(BitFunTheme.ink)
                                    if let description = option.description, !description.isEmpty {
                                        Text(description).font(MobileDesignTypography.labelSmall.font).foregroundStyle(BitFunTheme.muted)
                                    }
                                }
                                Spacer(minLength: 0)
                            }
                        ...
                        if selected && isOther(option) {
                            ...
                            .disabled(model.busy)
                            .background(BitFunTheme.card).clipShape(RoundedRectangle(cornerRadius: 9))
                            .overlay(RoundedRectangle(cornerRadius: 9).stroke(BitFunTheme.line, lineWidth: 1))
                        }
                    }
                }
            }
    ...
    private func detailText(_ title: String, _ value: String) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            Text(title).font(MobileDesignTypography.labelSmall.font).foregroundStyle(BitFunTheme.muted)
            Text(value).font(.system(size: 12.5, design: .monospaced)).foregroundStyle(BitFunTheme.ink)
