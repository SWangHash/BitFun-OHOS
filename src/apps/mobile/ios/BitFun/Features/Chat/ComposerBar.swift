...
struct ComposerBar: View {
    ...
    var body: some View {
        ...
        .overlay(
            ...
            .stroke(BitFunTheme.line, lineWidth: 0.5)
        )
        .clipShape(
            RoundedRectangle(
                cornerRadius: expanded || !model.composerImages.isEmpty
    ...
    private var primaryAction: some View {
        Button(action: performPrimaryAction) {
            ZStack {
                ...
                case .stopListening, .stopTurn:
                    RoundedRectangle(cornerRadius: 2.5)
                        .fill(BitFunTheme.contentOnAction)
                    ...
                    ReferenceGlyph(
                        ...
                        color:
                            primaryActionKind == .voice
                                ? BitFunTheme.ink
                                : BitFunTheme.muted.opacity(0.38)
                    )
                }
            }
            .frame(
            ...
            .background(primaryActionKind == .voiceBlocked ? BitFunTheme.soft : BitFunTheme.transparent)
            .clipShape(Circle())
        }
        .buttonStyle(.plain)
