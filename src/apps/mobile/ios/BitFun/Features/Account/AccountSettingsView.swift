    ...
    private var loginPage: some View {
        VStack(spacing: 0) {
            ...
            ConnectionSheetFooter(
                label: model.localized(model.accountAuthorizationURL != nil ? "打开 BitFun 授权"
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: "desktopcomputer")
                ...
                .foregroundStyle(connected ? BitFunTheme.ink : BitFunTheme.muted)
        ...
        .padding(.vertical, 10)
        .frame(minHeight: 62)
        .background(connected ? BitFunTheme.soft : BitFunTheme.card)
    ...
    var body: some View {
        AsyncImage(url: url.flatMap { value in
            ...
            ZStack {
                ...
                Image(systemName: "person.fill").foregroundStyle(BitFunTheme.ink)
            }
