                        ...
                        case .ready:
                            ...
                            VStack {
                                ...
                                if model.savedRuntimeConnectionsFailed {
                                    Text(model.localized("无法加载已保存连接，请刷新重试。")).foregroundStyle(BitFunTheme.muted)
                                }
        ...
        HStack(spacing: 12) {
            HStack(spacing: 2) {
                ForEach(0..<density, id: \.self) { index in
                    RoundedRectangle(cornerRadius: 2).fill(BitFunTheme.ink)
...
private struct NativeRuntimeTerminalView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        configuration.userContentController.add(context.coordinator, name: "bitfunTerminal")
    ...
    static func dismantleUIView(_ view: WKWebView, coordinator: Coordinator) {
        ...
        view.configuration.userContentController.removeScriptMessageHandler(forName: "bitfunTerminal")
        view.stopLoading()
    }
    final class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
        ...
        func render(force: Bool) {
            ...
            view?.evaluateJavaScript("window.BitFunTerminal.accept(\(json))")
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) { webView.evaluateJavaScript("window.BitFunTerminal.connect()") }
...
private final class NumberedCodeEditorView: UIView, UITextViewDelegate {
    ...
    override init(frame: CGRect) {
        ...
        for view in [editor, gutter] {
            ...
            view.backgroundColor = UIColor(BitFunTheme.card)
            view.textColor = UIColor(BitFunTheme.ink)
        ...
        gutter.textColor = UIColor(BitFunTheme.muted)
    ...
    func setContent(_ content: String) {
        ...
        editor.attributedText = NSAttributedString(string: content, attributes: [.font: codeFont, .paragraphStyle: paragraph, .foregroundColor: UIColor(BitFunTheme.ink)])
        editor.typingAttributes = [.font: codeFont, .paragraphStyle: paragraph, .foregroundColor: UIColor(BitFunTheme.ink)]
        updateLines()
    }
    func updateLines() {
