...
struct SessionActionSurface: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Divider().overlay(BitFunTheme.line).padding(.top, 6).padding(.bottom, 8)
    ...
    private var actionRows: some View {
        ...
        if canDelete {
            if canArchive || canExport {
                Divider().overlay(BitFunTheme.line).padding(.vertical, 6)
            }
...
struct SessionDetailsView: View {
    ...
    private func detailRow(_ label: String, value: String) -> some View {
        ...
        .padding(.vertical, 8)
        .frame(minHeight: 52)
        .overlay(alignment: .bottom) { Divider().overlay(BitFunTheme.line) }
    }
    ...
    private func pathRow(_ path: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(path)
                ...
                .frame(maxWidth: .infinity, alignment: .leading)
                .background(BitFunTheme.card)
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(BitFunTheme.line, lineWidth: 1))
