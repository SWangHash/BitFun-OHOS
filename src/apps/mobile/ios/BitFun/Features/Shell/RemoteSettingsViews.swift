        ...
        Button(action: action) {
            HStack(spacing: 12) {
                Image(systemName: selected ? "checkmark.circle.fill" : "circle")
                    ...
                    .foregroundStyle(selected ? BitFunTheme.ink : BitFunTheme.muted)
                    .frame(width: 22)
                Text(model.localized(title))
            ...
            .background(selected ? BitFunTheme.card : BitFunTheme.transparent)
...
struct RemoteControlSettingsView: View {
    ...
    private func remoteCard<Content: View>(
        ...
        BitFunModalCard(radius: radius, bordered: false, content: content)
    }
