...
struct RemoteFilePreviewSheet: View {
    ...
    private func color(_ kind: CodeSyntaxTokenKind) -> Color {
        ...
        default: return BitFunTheme.ink
        }
    }
                                ...
                                LazyVStack(alignment: .leading, spacing: 0) {
                                    ForEach(Array(codeLines.enumerated()), id: \.offset) { index, line in
                                        Text(line)
                                            ...
                                            .background(index + 1 >= Int(preview.lineStart) && index + 1 <= Int(max(preview.lineStart, preview.lineEnd)) && preview.lineStart > 0 ? MobileDesignColors.codeTargetBg : BitFunTheme.transparent)
                                            .background(GeometryReader { geometry in
                                                BitFunTheme.transparent.preference(
                                                    key: FilePreviewVisibleLinePreferenceKey.self,
                                                    value: {
