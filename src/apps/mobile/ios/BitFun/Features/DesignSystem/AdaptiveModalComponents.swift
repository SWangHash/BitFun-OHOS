    ...
    var body: some View {
        VStack(spacing: spacing) {
            if showScan {
                Button(action: onScan) {
                    Text(scanTitle)
                        ...
                        .foregroundStyle(primaryScan ? BitFunTheme.contentOnAction : BitFunTheme.ink)
                        .frame(maxWidth: .infinity, minHeight: buttonHeight)
                        .background(primaryScan ? MobileDesignColors.primaryAction : BitFunTheme.card)
            ...
            Button(action: onOpenAccount) {
                Text(accountTitle)
                    .font(.system(size: fontSize, weight: .bold))
                    .foregroundStyle(primaryScan ? BitFunTheme.ink : BitFunTheme.contentOnAction)
                    .frame(maxWidth: .infinity, minHeight: buttonHeight)
                    .background(primaryScan ? BitFunTheme.card : MobileDesignColors.primaryAction)
                    .overlay(Capsule().stroke(primaryScan ? BitFunTheme.line : MobileDesignColors.primaryAction, lineWidth: 1))
                    .clipShape(Capsule())
            }
            .buttonStyle(.plain)
            .disabled(!enabled)
...
private struct BitFunAdaptiveModalModifier<ModalContent: View>: ViewModifier {
    ...
    private var fittedCover: some View {
        if #available(iOS 16.4, *) {
            fittedPanel.presentationBackground(BitFunTheme.transparent)
        } else {
            fittedPanel
        }
    }
    ...
    private var fittedPanel: some View {
        GeometryReader { geometry in
            ZStack(alignment: .bottom) {
                BitFunTheme.scrim.ignoresSafeArea()
            ...
            Button(action: onClose) {
                Group {
                    if uniformGlyph {
                        ...
                        .stroke(BitFunTheme.ink, style: StrokeStyle(lineWidth: 1.5, lineCap: .round))
    ...
    var body: some View {
        Button(action: onAction) {
            Text(label)
                ...
                .background(primary ? MobileDesignColors.primaryAction : BitFunTheme.card)
                .clipShape(Capsule())
                .overlay(Capsule().stroke(primary ? BitFunTheme.transparent : BitFunTheme.line, lineWidth: 1))
