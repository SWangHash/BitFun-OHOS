...
struct PairingSheet: View {
    ...
    @State private var pairingURL = MobileLaunchConfiguration.pairingAccountPreview
        ? "https://remote.bitfun.com/v/1.0.1/#/pair?did=preview-device"
        : ""
                    ...
                    Button {
                        ...
                        HStack(spacing: 12) {
                            Image(systemName: "link")
                                ...
                                .foregroundStyle(BitFunTheme.muted.opacity(0.66))
                            ...
                            Image(systemName: "chevron.right")
                                ...
                                .foregroundStyle(BitFunTheme.muted.opacity(0.44))
                        }
                        .padding(.horizontal, 16)
                        .frame(height: 58)
                        .background(BitFunTheme.card)
                        .clipShape(RoundedRectangle(cornerRadius: 8))
                        .overlay(RoundedRectangle(cornerRadius: 8).stroke(BitFunTheme.line, lineWidth: 1))
                    }
                    .buttonStyle(.plain)
                }
                .frame(maxWidth: 520)
            ...
            HStack {
                ...
                Button(action: model.refreshRemoteDevices) {
                    Text(model.localized(model.accountRefreshing ? "正在加载" : "刷新"))
                        .font(MobileDesignTypography.bodyMedium.font)
                        .foregroundStyle(model.accountRefreshing ? BitFunTheme.muted : BitFunTheme.accent)
    ...
    private var accountDeviceSkeleton: some View {
        HStack(spacing: 12) {
            RoundedRectangle(cornerRadius: 5).fill(BitFunTheme.soft).frame(width: 26, height: 22)
            VStack(alignment: .leading, spacing: 7) {
                RoundedRectangle(cornerRadius: 4).fill(BitFunTheme.soft).frame(width: 142, height: 12)
                RoundedRectangle(cornerRadius: 4).fill(BitFunTheme.soft).frame(width: 52, height: 9)
            }
            Spacer(minLength: 0)
        }
        ...
        Button {
            ...
            HStack(spacing: 12) {
                Image(systemName: "desktopcomputer")
                    ...
                    .foregroundStyle(BitFunTheme.muted.opacity(device.online ? 0.68 : 0.38))
                ...
                VStack(spacing: 18) {
                    ZStack(alignment: .topLeading) {
                        RoundedRectangle(cornerRadius: 8)
                            .strokeBorder(BitFunTheme.ink, lineWidth: 5)
                            .frame(width: 58, height: 39).offset(x: 5)
                        Rectangle().strokeBorder(BitFunTheme.ink, lineWidth: 5)
            ...
            GeometryReader { geometry in
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        Text(model.localized("在 BitFun 桌面端点击「连接移动端」\n扫描二维码完成连接"))
                            .font(MobileDesignTypography.bodyMedium.font)
                        ...
                        inlineScanner
                        HStack(spacing: 8) {
                            Circle().fill(BitFunTheme.muted).frame(width: 8, height: 8)
    ...
    private var inlineScanner: some View {
            ...
            QRCodeScannerView(
                ...
                onPermissionDenied: {
                    scanError = model.localized(
                        "需要相机权限才能扫码，请在系统设置中允许 BitFun 访问相机，或改为手动配对。"
                    )
                },
                onUnavailable: {
                    scanError = model.localized(
        ...
        .overlay(RoundedRectangle(cornerRadius: 28).stroke(BitFunTheme.line, lineWidth: 1))
    }
