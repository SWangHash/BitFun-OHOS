                ...
                self.spawn_hook_management(
                    async move {
                        ...
                        let native = project_native_hook_overview(
                            bitfun_core::native_hooks::overview(workspace_id.as_deref())
                                .await
    ...
    fn start_hook_store_reset(
        ...
        self.spawn_hook_management(
            async move {
                ...
                .await?;
                let native = project_native_hook_overview(
                    bitfun_core::native_hooks::overview(workspace_id.as_deref()).await.map_err(ExternalSourceOperationError::invalid_request)?,
                    &workspace,
                );
                Ok(HookManagementResult::Changed {
                    snapshot: HookManagementSnapshot { native, imports },
