...
//! owned by `bitfun_core::service::remote_connect::relay_failure`, so this
...
//! reads what they can act on (retry, check the network, or update BitFun).

use bitfun_core::service::remote_connect::relay_failure::{
...
const UPDATE_SENTENCE: &str =
    "This BitFun build is no longer served by the relay. Update BitFun, then sign in again.";
/// Second line for the update kinds; it restates the one concrete action.
const UPDATE_HINT: &str = "Update BitFun on this device to continue.";
...
const UNKNOWN_SENTENCE: &str =
    "The relay rejected the request. Check the network or update BitFun, then retry.";
...
mod tests {
    use super::*;
    use bitfun_product_domains::account::DEFAULT_RELAY_URL;
