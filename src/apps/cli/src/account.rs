...
pub(crate) struct CliAccountRoutingHost {
    host_stream_hub:
        RwLock<Option<Arc<bitfun_core::service::remote_connect::host_stream::HostStreamHub>>>,
    ...
    async fn handle_current_relay_event(
        ...
        match event {
                    ...
                    .await
                {
                    ...
                    if session.token == expected_token
                        && self
                            ...
                            .await
                    {
                        let hub = bitfun_core::service::remote_connect::start_host_stream_hub(
            ...
            RelayEvent::DeviceMessageReceived {
                ...
                let response =
                    match bitfun_core::service::remote_connect::handle_host_stream_command(
                        hub.as_ref(),
                        &source_device_id,
                        &command,
                    )
    ...
    fn notify(&self, target_device_id: &str, payload: serde_json::Value) {
        ...
        tokio::spawn(async move {
            ...
            let envelope = match serde_json::to_string(&RemoteCommand::DeviceEvent {
                event:
                    bitfun_core::service::remote_connect::host_stream::HOST_STREAM_CHANGED_EVENT
                        .to_owned(),
                payload,
            }) {
