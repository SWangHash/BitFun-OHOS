...
describe('market image URLs', () => {
  it('adds a versioned image variant without losing existing URL state', () => {
    expect(marketImageUrl('/miniapp/api/v1/screenshots/hash?token=one#focus', 'compact-v1'))
      .toBe('/miniapp/api/v1/screenshots/hash?token=one&variant=compact-v1#focus');
  });
