...
```json
{"error":"relay_version_retired","message":"…Update BitFun on this device, then sign in again to continue.","update_required":true}
```
...
1. **Relay enforced (while a container still serves the prefix).** Point the
   ...
   ```nginx
   location ^~ /v/1.0.0/ {
       proxy_set_header X-BitFun-Relay-Served-Prefix /v/1.0.0;
...
```ini
# /etc/systemd/system/bitfun-relay-1.0.1-prune.service
[Unit]
Description=Prune the legacy BitFun Relay 1.0.1 realtime message log
...
StandardOutput=append:/var/log/bitfun-relay-1.0.1-prune.log
StandardError=append:/var/log/bitfun-relay-1.0.1-prune.log

# /etc/systemd/system/bitfun-relay-1.0.1-prune.timer
[Unit]
Description=Hourly prune of the legacy BitFun Relay 1.0.1 message log

[Timer]
OnCalendar=hourly
RandomizedDelaySec=300
Persistent=true
Unit=bitfun-relay-1.0.1-prune.service

[Install]
WantedBy=timers.target
```

`systemctl enable --now bitfun-relay-1.0.1-prune.timer`. The steady-state run
