...
class MarketDataMigrationTest(unittest.TestCase):
    def test_miniapp_migrates_to_the_single_bitfun_field_and_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for (raw,) in rows:
                ...
                self.assertEqual(value["minBitFunVersion"], "1.0.0")
    ...
    def test_skin_rewrites_packages_hashes_and_review_bundles(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(json.loads(draft_json)["minBitFunVersion"], "1.0.0")
            self.assertEqual(migrated_manifest["schema"], "bitfun.appearance")
            self.assertNotEqual(new_sha, retired_sha)

            migrated_path = migration.skin_package_path(
                artifacts, new_sha, migration.BITFUN_APPEARANCE_EXTENSION
            )
