...
脚本里的 `BITFUN_BASE_URL` 写死为 `https://bitfun.com/release`。
域名变了要改 `scripts/bitfun-release-sync.sh`，不能只改 Nginx。
...
```bash
OLD_HOST=lwb
NEW_HOST=new-bitfun   # 换成新机器 SSH 别名
...
`test -d /srv/bitfun-release/1.0.0`、`test -f /etc/bitfun-miniapp-market/market.env`。
...
`https://github.com/GCWing/BitFun/releases/latest/download/latest-v1.json` 的
...
`BITFUN_RELEASE_CHANNEL=beta` 读取的是独立的 `channel-v1-beta`。

#### 已有源站的 1.X 同步切换

2026-09-11 的故障来自部署未对齐：root cron 仍执行
`/root/repos/BitFun/scripts/bitfun-release-sync.sh`，读取兼容旧客户端的
`latest.json` 得到 0.2.19，随后因 Relay 仓库名称校验失败退出。只更新
BitFun checkout 不会修改已有 crontab。

在仍运行的源站修复时，先备份当前 crontab、同步脚本与根清单，再核对：

- cron 只执行 `/root/repos/BitFun/scripts/bitfun-release-sync.sh`，日志为
  `/var/log/bitfun-release-sync.log`。只替换该任务行，保留其它任务。
- 实际执行的脚本读取 `latest-v1.json` 和 `linux-binaries-v1.json`，并校验
  `ghcr.io/gcwing/bitfun-relay-server`。不要放宽校验来兼容错配的新旧清单。
- 脚本输出与 Nginx `/release/` alias 必须指向同一目录。当前 `lwb` 保留
  `/srv/bitfun-release` 的历史文件，以 `/srv/bitfun-release` 软链接指向它；
...
新服务只接受 `minBitFunVersion`、`bitfun.appearance`、
`.bitfun-appearance` 和 `bitfun_relay.db`，运行时不保留旧字段或旧文件名回退。
因此只能在旧容器停止后，先把数据库和 artifacts 复制到新的
`/srv/bitfun-*` 目录，再对**新副本**运行迁移工具。`--live-database` 是防误操作
...
`.backup` 把卷内 `bitfun_relay.db` 复制为 `bitfun_relay.db`，保留旧文件作为回滚
...
- 镜像：`new-api:v1.0.0-rc.14-bitfun-mod`
- 离线包：`/root/repos/new_api_bak/new-api-v1.0.0-rc.14-bitfun-mod.tar`
...
`/root/repos/BitFun/scripts/bitfun-release-sync.sh`；`list-users` 含旧账号。
...
| 更新 Release 镜像脚本 | 只改仓库里的 `scripts/bitfun-release-sync.sh`。cron 已经跑这份文件。 |
