#!/usr/bin/env python3
"""One-time BitFun 0.2.x -> BitFun 1.x market data migration.

Run this only against an offline copy of the production database and artifact
tree. The BitFun services intentionally keep no legacy field aliases, so
...
CURRENT_VERSION_FIELD = "minBitFunVersion"
RETIRED_VERSION_FIELDS = ("minBitFunVersion", "minBitfunVersion")
BITFUN_MINIMUM_VERSION = "1.0.0"
RETIRED_APPEARANCE_SCHEMA = "bitfun.appearance"
BITFUN_APPEARANCE_SCHEMA = "bitfun.appearance"
RETIRED_APPEARANCE_EXTENSION = "bitfun-appearance"
BITFUN_APPEARANCE_EXTENSION = "bitfun-appearance"
...
def migrate_version_field(value: dict[str, Any], context: str) -> bool:
    ...
    value[CURRENT_VERSION_FIELD] = BITFUN_MINIMUM_VERSION
    return True
...
def write_migrated_skin_archive(source: Path, temporary_root: Path) -> tuple[Path, str, int, str]:
    ...
    file_descriptor, temporary_name = tempfile.mkstemp(
        prefix="skin-v1-", suffix=".bitfun-appearance", dir=temporary_root
    )
        ...
        with zipfile.ZipFile(source, "r") as incoming, zipfile.ZipFile(
            ...
            for entry in incoming.infolist():
                ...
                if entry.filename == "appearance.json":
                    ...
                    manifest["schema"] = BITFUN_APPEARANCE_SCHEMA
...
def migrate_skin_packages(
    ...
    temporary_root = artifacts / ".migration-bitfun-v1"
    migrated_packages = 0
    for (old_sha256,) in rows:
        ...
        new_existing = skin_package_path(
            artifacts, old_sha256, BITFUN_APPEARANCE_EXTENSION
        )
        ...
        new_path = skin_package_path(
            artifacts, new_sha256, BITFUN_APPEARANCE_EXTENSION
        )
