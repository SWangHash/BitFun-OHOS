// Protocol smoke test for the dsh-bitfun bridge: drives a real
// `bitfun-sdk-host` process through initialize -> session/create ->
...
const HOST_BINARY_BASE = process.platform === 'win32' ? 'bitfun-sdk-host.exe' : 'bitfun-sdk-host';

function resolveHost() {
  const explicit = process.argv[2] ?? process.env.BITFUN_SDK_HOST;
...
try {
  const init = await request('initialize', {
    protocolVersion: 1,
    clientInfo: { name: 'dsh-bitfun-smoke', version: '0.1.0' },
    capabilities: { serverNotifications: true },
  });
  console.log(`initialized: protocol ${init.protocolVersion}, runtime ${init.runtimeVersion}`);

  const created = await request('session/create', { sessionName: 'dsh-bitfun smoke', cwd: PACKAGE_ROOT });
