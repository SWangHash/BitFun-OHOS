// dsh-bitfun: a DeepSeek Harness (dsh) plugin bundle that exposes the BitFun
// Agent SDK Host — a standalone `bitfun-sdk-host` process speaking
...
//   3. `<session cwd>/target/{debug,release}/bitfun-sdk-host(.exe)` walking up
//      the directory tree (a BitFun checkout-local build),
...
export const name = 'bitfun';
export const inject = ['tools', 'systemPrompt'];

const VERSION = '0.1.0';
const PROTOCOL_VERSION = 1;
const HOST_BINARY_NAME = 'bitfun-sdk-host';
...
function resolveHostPath(config, cwd) {
  ...
  const fromEnv = isNonEmptyString(process.env.BITFUN_SDK_HOST) ? process.env.BITFUN_SDK_HOST : undefined;
...
/**
 * One connection to a `bitfun-sdk-host` process: newline-delimited JSON-RPC
...
class SdkHostClient {
  ...
  stderrSuffix() {
    ...
    return tail === '' ? '' : `\nBitFun SDK Host stderr tail:\n${tail}`;
  }

  async ensureReady() {
    if (this.disposed) throw new Error('the BitFun SDK Host bridge is disposed');
  ...
  async #boot() {
    ...
    child.once('exit', (code, signal) => {
      ...
      this.readyPromise = null;
      const message =
        `BitFun SDK Host process exited unexpectedly (code ${code ?? 'null'}, signal ${signal ?? 'null'})` +
        this.stderrSuffix();
    ...
    child.once('error', (error) => {
      ...
      spawnErrorDeferred.reject(
        new HostError(`cannot start BitFun SDK Host "${this.spawnSpec}": ${error.message}`, 'SPAWN_FAILED', error),
      );
    });
      ...
      const result = await Promise.race([
        this.#requestRaw(
          'initialize',
          {
            ...
            clientInfo: { name: 'dsh-bitfun', version: VERSION },
  ...
  #requestRaw(method, params, timeoutMs = this.requestTimeoutMs) {
    ...
    const timer = setTimeout(() => {
      ...
      pending.reject(new HostError(`BitFun SDK Host request timed out: ${method} after ${timeoutMs}ms`, 'TIMEOUT'));
    }, timeoutMs);
    this.pending.set(id, {
      resolve: (value) => {
        clearTimeout(timer);
    ...
    if (child === null || child.stdin.destroyed) {
      this.pending.delete(id);
      clearTimeout(timer);
      pending.reject(new HostError('the BitFun SDK Host is not running', 'NOT_RUNNING'));
    ...
    child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id, method, params: params ?? {} })}\n`, (error) => {
      ...
      if (this.pending.delete(id)) {
        clearTimeout(timer);
        pending.reject(
          new HostError(`cannot write to BitFun SDK Host stdin: ${error.message}`, 'WRITE_FAILED', error),
        );
      }
    });
    return pending.promise;
  ...
  #dispatch(message) {
    ...
    if (message.error !== undefined && message.error !== null) {
      ...
      entry.reject(
        new HostError(
          `BitFun SDK Host error: ${String(message.error?.message ?? 'request failed')}`,
          'RPC_ERROR',
          undefined,
          data,
        ),
  ...
  /**
   * Start one BitFun turn and wait for its terminal `query/result`.
  ...
  async startQuery(params) {
    ...
    if (typeof started?.queryId !== 'string' || typeof started?.sessionId !== 'string') {
      throw new HostError('BitFun SDK Host returned an invalid query/start response', 'PROTOCOL');
    }
...
const RUN_TOOL_PARAMETERS = {
  ...
  properties: {
    prompt: {
      type: 'string',
      description:
        'The task for the BitFun agent. BitFun runs it autonomously with its own agent loop and returns its final text answer.',
    },
    cwd: {
      type: 'string',
      description:
        'Optional absolute workspace directory for the BitFun session. Defaults to this dsh session cwd.',
    },
    session_id: {
      type: 'string',
      description:
        'Optional BitFun session id returned by a previous bitfun_run call. Pass it to continue that BitFun conversation instead of starting a new one.',
    },
    agent: {
      type: 'string',
      description: 'Optional BitFun agent type for a new session (default: BitFun built-in default).',
    },
    model: {
      type: 'string',
      description:
        'Optional BitFun model id for a new session (default: the user configured BitFun model).',
    },
  },
  required: ['prompt'],
};
...
function renderRunOutput(_args, value) {
  const text = value.status === 'completed'
    ? value.text
    : `BitFun turn ended with status "${value.status}".\n` +
...
/**
 * Register the `bitfun_run` tool plus a short system-prompt section.
 * One SDK Host client is kept per resolved binary; BitFun sessions are
 * transient per Host connection, so the plugin maps each dsh session to one
 * BitFun session and closes the oldest mappings past `maxSessions`.
 */
export function apply(ctx, config) {
  ...
  function ensureClient(cwd) {
    ...
    if (resolved === null) {
      throw new Error(
        'cannot locate the BitFun SDK Host binary ("bitfun-sdk-host"). ' +
          'Build it with `cargo build -p bitfun-sdk-host-app`, set the BITFUN_SDK_HOST environment variable, ' +
          'or configure the hostPath option of the dsh-bitfun plugin row.',
      );
    }
  ...
  const bitfunRunTool = {
    name: 'bitfun_run',
    description:
      'Delegate a task to the BitFun agent runtime: BitFun executes the prompt in a workspace with its own ' +
      'agent loop and tools, then returns its final text answer. The result includes a session_id; pass it in a ' +
      'later call to continue the same BitFun conversation. BitFun interactive permission prompts are auto-rejected ' +
    ...
    async execute(args, exec) {
      ...
      const createSession = async () => {
        ...
        if (typeof created?.sessionId !== 'string' || created.sessionId === '') {
          throw new HostError('BitFun SDK Host returned an invalid session/create response', 'PROTOCOL');
        }
      ...
      try {
        ...
        // stale mapping and retry once with a fresh BitFun session.
        sessions.delete(key);
    ...
    presentCall(args) {
      return {
        card: 'generic',
        title: `BitFun: ${truncate(args.prompt, 80)}`,
        kind: 'tool',
      };
    },
    presentResult() {
      return { card: 'generic', title: 'BitFun', kind: 'tool' };
    },
  };

  ctx.tools.register(bitfunRunTool);

  ctx.systemPrompt.section({
    name: 'tool:bitfun',
    order: 200,
    text:
      'Use the bitfun_run tool to delegate a task to the BitFun agent runtime when the user asks to use BitFun, ' +
      'or when handing a whole subtask to BitFun is appropriate. BitFun runs the prompt autonomously in the ' +
      'workspace and returns its final text answer; pass the returned session_id back to continue that BitFun ' +
      'conversation across calls. Do not use it for tasks that need interactive permission approval, because the ' +
      'SDK host auto-rejects BitFun permission prompts.',
  });
