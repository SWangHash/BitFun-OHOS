...
test('theme contract defaults to system and exposes all three choices', () => {
  ...
  assert.match(templateSource, /data-bitfun-design-system-root/u);
  ...
  assert.match(appSource, /--bitfun-color-surface-canvas/u);
...
test('callouts and ordinary UI consume canonical semantic color tokens only', () => {
  assert.match(cssBlock('.detail-aside'), /background:\s*var\(--bitfun-color-status-success-surface\)/u);
  assert.match(cssBlock('.detail-aside'), /color:\s*var\(--bitfun-color-status-success-content\)/u);
  assert.match(cssBlock('.control-unsupported'), /color:\s*var\(--bitfun-color-status-danger-content\)/u);
